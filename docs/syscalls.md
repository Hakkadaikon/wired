...
`wired` is libc-free (`-ffreestanding -nostdlib`): every kernel interaction
goes through a typed `wired_arch_<name>` wrapper
(`src/common/arch/sysops.h`), reached by every domain via the facade
`src/common/platform/sys/syscall.h`. The SYS_* numbers, the raw `syscallN`
macros, and the `syscall` instruction sequence live one layer down in the
architecture adapter (`src/common/arch/x8664/x8664.h`) and are called only
from within `src/common/arch/`. This is the full list of syscalls the SDK
issues, why each is needed, and where. (The Defined column's `syscall.h`
means this shared surface; every number lives in the arch adapter.)
...
| `close` | 3 | `syscall.h` | Close a file descriptor. | Cleanup: `wired_fio_read`/`wired_fio_append`/`wired_fio_write_new`/`wired_fio_close` close the file they opened; `wired_udp_close` closes the UDP socket; `client_close` / `wired_server_close` close the connection's socket fd when set. |
| `poll` | 7 | `syscall.h` | Wait for events on a small set of file descriptors with a timeout. | `poll_wait_readable` blocks on exactly one fd (the UDP socket) until it is readable or `timeout_ms` elapses, driving both the client (`connrunner.c`) and server (`srvrun.c`) wire loops. Only ever one fd is watched, so a `pollfd[1]` is enough — no epoll-style fd set is needed. |
| `mmap` | 9 | `syscall.h` | Map pages into the process's address space, backed by a file/fd or anonymous memory. | `xsksetup_mmap`/`xsksetup_umem` (`xsksetup.c`) map the UMEM (one anonymous region shared with the kernel) and each of the four AF_XDP rings (RX/TX/Fill/Completion) at the socket's per-ring `pgoff`, so `xsksetup_open` can hand the driver direct pointers into kernel-shared ring/frame memory. The thread runtime (`common/platform/thread/`) maps each worker thread's stack. |
...
| `munmap` | 11 | `syscall.h` | Unmap a previously mapped region. | `xsksetup_munmap`, called from `xsksetup_unwind`/`xsksetup_close` (`xsksetup.c`), releases the UMEM and ring mappings on teardown or when a later step of `xsksetup_open` fails partway through setup. The thread runtime unmaps a joined thread's stack. |
...
| `getsockopt` | 55 | `syscall.h` | Get a socket option. | `xsksetup_get_offsets` (`xsksetup.c`) reads `XDP_MMAP_OFFSETS` to learn each ring's producer/consumer/descriptor byte offsets before mmap'ing it; `xsksetup_stats` reads `XDP_STATISTICS` to report kernel-side drop/fill counters for the AF_XDP socket. |
...
| `fcntl` | 72 | `syscall.h` | Get/set file descriptor flags. | `poll_set_nonblock` sets `O_NONBLOCK` on the UDP socket so a send/recv call returns immediately instead of blocking, letting the `poll`-driven loop own all waiting. |
...
| `clock_gettime` | 228 | `syscall.h` | Read a system clock. | `common/platform/clock/clock.c`'s `clock_ymdhms` reads `CLOCK_REALTIME` for certificate validity-time checks and similar wall-clock needs; `clock/mono.c`'s `clock_mono_ms` reads `CLOCK_MONOTONIC` for interval measurement (connection idle age, RFC 9000 10.1); `debug.c`'s `wired_log_ts` reads it to timestamp trace lines. |
...
| `getrandom` | 318 | `syscall.h` | Fill a buffer with random bytes from the kernel CSPRNG. | `common/platform/rng/rng.c`'s `rng_bytes` is the SDK's single source of randomness — connection IDs, key material, nonces — looping until the requested length is filled. |
| `bpf` | 321 | `syscall.h` | Issue a `bpf(2)` command (create/update a map, load a program, create a link). | `xdpbpf_map_create` (`BPF_MAP_CREATE`) makes the XSKMAP that redirects packets to an AF_XDP socket; `xdpbpf_map_set` (`BPF_MAP_UPDATE_ELEM`) binds one queue's socket fd into that map; `xdpbpf_prog_load` (`BPF_PROG_LOAD`) loads the fixed XDP filter program built by `xdpbpf_prog_build`; `xdpbpf_link_create` (`BPF_LINK_CREATE`) attaches that program to the target interface. |
...
1. Add `#define SYS_<name> <number>` to `src/common/arch/x8664/x8664.h` —
   every number lives in the arch adapter, never next to a call site.
2. Add a typed `wired_arch_<name>` wrapper to `src/common/arch/sysops.h`
   (kernel argument order, struct pointers as `void*`, raw i64 -errno
   return) and call only that from the domain — never a raw `syscallN` or
   `__asm__("syscall")` outside `src/common/arch/`.
