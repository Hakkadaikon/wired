...
| `clone` | 56 | `syscall.h:37` | Create a new thread (or process) sharing the caller's address space per flags. | `common/platform/thread/thread.c`'s `wired_thread_start`: spawns one worker thread (`CLONE_VM\|FS\|FILES\|SIGHAND\|THREAD\|SYSVSEM\|CHILD_CLEARTID`) on a freshly mmap'd stack, via a hand-written asm trampoline (the child returns on the new stack, so a plain C wrapper cannot issue it). |
...
| `openat` | 257 | `syscall.h:44` | Open (or create) a file relative to a directory fd. | `common/platform/fio/fio.c`: `wired_fio_read` opens a file read-only (e.g. a cert/key PEM); `wired_fio_append` opens (creating if needed) a file for append (e.g. ticket/log persistence), both relative to `AT_FDCWD`. |
