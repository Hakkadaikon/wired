...
> and interop against real independent clients has proven 16 of 22 QUIC
...
| `resumption` | ✅ | PSK-based session resumption over a real second handshake, wired end to end into the ClientHello receive path and the server flight builder. Getting a real client (quic-go) to accept the resumed connection took three fixes past what the unit suite alone had proven: (1) `resumption_master_secret` must span the transcript through the *client's* Finished, not just the server's -- RFC 8446 §7.1's key-schedule diagram singles this secret out from the other three that stop at the server's own Finished, confirmed against Go's `crypto/tls` (`sendClientFinished` computes it right after writing the client Finished into the transcript); (2) the ServerHello must echo `pre_shared_key` (`selected_identity`) when a PSK is accepted (RFC 8446 §4.1.3/§4.2.11) -- omitting it left the client unable to tell which key-schedule branch the server took; (3) a PSK handshake must omit Certificate/CertificateVerify entirely (RFC 8446 §4.4) -- sending them anyway got the flight rejected as an unexpected message. Two consecutive real-interop runs pass; `handshake`/`transfer`/`retry`/`http3` all still pass (no regression) |
| `zerortt` | ✅ | 0-RTT data received and acted on, wired end to end. Beyond resumption's PSK/key-schedule plumbing, three more gaps had to close before this passed against quic-go: (1) 0-RTT datagrams that arrive before their packet-protection keys exist (which don't exist until the ClientHello finishes processing and the PSK is accepted) were being dropped outright at the boot-accumulator layer -- they're now buffered verbatim (RFC 9001 §4.6.1) and replayed once keys are derived; (2) 0-RTT shares 1-RTT's App packet-number space (RFC 9000 §12.3) but keeps a long-header shape, and the PN-recovery code assumed every packet at that level used the short-header offset, producing bogus ACKs the client rejected as a protocol violation; (3) a response to a 0-RTT-carried request could be armed before the server's own 1-RTT send keys exist (which need the Finished-inclusive transcript) and was failing to send every time -- now deferred to the first post-confirm step. Finally, the interop runner's zerortt case opens 40 concurrent request streams in one burst, which this server's then-4-slot concurrent-stream table silently dropped past the 4th; raised to 40 (same tradeoff already made for the WebTransport stream cap). Two consecutive real-interop runs pass; `handshake`/`transfer`/`retry`/`resumption`/`http3` all still pass |
...
| RFC 9369 — QUIC Version 2 | ✅ | ✅ | — | 🟡 | **Unit:** v2 packet types, v2 salts/labels, v1↔v2 switching, and version-parameterized Initial key derivation/packet building/opening reaching the real server accept path (not just isolated helpers)<br>**Vectors:** §3.3.3 v2 Retry key/nonce, v2 initial salts (§3.3.1), and the full §Appendix A Initial key (key/iv/hp) vector, both directions, pinned against `quic_initial_derive`'s real call path<br>**Interop:** `v2` testcase run, but unverdictable (quic-go client declares it unsupported) |
...
- `resumption` and `zerortt` now both pass. Beyond the PSK/key-schedule
  unit work, each needed real-interop-only fixes no loopback test could
  have caught -- see their own entries above for the specifics (a
  transcript-span mismatch against RFC 8446 §7.1's key schedule, a missing
  ServerHello `pre_shared_key` echo, sending Certificate/CertificateVerify
  where PSK requires omitting them, 0-RTT datagrams dropped before their
  keys existed, a wrong 0-RTT packet-number offset, sending a 0-RTT
  response before its 1-RTT keys existed, and a concurrent-stream cap too
  small for the runner's 40-file burst). `retry`, `ipv6`, and `v2` are all
  implemented, see their own entries above.
