...
- 述語(CCN 2): `static int moqtrun_rel_budget_ok(hub, wt, n) { if (!hub->io.send_budget) return 1; return hub->io.send_budget(wt) >= n + WIRED_MOQTREL_HEADROOM; }`。待ち(CCN 2): `moqtrun_rel_budget_wait` = ok なら 0、そうでなければ `moqtrel_note_sent(rb, i, 0, now_ms)`(既存 API: cursor +0、stall クロック再始動 — 新 helper 不要)+ `stat_rel_wait++` で 1。`moqtrun_rel_send_round`(現 CCN 3)は「span==0 の FIN 枝」「budget_wait なら return」「`moqtrun_rel_send_span`(fin_flag + 受理判定、CCN 2)」に分割。`moqtrel_stalled` の順序は変えない(credit を永遠に上げない相手は QUIC idle timeout → `on_session_close` で片付く)。再試行は `wired_moqt_tick` が毎 step 全 ring を drain するので保証済み。
- fake io に `send_budget` を追加(既定 0 = 無制限)し、テスト 3 本: budget 不足で round を送らず `stat_rel_wait` が増え、budget 回復後に**同じ span** が送られる / headroom 分は常に残る(budget = n + HEADROOM − 1 では送らない、= n + HEADROOM で送る) / `send_budget == 0` なら従来どおり(既存 105+ 本無変更で緑)。
...
## 検証層の判定
- 状態遷移: セッション credit(max_data/sent_data)× staging × drain の相互作用は有限だが、既存の ReliableRelay モデルに「purse(budget)」を 1 変数足す程度。**モデル検査は行わない**(修正 1 は「閉じない」だけ、修正 2 は単調な待ち条件の追加で、性質は既存の L1/L2 に包含される。設計判断: YAGNI)。
- Lean: 不要。
- TDD: §1 srvrun_test(反転 4 + 新規 1)、§2 moqtrun_test(3 本)、§5 e2e(RED→GREEN)。
...
§2 のとおり。`src/app/moqt/run/moqtrun.h`(io 末尾 `send_budget` + doc、`stat_rel_wait` + doc)、`src/app/moqt/run/moqtrel.h`(`WIRED_MOQTREL_HEADROOM` + doc)、`src/app/moqt/run/moqtrun.c`(`moqtrun_rel_budget_ok`/`_budget_wait`/`_send_span` 分割)、`tests/app/moqtrun_test.c`(fake io `send_budget` + テスト 3 本)。CCN ≤ 3、既存テスト無変更緑。
