...
Results as of 2026-07 for the QUIC table below (quic-go peer), plus
ngtcp2/picoquic peer results noted inline for `ecn`/`v2`/`connectionmigration`
where quic-go itself cannot render a verdict. The WebTransport table below
reflects the `transfer-unidirectional-send` fix landed 2026-08-23
(commit `ee13cc7`) and the 2026-08-23 local serial re-run against
webtransport-go/ngtcp2/flupke-webtransport.
...
| `transfer` | ✅ | file download over streams. Regressed to ✕ between 2026-08-05 (DPLPMTUD wiring) and 2026-08-09: full-size slices at the PMTU-raised chunk size overflowed a fixed plaintext buffer *after* their bytes were consumed from the send queue, black-holing them beyond loss detection's reach. Re-verified 5/5 consecutive PASS at the fix, plus 3/3 at the Cubic-default build ([Comparison](performance/comparison.md) has the full story) |
...
| `transfer-unidirectional-send` | ✅ | root cause was the server's round-robin pump sending a WT-signalled stream before the session's own 2xx reached the wire, which draft-ietf-webtrans-http3-15 §3.2 requires first; a real ngtcp2-webtransport client silently discarded the misordered stream. Fixed 2026-08-23 (`ee13cc7`) and re-verified passing against both ngtcp2 and, in the 2026-08-23 local serial re-run, webtransport-go |
| `transfer-bidirectional-send` | ✅ | same fix; passing against webtransport-go in the 2026-08-23 local serial re-run |
| `transfer-datagram-send` | ✅ | same fix; passing against webtransport-go in the 2026-08-23 local serial re-run |
...
- 4 of 22 QUIC interop testcases have not passed yet against quic-go (both
  measurements have); this table's QUIC peer is quic-go, and the
  WebTransport table's peer is webtransport-go, but `ecn`/`v2`/
  `connectionmigration` have also been run against ngtcp2 and (for `v2`)
  picoquic as noted inline above.
- `ecn` and `v2` cannot get an end-to-end verdict from quic-go specifically:
  the quic-go interop client itself refuses both testcases. Both do pass
  end to end against ngtcp2 (and `v2` against picoquic too), so the gap is
  this peer's test-case support, not an implementation gap here.
- `rebind-port` / `rebind-addr` implement the full RFC 9000 8.2/9.3
  ...
  for an unrelated reason (a client limitation). Against ngtcp2 as the
  client it does pass end to end.
- The three WebTransport `*-send` interop cases now pass against
  webtransport-go (2026-08-23 local serial re-run, 28/35 cells across all
  peers tracked; remaining gaps are chrome/firefox client-side, recorded with
  evidence). `transfer-unidirectional-send`'s root cause (2xx/stream
  ordering, `ee13cc7`) was found via a comparison run against ngtcp2, then
  confirmed to also explain the webtransport-go stall once re-run.
