...
void test_h3settings_control_open(void) {
  ...
  u64 type;
  usz consumed = 0;
  CHECK(h3_stream_type_parse(wired_span_of(buf, n), &type, &consumed) == 1);
  CHECK(consumed == 1 && h3_stream_type_is_control(type));

  /* no room */
  CHECK(h3settings_control_prefix(buf, 0, &n) == 0);
}
