...
/* @file
 * sched_setaffinity/sched_getaffinity are raw syscalls against the real
 * kernel, so the test verifies by issuing the same syscalls directly (no
 * reimplementation of srvpin.c).
 */
