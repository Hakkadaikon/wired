...
static void test_datagram_with_len(void) {
  ...
  CHECK(
      r == w && out.length == 4 && out.data[0] == 0xDE && out.data[3] == 0xEF);
}
...
static void test_datagram_truncated(void) {
  ...
  datagram_frame in        = {.length = 5, .data = payload};
  u8             buf[16];
  usz            w = datagram_encode(wired_mspan_of(buf, sizeof(buf)), &in, 1);
  datagram_frame out;
  CHECK(datagram_decode(buf, w - 1, &out) == 0); /* data cut short */
  /* empty datagram (length 0) is allowed */
  datagram_frame e  = {.length = 0, .data = payload};
  usz            ew = datagram_encode(wired_mspan_of(buf, sizeof(buf)), &e, 1);
  CHECK(datagram_decode(buf, ew, &out) == ew && out.length == 0);
}
...
static void test_datagram_allowed(void) {
  CHECK(datagram_allowed(0, 10) == 0);      /* unsupported */
  CHECK(datagram_allowed(1200, 1200) == 1); /* exactly the limit */
  CHECK(datagram_allowed(1200, 1201) == 0); /* over the limit */
}
