...
static void test_huffman_curl_headers(void) {
  ...
  CHECK(quic_qpack_huffman_decode(host, sizeof(host), out, sizeof(out), &olen));
...
static void test_huffman_bad_padding(void) {
  ...
  usz      olen = 0;
  CHECK(
      quic_qpack_huffman_decode(enc, sizeof(enc), out, sizeof(out), &olen) ==
      0);
}
...
static void test_huffman_litname_hname(void) {
  ...
  usz      c     = quic_qpack_literal_name_decode(
      line, sizeof(line), &never, nm, sizeof(nm), &nlen, val, sizeof(val),
      &vlen);
  CHECK(c == sizeof(line));
