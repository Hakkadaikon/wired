...
static void test_priupdate_roundtrip(void) {
  ...
  usz               n    = quic_h3_priupdate_put(
      buf, sizeof buf, &(quic_h3_priupdate){0, 4, wired_span_of(fv, 6)});
  CHECK(n > 0);
  CHECK(quic_h3_priupdate_get(wired_span_of(buf, n), &out) == n);
  CHECK(out.push == 0 && out.element_id == 4);
  CHECK(out.value.n == 6 && out.value.p[0] == 'u' && out.value.p[5] == 'i');
  n = quic_h3_priupdate_put(
      buf, sizeof buf, &(quic_h3_priupdate){1, 2, wired_span_of(fv, 6)});
  CHECK(n > 0);
  CHECK(quic_h3_priupdate_get(wired_span_of(buf, n), &out) == n);
...
static void test_priupdate_rejects_malformed(void) {
  ...
  quic_h3_priupdate out = {0};
  usz               n   = quic_h3_priupdate_put(
      buf, sizeof buf, &(quic_h3_priupdate){0, 4, wired_span_of(0, 0)});
  CHECK(n > 0);
  CHECK(quic_h3_priupdate_get(wired_span_of(buf, n - 1), &out) == 0);
  buf[0] = 0x01; /* not a PRIORITY_UPDATE type */
  CHECK(quic_h3_priupdate_get(wired_span_of(buf, n), &out) == 0);
}
...
static void test_priupdate_sfv_parse(void) {
  quic_h3_priority p;
  quic_h3_priority_sfv(wired_span_of((const u8*)"u=1, i", 6), &p);
  CHECK(p.urgency == 1 && p.incremental == 1);
  quic_h3_priority_sfv(wired_span_of((const u8*)"u=7", 3), &p);
  CHECK(p.urgency == 7 && p.incremental == 0);
  quic_h3_priority_sfv(wired_span_of((const u8*)"i=?1", 4), &p);
  CHECK(p.urgency == 3 && p.incremental == 1);
  quic_h3_priority_sfv(wired_span_of((const u8*)"i=?0, u=0", 9), &p);
  CHECK(p.urgency == 0 && p.incremental == 0);
  /* out-of-range urgency and junk keep the defaults */
  quic_h3_priority_sfv(wired_span_of((const u8*)"u=9", 3), &p);
  CHECK(p.urgency == 3);
  quic_h3_priority_sfv(wired_span_of((const u8*)"x=2,junk", 8), &p);
  CHECK(p.urgency == 3 && p.incremental == 0);
  quic_h3_priority_sfv(wired_span_of(0, 0), &p);
  CHECK(p.urgency == 3 && p.incremental == 0);
}
