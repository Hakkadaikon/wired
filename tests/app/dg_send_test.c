...
void test_dg_send_over_max(void) {
  ...
  dgdeliver_opts o  = {1, 4};
  int            ok = dgdeliver_frame(wired_span_of(data, 5), &o, &ob);
...
void test_dg_send_unsupported(void) {
  ...
  u8             out[16];
  wired_obuf     ob = obuf_of(out, sizeof(out));
  dgdeliver_opts o  = {1, 0};
  CHECK(dgdeliver_frame(wired_span_of(data, 1), &o, &ob) == 0);
}
...
void test_dg_send_no_room(void) {
  ...
  ob.len           = 7;
  dgdeliver_opts o = {1, 64};
  CHECK(dgdeliver_frame(wired_span_of(data, 4), &o, &ob) == 0);
  CHECK(ob.len == 7);
}
