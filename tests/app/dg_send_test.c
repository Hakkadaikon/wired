...
void test_dg_send_with_length(void) {
  ...
  int ok = quic_dgdeliver_frame(wired_span_of(data, 3), &o, &ob);
...
void test_dg_send_no_length(void) {
  ...
  int ok = quic_dgdeliver_frame(wired_span_of(data, 4), &o, &ob);
...
void test_dg_send_over_max(void) {
  ...
  int ok                = quic_dgdeliver_frame(wired_span_of(data, 5), &o, &ob);
...
void test_dg_send_unsupported(void) {
  ...
  CHECK(quic_dgdeliver_frame(wired_span_of(data, 1), &o, &ob) == 0);
}
...
void test_dg_send_no_room(void) {
  ...
  ob.len                = 7;
  quic_dgdeliver_opts o = {1, 64};
  CHECK(quic_dgdeliver_frame(wired_span_of(data, 4), &o, &ob) == 0);
  CHECK(ob.len == 7);
}
