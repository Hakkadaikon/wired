...
static void test_push_set_max(void) {
  h3_push_state s;
  h3_push_init(&s);
  CHECK(h3_push_set_max(&s, 3) == 1);
  CHECK(h3_push_allowed(&s, 0) == 1);
  CHECK(h3_push_allowed(&s, 2) == 1);
  CHECK(h3_push_allowed(&s, 3) == 0); /* id == max is not allowed */
}
...
static void test_push_monotonic(void) {
  h3_push_state s;
  h3_push_init(&s);
  CHECK(h3_push_set_max(&s, 5) == 1);
  CHECK(h3_push_set_max(&s, 5) == 1);  /* equal holds */
  CHECK(h3_push_set_max(&s, 10) == 1); /* raise */
  CHECK(h3_push_set_max(&s, 9) == 0);  /* lower rejected */
  CHECK(h3_push_allowed(&s, 9) == 1);  /* max unchanged at 10 */
}
 ...
 * same boundary h3_push_allowed enforces for an outgoing push. */
static void test_push_cancel_in_range_ok(void) {
  h3_push_state s;
  h3_push_init(&s);
  CHECK(h3_push_set_max(&s, 3) == 1);
  CHECK(h3_push_cancel_ok(&s, 0) == 1);
  CHECK(h3_push_cancel_ok(&s, 2) == 1);
  CHECK(h3_push_cancel_ok(&s, 3) == 0);   /* == max: never granted */
  CHECK(h3_push_cancel_ok(&s, 100) == 0); /* well past max */
}
