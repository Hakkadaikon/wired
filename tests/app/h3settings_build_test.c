...
void test_h3settings_build(void) {
  u8            buf[32];
  h3settings_in in = {0x4000, 0, 100, 0, 0, 0, 0, 0, 0, 0};
  ...
  CHECK(h3settings_build(&in, &ob) == 0);
}
...
void test_h3settings_build_connect_protocol(void) {
  u8            buf[32];
  h3settings_in in = {0x4000, 0, 100, 1, 0, 0, 0, 0, 0, 0};
  wired_obuf    ob = {buf, sizeof buf, 0};
  CHECK(h3settings_build(&in, &ob) == 1);
...
void test_h3settings_build_h3_datagram_and_wt_enabled(void) {
  u8            buf[48];
  h3settings_in in = {0x4000, 0, 100, 0, 1, 5, 0, 0, 0, 0};
  wired_obuf    ob = {buf, sizeof buf, 0};
  CHECK(h3settings_build(&in, &ob) == 1);
...
void test_h3settings_build_grease_nonzero_appends_pair(void) {
  u8            buf[32];
  u64           gid = h3_grease_value(9);
  h3settings_in in  = {0x4000, 0, 100, 0, 0, 0, gid, 0, 0, 0};
  wired_obuf    ob  = {buf, sizeof buf, 0};
  CHECK(h3settings_build(&in, &ob) == 1);
  ...
  CHECK(h3_is_reserved(s.pairs[3].id) == 1);
}
