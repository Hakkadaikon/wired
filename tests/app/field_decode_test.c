#include "app/qpack/qpackdyn/field_decode.h"

#include "app/qpack/qpack/dynfind.h"
#include "app/qpack/qpack/dyntable.h"
#include "app/qpack/qpack/relindex.h"
#include "app/qpack/qpackdyn/cstr.h"
#include "app/qpack/qpackdyn/field_encode.h"
#include "test.h"

#define eq_len(s) qdyn_cstr_len(s)

static int eq(wired_span a, const char* b, usz blen) {
  if (a.n != blen) return 0;
  for (usz i = 0; i < a.n; i++)
    if (a.p[i] != (u8)b[i]) return 0;
  return 1;
}

static qpack_field fd_field(const char* name, const char* value) {
  qpack_field f = {
      wired_span_of((const u8*)name, eq_len(name)),
      wired_span_of((const u8*)value, eq_len(value))};
  return f;
}

/* Insert (name, value), encode the dynamic reference relative to base, then
 * decode it back and assert the borrowed (name, value) round-trips. */
static void qd_roundtrip(qpack_dyn* t, const char* name, const char* value) {
  u8          fs[8];
  wired_obuf  ob       = obuf_of(fs, sizeof(fs));
  usz         consumed = 0;
  u64         base, rel;
  qpack_field f = fd_field(name, value);
  qpack_match m;
  qpack_field d;

  CHECK(qpack_dyn_insert(t, &f));
  base = t->dropped + t->count;
  CHECK(qpack_dyn_find(t, &f, &m));
  rel = base - m.abs_index - 1;
  CHECK(qdyn_indexed_dynamic(rel, &ob));
  qdyn_src src = {t, base, wired_span_of(fs, ob.len)};
  CHECK(qdyn_decode_field(&src, &d, &consumed));
  CHECK(consumed == ob.len);
  CHECK(eq(d.name, name, eq_len(name)));
  CHECK(eq(d.value, value, eq_len(value)));
}

static void test_dynamic_roundtrip(void) {
  qpack_dyn t;
  qpack_dyn_init(&t, 4096);
  qd_roundtrip(&t, "x-a", "1");
  qd_roundtrip(&t, "x-b", "two");
  /* The older entry still resolves after a second insert (base advanced). */
  {
    u8          fs[8];
    wired_obuf  ob       = obuf_of(fs, sizeof(fs));
    usz         consumed = 0;
    u64         base     = t.dropped + t.count;
    qpack_field d;
    /* abs 0 ("x-a") relative to base 2 is rel = 1. */
    CHECK(qdyn_indexed_dynamic(1, &ob));
    qdyn_src src = {&t, base, wired_span_of(fs, ob.len)};
    CHECK(qdyn_decode_field(&src, &d, &consumed));
    CHECK(eq(d.name, "x-a", 3) && eq(d.value, "1", 1));
  }
}

/* RFC 9204 4.5.2: a static reference (T=1) resolves from the static table.
 * Static index 17 is :method GET. */
static void test_static_reference(void) {
  qpack_dyn   t;
  u8          fs[2]    = {0xD1, 0x00}; /* 1Tiiiiii, T=1, index 17 */
  usz         consumed = 0;
  qpack_field d;
  qpack_dyn_init(&t, 256);
  qdyn_src src = {&t, 0, wired_span_of(fs, 1)};
  CHECK(qdyn_decode_field(&src, &d, &consumed));
  CHECK(consumed == 1);
  CHECK(eq(d.name, ":method", 7) && eq(d.value, "GET", 3));
}

/* A first byte without bit 7 is not an indexed field line. */
static void test_non_indexed_reject(void) {
  qpack_dyn   t;
  u8          fs[1]    = {0x20};
  usz         consumed = 99;
  qpack_field d;
  qpack_dyn_init(&t, 256);
  qdyn_src src = {&t, 0, wired_span_of(fs, 1)};
  CHECK(qdyn_decode_field(&src, &d, &consumed) == 0);
}

/* A dynamic index past the live window resolves to no entry. */
static void test_dynamic_miss(void) {
  qpack_dyn   t;
  u8          fs[2];
  wired_obuf  ob       = obuf_of(fs, sizeof(fs));
  usz         consumed = 0;
  qpack_field f        = fd_field("a", "b");
  qpack_field d;
  qpack_dyn_init(&t, 256);
  CHECK(qpack_dyn_insert(&t, &f));
  /* base 1, rel 5 -> abs underflow / absent entry. */
  CHECK(qdyn_indexed_dynamic(5, &ob));
  qdyn_src src = {&t, 1, wired_span_of(fs, ob.len)};
  CHECK(qdyn_decode_field(&src, &d, &consumed) == 0);
}

/* RFC 9204 4.5.3 / 3.2.6: a post-Base indexed field line resolves through
 * qdyn_decode_field alongside the plain-indexed forms. */
static void test_postbase_field_decode(void) {
  qpack_dyn   t;
  u8          fs[2]    = {0x10, 0x00}; /* 0001iiii, post-Base index 0 */
  usz         consumed = 0;
  qpack_field d;
  qpack_field f = fd_field("x-a", "1");
  qpack_dyn_init(&t, 256);
  CHECK(qpack_dyn_insert(&t, &f));
  /* Base=0 (as if the section prefix declared Base before this insert),
   * post-Base index 0 -> absolute index 0, the entry just inserted. */
  qdyn_src src = {&t, 0, wired_span_of(fs, 1)};
  CHECK(qdyn_decode_field(&src, &d, &consumed));
  CHECK(consumed == 1);
  CHECK(eq(d.name, "x-a", 3) && eq(d.value, "1", 1));
}

/* A post-Base index past the live window resolves to no entry. */
static void test_postbase_field_miss(void) {
  qpack_dyn   t;
  u8          fs[2]    = {0x11, 0x00}; /* post-Base index 1 */
  usz         consumed = 0;
  qpack_field d;
  qpack_dyn_init(&t, 256);
  qdyn_src src = {&t, 0, wired_span_of(fs, 1)};
  CHECK(qdyn_decode_field(&src, &d, &consumed) == 0);
}

/* RFC 9204 4.5.5 / 3.2.6: the name-only resolver used by a post-Base name
 * reference literal (the value comes from the literal, not the table). */
static void test_postbase_name_resolve(void) {
  qpack_dyn   t;
  qpack_field f = fd_field("x-b", "two");
  wired_span  name;
  qpack_dyn_init(&t, 256);
  CHECK(qpack_dyn_insert(&t, &f));
  {
    qdyn_src src = {&t, 0, wired_span_of(0, 0)};
    CHECK(qdyn_resolve_postbase_name(&src, 0, &name));
    CHECK(eq(name, "x-b", 3));
  }
}

/* A post-Base index past the live window resolves to no name. */
static void test_postbase_name_miss(void) {
  qpack_dyn  t;
  wired_span name;
  qpack_dyn_init(&t, 256);
  {
    qdyn_src src = {&t, 0, wired_span_of(0, 0)};
    CHECK(qdyn_resolve_postbase_name(&src, 0, &name) == 0);
  }
}

void test_qpackdyn_field_decode(void) {
  test_dynamic_roundtrip();
  test_static_reference();
  test_non_indexed_reject();
  test_dynamic_miss();
  test_postbase_field_decode();
  test_postbase_field_miss();
  test_postbase_name_resolve();
  test_postbase_name_miss();
}
