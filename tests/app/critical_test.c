...
static void test_critical_classify(void) {
  CHECK(h3_stream_is_critical(QUIC_H3_STREAM_CONTROL) == 1);
  CHECK(h3_stream_is_critical(QUIC_H3_STREAM_QPACK_ENCODER) == 1);
  CHECK(h3_stream_is_critical(QUIC_H3_STREAM_QPACK_DECODER) == 1);
  CHECK(h3_stream_is_critical(QUIC_H3_STREAM_PUSH) == 0);
...
static void test_critical_close_error(void) {
  CHECK(
      h3_critical_close_error(QUIC_H3_STREAM_CONTROL) ==
      QUIC_H3_CLOSED_CRITICAL_STREAM);
  CHECK(
      h3_critical_close_error(QUIC_H3_STREAM_QPACK_DECODER) ==
      QUIC_H3_CLOSED_CRITICAL_STREAM);
  CHECK(h3_critical_close_error(QUIC_H3_STREAM_PUSH) == 0);
}
