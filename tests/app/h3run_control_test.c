...
static void test_control_open_prefix(void) {
  u8 buf[1] = {0xFF};
  CHECK(h3run_control_open(buf, sizeof buf) == 1);
  CHECK(buf[0] == 0x00);
  CHECK(h3run_control_open(buf, 0) == 0); /* no room */
}

/* RFC 9114 6.2.1: a second control stream is H3_STREAM_CREATION_ERROR. */
static void test_control_single(void) {
  h3_control_state s = {0};
  CHECK(h3_control_seen(&s) == 1);
  CHECK(h3_control_seen(&s) == 0);
  CHECK(h3_control_seen(&s) == 0);
}
