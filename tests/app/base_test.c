...
static void test_qpack_base_valid_sign0_always_ok(void) {
  CHECK(qpack_base_valid(0, 0, 0));
  CHECK(qpack_base_valid(1, 0, 5));
}

/* RFC 9204 4.5.1.2: Sign 1 with RIC greater than DeltaBase is valid (Base
 * stays non-negative). */
static void test_qpack_base_valid_sign1_ok(void) {
  CHECK(qpack_base_valid(6, 1, 0));
  CHECK(qpack_base_valid(6, 1, 5)); /* Base == 0, still valid */
}
...
static void test_qpack_base_valid_sign1_rejected(void) {
  CHECK(!qpack_base_valid(5, 1, 5)); /* RIC == DeltaBase */
  CHECK(!qpack_base_valid(4, 1, 5)); /* RIC < DeltaBase */
  CHECK(!qpack_base_valid(0, 1, 0)); /* RIC == DeltaBase == 0 */
}
