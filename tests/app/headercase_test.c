...
static void test_headercase_lower(void) {
  ...
  CHECK(h3_header_name_ok((const u8*)":path", 5) == 1);
  CHECK(h3_header_name_ok((const u8*)"", 0) == 1);
}
...
static void test_headercase_upper(void) {
  ...
  CHECK(h3_header_name_ok((const u8*)"hostX", 5) == 0);
  CHECK(h3_header_name_ok((const u8*)"Authorization", 13) == 0);
}

/* Boundary bytes around A-Z stay allowed. */
static void test_headercase_boundary(void) {
  CHECK(h3_header_name_ok((const u8*)"@[", 2) == 1); /* 0x40, 0x5b */
  CHECK(h3_header_name_ok((const u8*)"a-z_0", 5) == 1);
}
...
static void test_headerbytes_ok(void) {
  ...
  CHECK(h3_header_bytes_ok((const u8*)"text/plain", 10) == 1);
  CHECK(h3_header_bytes_ok((const u8*)"", 0) == 1);
}
...
static void test_headerbytes_cr(void) {
  ...
  CHECK(h3_header_bytes_ok((const u8*)"\r", 1) == 0);
}
...
static void test_headerbytes_lf(void) {
  ...
  CHECK(h3_header_bytes_ok((const u8*)"\n", 1) == 0);
}
...
static void test_headerbytes_nul(void) {
  ...
  u8 nul = 0x00;
  CHECK(h3_header_bytes_ok(&nul, 1) == 0);
}
...
static void test_headername_connection_specific(void) {
  ...
  CHECK(h3_header_name_forbidden((const u8*)"keep-alive", 10) == 1);
  CHECK(h3_header_name_forbidden((const u8*)"proxy-connection", 16) == 1);
  CHECK(h3_header_name_forbidden((const u8*)"upgrade", 7) == 1);
}

/* Ordinary field names are not forbidden. */
static void test_headername_forbidden_ok(void) {
  CHECK(h3_header_name_forbidden((const u8*)"content-type", 12) == 0);
  CHECK(h3_header_name_forbidden((const u8*)":path", 5) == 0);
  CHECK(h3_header_name_forbidden((const u8*)"", 0) == 0);
  /* a name that merely starts with a forbidden one is not a match */
  CHECK(h3_header_name_forbidden((const u8*)"upgrade-insecure", 16) == 0);
}

/* RFC 9114 4.2: a TE field value other than "trailers" is malformed. */
static void test_header_te_ok(void) {
  CHECK(h3_header_te_ok((const u8*)"trailers", 8) == 1);
  CHECK(h3_header_te_ok((const u8*)"gzip", 4) == 0);
  CHECK(h3_header_te_ok((const u8*)"trailers, gzip", 14) == 0);
