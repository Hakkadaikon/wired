...
static void test_req_frames_headers_first(void) {
  ...
  usz        n  = quic_h3_frame_put(
      &ob, QUIC_H3_FRAME_HEADERS, wired_span_of(fs, sizeof fs));
  wired_span out = {0, 0};
  CHECK(quic_h3req_recv_first_headers(wired_span_of(buf, n), &out) == 1);
...
static void test_req_frames_data_first(void) {
  ...
  wired_span out = {0, 0};
  CHECK(quic_h3req_recv_first_headers(wired_span_of(buf, n), &out) == 0);
}
...
static void test_req_frames_truncated(void) {
  ...
  CHECK(quic_h3req_recv_first_headers(wired_span_of(buf, 1), &out) == 0);
}
