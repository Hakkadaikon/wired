...
static void test_req_frames_headers_first(void) {
  ...
  usz n = h3_frame_put(&ob, H3_FRAME_HEADERS, wired_span_of(fs, sizeof fs));
  wired_span out = {0, 0};
  CHECK(h3req_recv_first_headers(wired_span_of(buf, n), &out) == 1);
...
static void test_req_frames_data_first(void) {
  ...
  u8         buf[16];
  wired_obuf ob = {buf, sizeof buf, 0};
  usz n = h3_frame_put(&ob, H3_FRAME_DATA, wired_span_of(body, sizeof body));
  wired_span out = {0, 0};
  CHECK(h3req_recv_first_headers(wired_span_of(buf, n), &out) == 0);
}

static void test_req_frames_truncated(void) {
  u8         buf[1] = {H3_FRAME_HEADERS};
  wired_span out    = {0, 0};
  /* length varint missing -> no complete frame. */
  CHECK(h3req_recv_first_headers(wired_span_of(buf, 1), &out) == 0);
}
