...
void test_h3settings_control_settings(void) {
  ...
  CHECK(consumed == 1 && h3_stream_type_is_control(stype));
  ...
  CHECK(h3_settings_first(&st, f.type) == 1);

  /* no room */
  CHECK(h3settings_control_stream(0, buf, 1, &n) == 0);
}
...
void test_h3settings_control_settings_advertises_connect_protocol(void) {
  ...
  h3_frame f;
  h3_frame_get(wired_span_of(buf + consumed, n - consumed), &f);

  h3_settings s;
  usz         sr = h3_settings_get(buf + consumed, n - consumed, &s);
  CHECK(sr > 0);
...
static int hcs_has_pair(const h3_settings* s, u64 id, u64 value) {
  for (usz i = 0; i < s->n; i++)
...
void test_h3settings_h3_datagram_monotonic_ok(void) {
  CHECK(h3settings_h3_datagram_monotonic_ok(0, 0) == 1); /* equal */
  CHECK(h3settings_h3_datagram_monotonic_ok(1, 1) == 1); /* equal */
  CHECK(h3settings_h3_datagram_monotonic_ok(0, 1) == 1); /* raised */
  CHECK(h3settings_h3_datagram_monotonic_ok(1, 0) == 0); /* lowered */
}
...
void test_h3settings_zerortt_compatible_identical_ok(void) {
  h3settings_in s = {0x4000, 100, 4, 1, 1, 1, 0, 0, 0, 0};
  CHECK(h3settings_zerortt_compatible(&s, &s) == 1);
}
...
void test_h3settings_zerortt_compatible_raised_fields_ok(void) {
  h3settings_in prior    = {0x4000, 100, 4, 1, 1, 1, 0, 0, 0, 0};
  h3settings_in raised[] = {
  ...
  for (usz i = 0; i < sizeof raised / sizeof raised[0]; i++)
    CHECK(h3settings_zerortt_compatible(&prior, &raised[i]) == 1);
}
...
void test_h3settings_zerortt_compatible_extension_withdrawn_rejected(void) {
  h3settings_in prior   = {0x4000, 100, 4, 1, 1, 1, 0, 0, 0, 0};
  h3settings_in current = {0x4000, 100, 4, 1, 0, 1, 0, 0, 0, 0};
  CHECK(h3settings_zerortt_compatible(&prior, &current) == 0);
}
...
void test_h3settings_zerortt_compatible_lowered_core_limit_rejected(void) {
  ...
  for (usz i = 0; i < sizeof lowered / sizeof lowered[0]; i++)
    CHECK(h3settings_zerortt_compatible(&prior, &lowered[i]) == 0);
}
 ...
 * four already participate in h3settings_zerortt_compatible's
...
void test_h3settings_zerortt_compatible_wt_flow_control_lowered_rejected(void) {
  ...
  h3settings_in bidi_lowered               = prior;
  bidi_lowered.wt_initial_max_streams_bidi = 1;
  CHECK(h3settings_zerortt_compatible(&prior, &bidi_lowered) == 0);

  h3settings_in data_lowered       = prior;
  data_lowered.wt_initial_max_data = 1;
  CHECK(h3settings_zerortt_compatible(&prior, &data_lowered) == 0);
}
