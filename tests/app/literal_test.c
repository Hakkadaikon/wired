...
static void test_literal_namref_golden(void) {
  ...
  u8  out[8];
  usz olen;
  usz r = quic_qpack_literal_namref_decode(
      buf, w, &idx, &st, &nv, out, sizeof(out), &olen);
...
static void test_literal_namref_reject(void) {
  ...
  usz olen;
  CHECK(
      quic_qpack_literal_namref_decode(
          &bad, 1, &idx, &st, &nv, out, sizeof(out), &olen) == 0);
}
...
static void test_literal_name_golden(void) {
  ...
  u8  outn[8], outv[8];
  usz nlen, vlen;
  usz r = quic_qpack_literal_name_decode(
      buf, w, &nv, outn, sizeof(outn), &nlen, outv, sizeof(outv), &vlen);
...
static void test_literal_name_reject(void) {
  ...
  usz nlen, vlen;
  CHECK(
      quic_qpack_literal_name_decode(
          &huff, 1, &nv, outn, sizeof(outn), &nlen, outv, sizeof(outv),
          &vlen) == 0);
  CHECK(
      quic_qpack_literal_name_decode(
          &wrong, 1, &nv, outn, sizeof(outn), &nlen, outv, sizeof(outv),
          &vlen) == 0);
}
