...
static void test_goaway_monotonic_ok(void) {
  h3_goaway_state s = {0};
  CHECK(h3_goaway_ok(&s, 100) == 1); /* first */
  CHECK(h3_goaway_ok(&s, 100) == 1); /* equal */
  CHECK(h3_goaway_ok(&s, 40) == 1);  /* decrease */
  CHECK(h3_goaway_ok(&s, 0) == 1);   /* decrease to zero */
}

/* RFC 9114 5.2: an increasing GOAWAY id is H3_ID_ERROR. */
static void test_goaway_increase_rejected(void) {
  h3_goaway_state s = {0};
  CHECK(h3_goaway_ok(&s, 8) == 1);
  CHECK(h3_goaway_ok(&s, 12) == 0); /* increase: H3_ID_ERROR */
}
