...
static void test_qpack_ric_zero(void) {
  CHECK(qpack_ric_encode(0, 3) == 0);
  u64           ric = 99;
  qpack_ric_ctx c   = {3, 0};
  CHECK(qpack_ric_decode(0, &c, &ric) == 1 && ric == 0);
}
...
static void test_qpack_ric_example(void) {
  CHECK(qpack_ric_encode(9, 3) == 4);
  u64           ric = 0;
  qpack_ric_ctx c   = {3, 10};
  CHECK(qpack_ric_decode(4, &c, &ric) == 1 && ric == 9);
}
...
static void test_qpack_ric_invalid(void) {
  u64           ric = 0;
  qpack_ric_ctx c   = {3, 10};
  CHECK(qpack_ric_decode(7, &c, &ric) == 0);
  /* encoded 1 with no inserts wraps to candidate 0, which is invalid. */
  qpack_ric_ctx empty = {3, 0};
  CHECK(qpack_ric_decode(1, &empty, &ric) == 0);
}
...
static void test_qpack_ric_roundtrip(void) {
  ...
  for (u64 r = 1; r <= 2 * max_entries; r++) {
    u64           enc = qpack_ric_encode(r, max_entries);
    u64           ric = 0;
    qpack_ric_ctx c   = {max_entries, r};
    CHECK(qpack_ric_decode(enc, &c, &ric) == 1);
    CHECK(ric == r);
  }
}
...
static void test_qpack_ric_min_below_expected_rejected(void) {
  CHECK(!qpack_ric_min_ok(3, 1, 3)); /* expected 4, got 3 */
  CHECK(!qpack_ric_min_ok(0, 1, 0)); /* expected 1, got 0 */
}

/* RFC 9204 2.2.1: a RIC exactly equal to the expected minimum is accepted. */
static void test_qpack_ric_min_exact_accepted(void) {
  CHECK(qpack_ric_min_ok(4, 1, 3));
  CHECK(qpack_ric_min_ok(1, 1, 0));
}
...
static void test_qpack_incr_zero_rejected(void) {
  CHECK(!qpack_incr_valid(0, 0, 5));
  CHECK(!qpack_incr_valid(2, 0, 5));
}
...
static void test_qpack_incr_golden(void) { CHECK(qpack_incr_valid(2, 1, 3)); }
...
static void test_qpack_incr_overshoot_rejected(void) {
  CHECK(!qpack_incr_valid(2, 2, 3)); /* would reach 4, only 3 sent */
}
...
static void test_qpack_section_ack_pending_accepted(void) {
  CHECK(qpack_section_ack_valid(1));
  CHECK(qpack_section_ack_valid(5));
}
...
static void test_qpack_section_ack_none_pending_rejected(void) {
  CHECK(!qpack_section_ack_valid(0));
}
