...
static void test_moqsess_establish(void) {
  quic_moqsess s;
  quic_moqsess_init(&s);
  CHECK(quic_moqsess_should_buffer(&s) == 1);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_SENT_SETUP) ==
      QUIC_MOQSESS_CLOSE_NONE);
  CHECK(quic_moqsess_established(&s) == 0);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP) ==
      QUIC_MOQSESS_CLOSE_NONE);
  CHECK(quic_moqsess_established(&s) == 1);
  CHECK(quic_moqsess_should_buffer(&s) == 0);
}
...
static void test_moqsess_pipeline_before_peer_setup(void) {
  ...
  CHECK(quic_moqsess_may_reject_request(&s) == 0);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP) ==
      QUIC_MOQSESS_CLOSE_NONE);
  CHECK(quic_moqsess_established(&s) == 1);
 ...
 * data-stream buffering scenarios: quic_moqsess_should_buffer is the same
 * gate for either). */
static void test_moqsess_buffer_before_setup_then_deliver(void) {
  quic_moqsess s;
  quic_moqsess_init(&s);
  CHECK(quic_moqsess_should_buffer(&s) == 1); /* stream arrives here */
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(quic_moqsess_should_buffer(&s) == 1); /* still half-open */
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_SENT_SETUP);
  CHECK(quic_moqsess_should_buffer(&s) == 0); /* now deliverable */
}
...
static void test_moqsess_goaway_reject_new_request(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_SEND_GOAWAY) ==
      QUIC_MOQSESS_CLOSE_NONE);
  CHECK(quic_moqsess_may_reject_request(&s) == 1);
  CHECK(s.state == QUIC_MOQSESS_ESTABLISHED);
}
...
static void test_moqsess_goaway_clean_shutdown(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_GOAWAY) ==
      QUIC_MOQSESS_CLOSE_NONE);
  CHECK(quic_moqsess_suppress_own_requests(&s) == 1);
  CHECK(s.state == QUIC_MOQSESS_ESTABLISHED);
}
...
static void test_moqsess_goaway_timeout_closes(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_SEND_GOAWAY);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_GOAWAY_TIMEOUT) ==
...
static void test_moqsess_bad_first_message(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_BAD_FIRST) ==
...
static void test_moqsess_bad_request_id_parity(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_BAD_REQUEST_ID) ==
...
static void test_moqsess_malformed_control_message(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_MALFORMED_CTRL) ==
      QUIC_MOQSESS_CLOSE_PROTOCOL_VIOLATION);
}
...
static void test_moqsess_ctrl_stream_transport_close(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_CTRL_CLOSED) ==
      QUIC_MOQSESS_CLOSE_PROTOCOL_VIOLATION);
}
...
static void test_moqsess_unknown_uni_stream_type(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_UNKNOWN_UNI) ==
      QUIC_MOQSESS_CLOSE_PROTOCOL_VIOLATION);
}
...
static void test_moqsess_goaway_bad_uri(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_GOAWAY_BAD_URI) ==
      QUIC_MOQSESS_CLOSE_PROTOCOL_VIOLATION);
}
...
static void test_moqsess_second_control_stream(void) {
  ...
  quic_moqsess_step(&s, QUIC_MOQSESS_EV_RECV_SETUP);
  CHECK(
      quic_moqsess_step(&s, QUIC_MOQSESS_EV_SECOND_CTRL) ==
      QUIC_MOQSESS_CLOSE_PROTOCOL_VIOLATION);
}
...
static void test_moqsub_subscribe_establish(void) {
  quic_moqsub s;
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_SUBSCRIBER);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN) == 1);
  CHECK(s.state == QUIC_MOQSUB_PENDING);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK) == 1);
  CHECK(quic_moqsub_established(&s) == 1);
}
...
static void test_moqsub_object_delivery_after_subscribe(void) {
  quic_moqsub s;
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_SUBSCRIBER);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK);
  CHECK(quic_moqsub_may_forward(&s, 1) == 1);
}

/* PUBLISH -> PUBLISH_OK (REQUEST_OK) establishes the subscription. */
static void test_moqsub_publish_establish(void) {
  quic_moqsub s;
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_PUBLISHER);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK) == 1);
  CHECK(quic_moqsub_established(&s) == 1);
}
...
static void test_moqsub_object_before_publish_ok(void) {
  ...
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_PUBLISHER);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  CHECK(quic_moqsub_may_forward(&s, 1) == 1); /* Object arrives here */
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK) == 1);
  CHECK(quic_moqsub_established(&s) == 1);
}
...
static void test_moqsub_forward_state_zero_blocks_objects(void) {
  ...
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK);
  CHECK(quic_moqsub_may_forward(&s, 0) == 0);
  CHECK(quic_moqsub_established(&s) == 1); /* OK/PUBLISH_DONE still legal */
}
...
static void test_moqsub_update_keeps_established(void) {
  ...
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_UPDATE) == 1);
  CHECK(quic_moqsub_established(&s) == 1);
}
 ...
 * afterward (that gate is quic_moqsub_may_send_publish_done at the call
...
static void test_moqsub_publish_done_terminates_and_reclaims(void) {
  ...
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK);
  CHECK(quic_moqsub_may_send_publish_done(0) == 1); /* 0 open streams */
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_PUBLISH_DONE) == 1);
  CHECK(quic_moqsub_terminated(&s) == 1);
...
static void test_moqsub_requester_early_fin_not_failed(void) {
  ...
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_SUBSCRIBER);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_FIN_REQ) == 1);
  CHECK(quic_moqsub_terminated(&s) == 0);
  CHECK(s.state == QUIC_MOQSUB_PENDING);
}
...
static void test_moqsub_cancel_established(void) {
  ...
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_STOP_SENDING) == 1);
  CHECK(quic_moqsub_terminated(&s) == 1);
  CHECK(s.term_reason == QUIC_MOQSUB_TERM_STOP_SENDING);
  CHECK(quic_moqsub_may_forward(&s, 1) == 0);
}
...
static void test_moqsub_cancel_after_fin(void) {
  ...
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_SUBSCRIBER);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_FIN_REQ);
  CHECK(s.fin_req == 1);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_STOP_SENDING) == 1);
  CHECK(quic_moqsub_terminated(&s) == 1);
}
...
static void test_moqsub_subscribe_rejected(void) {
  ...
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_SUBSCRIBER);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_ERROR) == 1);
  CHECK(quic_moqsub_terminated(&s) == 1);
...
static void test_moqsub_duplicate_response_is_session_fault(void) {
  ...
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_OK) == 0);
...
static void test_moqsub_early_fin_before_response(void) {
  ...
  quic_moqsub_init(&s, QUIC_MOQSUB_ROLE_SUBSCRIBER);
  quic_moqsub_step(&s, QUIC_MOQSUB_EV_OPEN);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_FIN_RESP) == 1);
  CHECK(s.fin_resp == 1);
  CHECK(s.responded == 0);
  CHECK(quic_moqsub_step(&s, QUIC_MOQSUB_EV_STOP_SENDING) == 1);
  CHECK(quic_moqsub_terminated(&s) == 1);
