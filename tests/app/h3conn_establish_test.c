...
void test_h3conn_establish(void) {
  ...
  CHECK(h3conn_open_control(0, cs, sizeof(cs), &n));
  CHECK(n > 0);
  CHECK(h3conn_peer_settings_ok(cs, n));

  /* cap too small for the control stream */
  CHECK(!h3conn_open_control(0, cs, 0, &n));

  /* a non-control stream type (0x01 push) is rejected */
  {
    u8 bad[4] = {0x01, 0x04, 0x00, 0x00};
    CHECK(!h3conn_peer_settings_ok(bad, sizeof(bad)));
  }
  ...
  {
    u8 only[1] = {0x00};
    CHECK(!h3conn_peer_settings_ok(only, sizeof(only)));
  }
}
