...
static void test_priority_higher(void) {
  CHECK(h3_priority_higher(0, 7));
  CHECK(h3_priority_higher(2, 3));
  CHECK(!h3_priority_higher(3, 3));
  CHECK(!h3_priority_higher(7, 0));
}

/* Urgency boundaries: 0 and 7 valid, 8 (and above) invalid. */
static void test_urgency_valid_bounds(void) {
  CHECK(h3_urgency_valid(0));
  CHECK(h3_urgency_valid(3));
  CHECK(h3_urgency_valid(7));
  CHECK(!h3_urgency_valid(8));
  CHECK(!h3_urgency_valid(255));
}
