...
static void test_fieldline_indexed_golden(void) {
  u8  buf[4];
  usz w = quic_qpack_indexed_encode(buf, sizeof(buf), 17, 1);
  CHECK(w == 1 && buf[0] == 0xD1);

  u64 idx;
  int st;
  usz r = quic_qpack_indexed_decode(buf, w, &idx, &st);
...
static void test_fieldline_indexed_prefix_boundary(void) {
  u8  buf[4];
  usz w = quic_qpack_indexed_encode(buf, sizeof(buf), 63, 1);
...
static void test_fieldline_indexed_dynamic(void) {
  u8  buf[4];
  usz w = quic_qpack_indexed_encode(buf, sizeof(buf), 5, 0);
...
static void test_fieldline_indexed_reject(void) {
  ...
  u64 idx;
  int st;
  CHECK(quic_qpack_indexed_decode(&bad, 1, &idx, &st) == 0);
  CHECK(quic_qpack_indexed_decode(&bad, 0, &idx, &st) == 0);
}
