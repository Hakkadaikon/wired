...
static void test_fieldline_postbase_golden(void) {
  u8  buf[4];
  usz w =
      quic_qpack_indexed_postbase_encode(quic_mspan_of(buf, sizeof(buf)), 0);
  CHECK(w == 1 && buf[0] == 0x10);

  u64 pb;
  usz r = quic_qpack_indexed_postbase_decode(quic_span_of(buf, w), &pb);
  CHECK(r == w && pb == 0);
}
...
static void test_fieldline_postbase_index_one(void) {
  u8  buf[4];
  usz w =
      quic_qpack_indexed_postbase_encode(quic_mspan_of(buf, sizeof(buf)), 1);
...
static void test_fieldline_postbase_prefix_boundary(void) {
  u8  buf[4];
  usz w =
      quic_qpack_indexed_postbase_encode(quic_mspan_of(buf, sizeof(buf)), 15);
...
static void test_fieldline_postbase_reject(void) {
  ...
  u64 pb;
  CHECK(
      quic_qpack_indexed_postbase_decode(quic_span_of(&indexed, 1), &pb) == 0);
  CHECK(
      quic_qpack_indexed_postbase_decode(quic_span_of(&litname, 1), &pb) == 0);
  CHECK(
      quic_qpack_indexed_postbase_decode(quic_span_of(&indexed, 0), &pb) == 0);
}
