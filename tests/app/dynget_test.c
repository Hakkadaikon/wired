...
static qpack_field dg_field(const char* n, usz nl, const char* v, usz vl) {
  qpack_field f = {
...
static void test_get_evicted(void) {
  qpack_dyn   t;
  qpack_field a = dg_field("a", 1, "1", 1);
  qpack_field b = dg_field("b", 1, "2", 1);
  qpack_field c = dg_field("c", 1, "3", 1);
  qpack_dyn_init(&t, 70);
  qpack_dyn_insert(&t, &a);
  qpack_dyn_insert(&t, &b);
  qpack_dyn_insert(&t, &c); /* evicts abs 0 */
  qpack_field e;
  CHECK(qpack_dyn_get(&t, 0, &e) == 0);
  CHECK(qpack_dyn_get(&t, 2, &e) == 1);
  CHECK(qpack_dyn_get(&t, 3, &e) == 0); /* never inserted */
}
...
static void test_get_enc_rel_most_recent(void) {
  ...
  qpack_dyn_insert(&t, &b);
  qpack_field e;
  CHECK(qpack_dyn_get_enc_rel(&t, 0, &e) == 1);
  CHECK(e.name.n == 2 && e.name.p[0] == 'b' && e.name.p[1] == 'b');
  CHECK(qpack_dyn_get_enc_rel(&t, 1, &e) == 1);
