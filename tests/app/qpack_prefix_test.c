...
static void test_qpack_prefix_empty(void) {
  qpack_prefix in = {.required_insert_count = 0, .sign = 0, .delta_base = 0};
  u8           buf[8];
  usz          w = qpack_prefix_encode(buf, sizeof(buf), &in);
  CHECK(w == 2 && buf[0] == 0x00 && buf[1] == 0x00);

  qpack_prefix out;
  usz          r = qpack_prefix_decode(buf, w, &out);
...
static void test_qpack_prefix_roundtrip(void) {
  ...
  for (usz i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
    u8  buf[16];
    usz w = qpack_prefix_encode(buf, sizeof(buf), &cases[i]);
    CHECK(w != 0);
    qpack_prefix out;
    usz          r = qpack_prefix_decode(buf, w, &out);
    CHECK(r == w);
...
static void test_qpack_prefix_truncated(void) {
  ...
  usz          w = qpack_prefix_encode(buf, sizeof(buf), &in);
  qpack_prefix out;
  CHECK(qpack_prefix_decode(buf, w - 1, &out) == 0);
  CHECK(qpack_prefix_decode(buf, 0, &out) == 0);
}
