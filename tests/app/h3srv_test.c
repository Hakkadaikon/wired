...
static void test_h3srv_control_settings_first(void) {
  ...
  CHECK(quic_h3srv_open_control(&st, &ob));
  CHECK(st.settings_sent);
...
static void test_h3srv_request_answered(void) {
  ...
  CHECK(quic_h3srv_on_request(
      ...
      &r));
  {
    quic_h3srv_send_in send = {0, {200, quic_span_of(body, sizeof body)}};
    CHECK(quic_h3srv_build_response(&st, &send, &resp_ob));
  }
...
static void test_h3srv_head_no_body(void) {
  ...
  {
    quic_h3srv_send_in send = {0, {200, quic_span_of(body, sizeof body)}};
    quic_h3srv_resp_for_method_in in = {quic_span_of(head, sizeof head), send};
    CHECK(quic_h3srv_build_response_for_method(&st, &in, &resp_ob));
  }
  CHECK(quic_h3conn_recv_response(
      quic_span_of(resp, resp_ob.len), &resp_out));
...
static void test_h3srv_get_keeps_body(void) {
  ...
  {
    quic_h3srv_send_in send = {0, {200, quic_span_of(body, sizeof body)}};
    quic_h3srv_resp_for_method_in in = {quic_span_of(get, sizeof get), send};
    CHECK(quic_h3srv_build_response_for_method(&st, &in, &resp_ob));
  }
  CHECK(quic_h3conn_recv_response(
      quic_span_of(resp, resp_ob.len), &resp_out));
...
static void test_h3srv_no_response_without_request(void) {
  ...
  quic_h3srv_send_in send = {0, {200, quic_span_of(0, 0)}};
...
static void test_h3srv_respond_without_peer_settings(void) {
  ...
  CHECK(quic_h3conn_recv_response(
      quic_span_of(resp, resp_ob.len), &resp_out));
  CHECK(resp_out.status == 200); /* :status present without waiting on peer SETTINGS */
}
