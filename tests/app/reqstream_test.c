...
static void test_reqstream_ok_order(void) {
  h3_req_state s = H3_REQ_START;
  CHECK(h3_reqstream_frame(&s, H3_FRAME_HEADERS) == 1);
  CHECK(s == H3_REQ_HEADERS);
  CHECK(h3_reqstream_frame(&s, H3_FRAME_DATA) == 1);
  CHECK(h3_reqstream_frame(&s, H3_FRAME_DATA) == 1); /* DATA repeats */
  CHECK(s == H3_REQ_DATA);
  CHECK(h3_reqstream_frame(&s, H3_FRAME_HEADERS) == 1); /* trailer */
  CHECK(s == H3_REQ_TRAILERS);
}
...
static void test_reqstream_data_first(void) {
  h3_req_state s = H3_REQ_START;
  CHECK(h3_reqstream_frame(&s, H3_FRAME_DATA) == 0);
  CHECK(s == H3_REQ_START); /* state unchanged on violation */
}

/* A trailing HEADERS terminates the stream; nothing may follow. */
static void test_reqstream_after_trailers(void) {
  h3_req_state s = H3_REQ_START;
  h3_reqstream_frame(&s, H3_FRAME_HEADERS);
  h3_reqstream_frame(&s, H3_FRAME_HEADERS); /* trailer */
  CHECK(h3_reqstream_frame(&s, H3_FRAME_DATA) == 0);
  CHECK(h3_reqstream_frame(&s, H3_FRAME_HEADERS) == 0); /* 3rd HEADERS */
  CHECK(s == H3_REQ_TRAILERS);
}
...
static void test_reqstream_wrong_type(void) {
  h3_req_state s = H3_REQ_START;
  CHECK(h3_reqstream_frame(&s, H3_FRAME_SETTINGS) == 0);
  CHECK(s == H3_REQ_START);
}
