...
static void test_moqkvp_roundtrip(void) {
  ...
  u8       buf[16];
  const u8 aa   = 0xAA;
  moqkvp   even = {2, 0, 5, {0, 0}};
  moqkvp   odd  = {5, 1, 0, {&aa, 1}};
  usz      off  = 0;
  u64      prev = 0;

  CHECK(moqkvp_put(wired_mspan_of(buf, sizeof buf), &off, &prev, &even));
  CHECK(moqkvp_put(wired_mspan_of(buf, sizeof buf), &off, &prev, &odd));
...
static void test_moqkvp_put_rejects(void) {
  ...
  CHECK(!moqkvp_put(wired_mspan_of(buf, sizeof buf), &off, &prev, &back));
  CHECK(!moqkvp_put(wired_mspan_of(buf, sizeof buf), &off, &prev, &too_long));
  CHECK(!moqkvp_put(wired_mspan_of(buf, 1), &off, &prev, &ok));
  CHECK(off == 0 && prev == 2);
}
