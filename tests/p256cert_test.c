...
static const u8 pc_oid_ecdsa_sha256[] =
...
static void pc_be32(u8 b[32], const u8 *v, usz n)
{
    ...
    if (n > 32) { v += n - 32; n = 32; }   /* drop a leading 0x00 sign pad */
...
static int pc_split_spki(const u8 *spki, usz n, const u8 **oid, usz *oid_len,
...
static void test_cert_spki_key(void)
{
    ...
    const u8 *oid, *key;
    usz oid_len, key_len;
    CHECK(quic_x509_public_key(c.tbs, c.tbs_len, &oid, &oid_len,
                               &key, &key_len) == 1);
    CHECK(quic_x509_is_ec(oid, oid_len) == 1);
