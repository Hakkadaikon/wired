...
static void test_conn_handshake(void)
{
    ...
    CHECK(quic_conn_step(&c, QUIC_CONN_EV_HS_PROGRESS) &&
          c.phase == QUIC_PHASE_HANDSHAKE);
    CHECK(quic_conn_step(&c, QUIC_CONN_EV_HS_CONFIRMED) &&
...
static void test_conn_close(void)
{
    ...
    quic_conn_init(&c);
    quic_conn_step(&c, QUIC_CONN_EV_HS_PROGRESS);
    CHECK(quic_conn_step(&c, QUIC_CONN_EV_CLOSE) &&
          c.phase == QUIC_PHASE_CLOSING);
    CHECK(quic_conn_step(&c, QUIC_CONN_EV_CLOSED) &&
