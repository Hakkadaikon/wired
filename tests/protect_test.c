...
{
    u8 iv[12] = {0xfa,0x04,0x4b,0x2f,0x42,0xa3,0xfd,0x3b,0x46,0xfb,0x25,0x5c};
    u8 nonce[12];
    quic_protect_nonce(iv, 2, nonce);
    u8 want[12] = {0xfa,0x04,0x4b,0x2f,0x42,0xa3,0xfd,0x3b,0x46,0xfb,0x25,0x5e};
...
static void test_protect_roundtrip(void)
{
    ...
    u8 hdr[18] = {0xc3, 0,0,0,1, 8, 0x83,0x94,0xc8,0xf0,0x3e,0x51,0x57,0x08,
                  0, 0,0,0}; /* scid len 0, token len 0, then pn placeholder */
    /* place the 4-byte pn (=2) at the end of the header */
    hdr[14] = 0; hdr[15] = 0; hdr[16] = 0; hdr[17] = 2;
    const u8 payload[] = {0x06, 0x00, 0x05, 'h','e','l','l','o'};

    u8 pkt[64];
    usz total = quic_protect_seal(&keys, &hp, hdr, 18, 14, 4, 2,
