...
static void test_early_client(void) {
  CHECK(greasebit_client_early_ok(0) == 0);
  CHECK(greasebit_client_early_ok(1) == 1);
}
...
static void test_early_server(void) {
  CHECK(greasebit_server_ok(0, 0) == 0);
  CHECK(greasebit_server_ok(1, 0) == 0);
  CHECK(greasebit_server_ok(0, 1) == 0);
  CHECK(greasebit_server_ok(1, 1) == 1);
}
