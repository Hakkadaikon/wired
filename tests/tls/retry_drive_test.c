...
static usz make_retry(u8 *buf, usz cap, const u8 *orig_dcid, u8 odcil,
                      const u8 *scid, u8 scil, const u8 *token, usz tlen)
{
    static const u8 dummy_dcid[4] = {0xaa, 0xbb, 0xcc, 0xdd};
    u8 tag[QUIC_RETRY_TAG_LEN] = {0};
    usz n = quic_retry_build(buf, cap, QUIC_VERSION_1, dummy_dcid, 4,
                             scid, scil, token, tlen, tag);
    /* recompute the real tag over (orig_dcid || retry-without-tag) */
    quic_retry_tag(orig_dcid, odcil, buf, n - QUIC_RETRY_TAG_LEN,
...
static void test_retry_process_ok(void)
{
    ...
    usz tlen = 0;
    CHECK(quic_retry_process(pkt, n, odcid, 8, out_token, &tlen,
                             new_dcid, &new_dcil) == 1);
    CHECK(tlen == 6);
...
static void test_retry_process_bad_tag(void)
{
    ...
    u8 wrong[8]; for (usz i = 0; i < 8; i++) wrong[i] = odcid[i];
    wrong[0] ^= 0x80;

    u8 out_token[64], new_dcid[QUIC_MAX_CID_LEN], new_dcil = 0xff;
    usz tlen = 0xff;
    CHECK(quic_retry_process(pkt, n, wrong, 8, out_token, &tlen,
                             new_dcid, &new_dcil) == 0);
}
...
static void test_retry_process_short(void)
{
    ...
    usz tlen;
    CHECK(quic_retry_process(pkt, sizeof(pkt), 0, 0, out_token, &tlen,
                             new_dcid, &new_dcil) == 0);
}
