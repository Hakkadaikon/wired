...
static void test_legacy_version_ok(void) {
  ...
  CHECK(legacy_check_client_hello(buf, w) == 1);
}
...
static void test_legacy_version_wrong(void) {
  ...
  CHECK(legacy_check_client_hello(buf, w) == 0);
}
...
static void test_legacy_truncated(void) {
  ...
  u8  comp[1] = {0};
  usz w       = legacy_build_ch(buf, 0x0303, 0, comp, 1);
  CHECK(legacy_check_client_hello(buf, 3) == 0);
  CHECK(legacy_check_client_hello(buf, w - 1) == 0);
}
...
static void test_legacy_session_id_empty(void) {
  ...
  u8        sid_len = 99;
  usz       w       = legacy_build_ch(buf, 0x0303, 0, comp, 1);
  CHECK(legacy_session_id(wired_span_of(buf, w), &sid, &sid_len) == 1);
  CHECK(sid_len == 0);
}
...
static void test_legacy_session_id_overlong(void) {
  ...
  const u8* sid     = 0;
  u8        sid_len = 0;
  usz       off     = hs_begin(buf, 256, 1);
  buf[off]          = 0x03;
  buf[off + 1]      = 0x03;
  for (usz i = 0; i < 32; i++) buf[off + 2 + i] = 0;
  buf[off + 34] = 33; /* invalid: > 32 */
  hs_finish(buf, off + 35 + 33);
  CHECK(
      legacy_session_id(wired_span_of(buf, off + 35 + 33), &sid, &sid_len) ==
      0);
}
