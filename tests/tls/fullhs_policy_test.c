...
/* fullhs_recv_cert under the given policy, on a fresh client. Only the
...
static int fp_cert_result(
  ...
  fullhs_set_policy(&h, now, wired_span_of(host, host_len));
  return fullhs_recv_cert(&h, msg, msg_len);
}
...
static void test_fullhs_policy_gate(void) {
  ...
  fullhs_set_policy(&cl, 20370101000000ULL, wired_span_of(0, 0)); /* expired */
