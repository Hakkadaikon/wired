...
static void test_preshared_golden(void)
{
    ...
    u8 buf[64];
    usz w = 0;
    CHECK(quic_tlsext_pre_shared_key(id, 3, 0x01020304, bd, 2, buf,
                                     sizeof(buf), &w) == 1);
    ...
    CHECK(buf[11] == 0x01 && buf[12] == 0x02 && buf[13] == 0x03 && buf[14] == 0x04);
    /* binders_len = 1+2 = 3 */
...
static void test_preshared_roundtrip(void)
{
    ...
    quic_tlsext_pre_shared_key(id, 4, 0x12345678, bd, 3, buf, sizeof(buf), &w);
    CHECK(quic_tlsext_pre_shared_key_parse(buf, w, &off) == 1);
    CHECK(off.id_len == 4);
...
static void test_preshared_guards(void)
{
    ...
    usz w = 0;
    quic_tlsext_psk_offer off;
    quic_tlsext_pre_shared_key(id, 2, 1, bd, 2, buf, sizeof(buf), &w);
    /* truncated */
    CHECK(quic_tlsext_pre_shared_key_parse(buf, w - 1, &off) == 0);
    /* wrong type */
    buf[1] = 0x2a;
    CHECK(quic_tlsext_pre_shared_key_parse(buf, w, &off) == 0);
}
...
static void test_preshared_encode_guard(void)
{
    ...
    CHECK(quic_tlsext_pre_shared_key(id, 2, 1, bd, 2, buf, sizeof(buf), &w) == 0);
}
