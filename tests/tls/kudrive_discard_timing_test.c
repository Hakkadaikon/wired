...
void test_kudrive_discard_timing(void) {
  ...
  CHECK(kudrive_can_discard_old(1149, completed, pto) == 0);
  /* boundary: now = completed + 3*PTO -> discardable */
  CHECK(kudrive_can_discard_old(1150, completed, pto) == 1);
  CHECK(kudrive_can_discard_old(2000, completed, pto) == 1);

  /* re-initiate: within 3*PTO of last update is rejected (DoS floor) */
  u64 last = 500; /* last+3PTO = 650 */
  CHECK(kudrive_can_initiate_again(649, last, pto) == 0);
  CHECK(kudrive_can_initiate_again(650, last, pto) == 1);
}
