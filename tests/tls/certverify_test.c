...
static void test_certverify_bad_scheme(void) {
  ...
  CHECK(
      quic_tls_verify_cert_signature(
          0x0000, quic_x509_golden, sizeof(quic_x509_golden), sig, sizeof(sig),
          th) == 0);
  CHECK(
      quic_tls_verify_cert_signature(
          0xffff, quic_x509_golden, sizeof(quic_x509_golden), sig, sizeof(sig),
          th) == 0);
}
...
static void test_certverify_ecdsa_bogus(void) {
  ...
  CHECK(
      quic_tls_verify_cert_signature(
          QUIC_TLS_SCHEME_ECDSA_P256, quic_x509_golden,
