...
static void test_grease_param(void) {
  u8  buf[8];
  usz w = grease_encode(buf, sizeof(buf));
  CHECK(w != 0);
  CHECK(grease_decode(buf, w) == w);

  /* a non-empty value is rejected */
  buf[w - 1] = 1; /* set the length byte to 1 */
  CHECK(grease_decode(buf, w + 1) == 0);
}
...
static void test_grease_accept(void) {
  ...
  CHECK(grease_accept_byte0(0xC0, 0) == 1); /* bit set */
  CHECK(grease_accept_byte0(0x80, 0) == 0); /* bit cleared, refused */
  /* peer greases: either value is accepted */
  CHECK(grease_accept_byte0(0xC0, 1) == 1);
  CHECK(grease_accept_byte0(0x80, 1) == 1); /* cleared but accepted */
}
