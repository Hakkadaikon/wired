...
void test_kudrive_trigger(void) {
  ...
  CHECK(kudrive_should_initiate(100, 10, 0) == 0);
  CHECK(kudrive_should_initiate(0, 0, 0) == 0);

  /* confirmed but below threshold */
  CHECK(kudrive_should_initiate(9, 10, 1) == 0);

  /* confirmed, boundary: sent == threshold initiates */
  CHECK(kudrive_should_initiate(10, 10, 1) == 1);

  /* confirmed, above threshold */
  CHECK(kudrive_should_initiate(11, 10, 1) == 1);
}
