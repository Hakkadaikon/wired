...
static void test_blob_token(void)
{
    ...
    const u8 *v;
    usz vlen;
    usz w = quic_tparam_put_blob(buf, sizeof(buf),
                                 QUIC_TP_STATELESS_RESET_TOKEN, tok, 16);
...
static void test_blob_variable_cid(void)
{
    ...
    usz vlen;
    usz w = quic_tparam_put_blob(buf, sizeof(buf),
                                 QUIC_TP_INITIAL_SOURCE_CONNECTION_ID, cid, 8);
    usz r = quic_tparam_get_blob(buf, w, &id, &v, &vlen);
...
static void test_blob_empty(void)
{
    ...
    usz vlen;
    usz w = quic_tparam_put_blob(buf, sizeof(buf),
                                 QUIC_TP_DISABLE_ACTIVE_MIGRATION, buf, 0);
    usz r = quic_tparam_get_blob(buf, w, &id, &v, &vlen);
    CHECK(w != 0 && r == w && id == QUIC_TP_DISABLE_ACTIVE_MIGRATION && vlen == 0);
}
...
static int pa_eq(const struct quic_preferred_address *a,
