...
static void test_v2ku_appendix_key_iv_hp(void) {
  ...
  quic_hkdf_expand_label(A5_SECRET, &lk, quic_mspan_of(key, 32));
  quic_hkdf_expand_label(A5_SECRET, &li, quic_mspan_of(iv, 12));
  quic_hkdf_expand_label(A5_SECRET, &lh, quic_mspan_of(hp, 32));
...
static void test_v2ku_appendix_seal(void) {
  ...
  u8       ct[17];
  quic_chapoly_ctx c = {A5_KEY, nonce, quic_span_of(aad, 4)};
  quic_chapoly_seal(&c, quic_span_of(pt, 1), ct);
