...
void test_hsdone(void) {
  CHECK(hs_complete(0, 0) == 0);
  CHECK(hs_complete(1, 0) == 0);
  CHECK(hs_complete(0, 1) == 0);
  CHECK(hs_complete(1, 1) == 1);

  /* server: confirmed iff HANDSHAKE_DONE sent, received irrelevant */
  CHECK(hs_confirmed(1, 0, 0) == 0);
  CHECK(hs_confirmed(1, 0, 1) == 0);
  CHECK(hs_confirmed(1, 1, 0) != 0);

  /* client: confirmed iff HANDSHAKE_DONE received, sent irrelevant */
  CHECK(hs_confirmed(0, 0, 0) == 0);
  CHECK(hs_confirmed(0, 1, 0) == 0);
  CHECK(hs_confirmed(0, 0, 1) != 0);
}
