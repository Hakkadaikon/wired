...
static void test_pskmodes_golden(void) {
  ...
  CHECK(tlsext_psk_modes(buf, sizeof(buf), &w) == 1);
  CHECK(w == 6);
...
static void test_pskmodes_roundtrip(void) {
  u8  buf[16];
  usz w = 0;
  tlsext_psk_modes(buf, sizeof(buf), &w);
  CHECK(tlsext_psk_modes_parse(buf, w) == 1);
}
...
static void test_pskmodes_absent(void) {
  ...
  CHECK(tlsext_psk_modes_parse(buf, sizeof(buf)) == 0);
}
...
static void test_pskmodes_guards(void) {
  ...
  tlsext_psk_modes(buf, sizeof(buf), &w);
  /* truncated body */
  CHECK(tlsext_psk_modes_parse(buf, w - 1) == 0);
  /* wrong extension_type */
  buf[1] = 0x2e;
  CHECK(tlsext_psk_modes_parse(buf, w) == 0);
  /* list length disagrees with ext_data length */
  u8 bad[6] = {0x00, 0x2d, 0x00, 0x02, 0x02, 0x01};
  CHECK(tlsext_psk_modes_parse(bad, sizeof(bad)) == 0);
}

static void test_pskmodes_encode_guard(void) {
  u8  buf[5];
  usz w = 0;
  CHECK(tlsext_psk_modes(buf, sizeof(buf), &w) == 0);
}
