...
static void test_earlydrive_can_send(void) {
  CHECK(earlydata_can_send(1, 1) == 1);
  CHECK(earlydata_can_send(1, 0) == 0); /* no resumption */
  CHECK(earlydata_can_send(0, 1) == 0); /* no 0-RTT keys */
  CHECK(earlydata_can_send(0, 0) == 0);
}
...
static void test_earlydrive_must_resend(void) {
  CHECK(earlydata_must_resend(0) == 1); /* server rejected */
  CHECK(earlydata_must_resend(1) == 0); /* server accepted */
}
...
static void test_earlydrive_can_migrate(void) {
  CHECK(earlydata_can_migrate(1, 1) == 1);
  CHECK(earlydata_can_migrate(0, 1) == 0); /* path not validated */
  CHECK(earlydata_can_migrate(1, 0) == 0); /* handshake not confirmed */
  CHECK(earlydata_can_migrate(0, 0) == 0);
}
