...
static void test_retry_tag_roundtrip(void) {
  ...
  u8 tag[RETRY_TAG];
  retry_tag(wired_span_of(odcid, 8), wired_span_of(retry, sizeof(retry)), tag);

  /* assemble the full Retry packet (retry || tag) and verify */
  u8 full[32 + RETRY_TAG];
  for (usz i = 0; i < sizeof(retry); i++) full[i] = retry[i];
  for (usz i = 0; i < RETRY_TAG; i++) full[sizeof(retry) + i] = tag[i];
  CHECK(
      retry_verify(
...
static void test_retry_tag_deterministic(void) {
  ...
  u8       a[RETRY_TAG], b[RETRY_TAG];
  retry_tag(wired_span_of(odcid, 4), wired_span_of(retry, 10), a);
  retry_tag(wired_span_of(odcid, 4), wired_span_of(retry, 10), b);
  for (usz i = 0; i < RETRY_TAG; i++) CHECK(a[i] == b[i]);
}
...
static void test_retry_tag_boundaries(void) {
  ...
  u8       full[12 + RETRY_TAG], tag[RETRY_TAG];

  /* empty ODCID */
  retry_tag(wired_span_of(0, 0), wired_span_of(retry, 12), tag);
  for (usz i = 0; i < 12; i++) full[i] = retry[i];
  for (usz i = 0; i < RETRY_TAG; i++) full[12 + i] = tag[i];
  CHECK(
      retry_verify(wired_span_of(0, 0), wired_span_of(full, sizeof(full))) ==
      1);
  ...
  retry_tag(wired_span_of(odcid, 20), wired_span_of(retry, 12), tag);
  for (usz i = 0; i < RETRY_TAG; i++) full[12 + i] = tag[i];
  CHECK(
      retry_verify(
