...
static void test_retry_tag_roundtrip(void) {
  ...
  quic_retry_tag(odcid, 8, retry, sizeof(retry), tag);
  ...
  CHECK(quic_retry_verify(odcid, 8, full, sizeof(full)) == 1);

  /* flip one Retry byte: verification must fail */
  full[3] ^= 0x01;
  CHECK(quic_retry_verify(odcid, 8, full, sizeof(full)) == 0);
  full[3] ^= 0x01;
  /* flip an ODCID byte: verification must fail */
  u8 bad_odcid[8];
  for (usz i = 0; i < 8; i++) bad_odcid[i] = odcid[i];
  bad_odcid[0] ^= 0x80;
  CHECK(quic_retry_verify(bad_odcid, 8, full, sizeof(full)) == 0);
}
...
static void test_retry_tag_deterministic(void) {
  ...
  quic_retry_tag(odcid, 4, retry, 10, a);
  quic_retry_tag(odcid, 4, retry, 10, b);
