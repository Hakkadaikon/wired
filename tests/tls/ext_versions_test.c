...
static void test_ext_versions_golden(void) {
  u8  buf[16];
  usz w = tls_ext_supported_versions(buf, sizeof(buf));
...
static void test_ext_versions_roundtrip(void) {
  u8  buf[16];
  usz w = tls_ext_supported_versions(buf, sizeof(buf));
  CHECK(tls_ext_versions_has_tls13(buf, w) == 1);
}
...
static void test_ext_versions_absent(void) {
  ...
  CHECK(tls_ext_versions_has_tls13(buf, sizeof(buf)) == 0);
}
...
static void test_ext_versions_decode_guards(void) {
  ...
  usz w = tls_ext_supported_versions(buf, sizeof(buf));
  /* truncated body */
  CHECK(tls_ext_versions_has_tls13(buf, w - 1) == 0);
  /* wrong extension_type */
  buf[1] = 0x2c;
  CHECK(tls_ext_versions_has_tls13(buf, w) == 0);
  /* odd list length */
  u8 odd[6] = {0x00, 0x2b, 0x00, 0x02, 0x01, 0x03};
  CHECK(tls_ext_versions_has_tls13(odd, sizeof(odd)) == 0);
}

static void test_ext_versions_encode_guard(void) {
  u8 buf[6];
  CHECK(tls_ext_supported_versions(buf, sizeof(buf)) == 0);
}
