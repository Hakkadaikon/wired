...
static void test_hs_clienthello_roundtrip(void)
{
    ...
    usz total = quic_hs_build_hello(out, sizeof(out), QUIC_HS_CLIENT_HELLO,
                                    random, pub);
    CHECK(total != 0 && out[0] == QUIC_HS_CLIENT_HELLO);

    u8 type; usz body_len;
    usz body = quic_hs_parse(out, total, &type, &body_len);
...
static void test_hs_serverhello_roundtrip(void)
{
    ...
    for (usz i = 0; i < 32; i++) { random[i] = (u8)(i * 2); pub[i] = (u8)(0x90 + i); }
    usz total = quic_hs_build_hello(out, sizeof(out), QUIC_HS_SERVER_HELLO,
                                    random, pub);
    u8 type; usz body_len;
    usz body = quic_hs_parse(out, total, &type, &body_len);
...
static void test_hs_truncated(void)
{
    ...
    CHECK(quic_hs_parse(bad, 3, &type, &body_len) == 0);
    /* claims body 100 but only 4 bytes present */
    const u8 bad2[4] = {1, 0, 0, 100};
    CHECK(quic_hs_parse(bad2, 4, &type, &body_len) == 0);
}
