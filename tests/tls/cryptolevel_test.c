...
static void test_cryptolevel_stale_extends_true(void) {
  CHECK(quic_cryptolevel_stale_extends(10, 5, 10)); /* 5+10=15 > 10 */
  CHECK(quic_cryptolevel_stale_extends(0, 0, 1));   /* first byte, max 0 */
}

/* Data fully within the already-seen range is not a violation. */
static void test_cryptolevel_stale_extends_false(void) {
  CHECK(!quic_cryptolevel_stale_extends(10, 0, 10)); /* exactly reaches 10 */
  CHECK(!quic_cryptolevel_stale_extends(10, 2, 3));  /* [2,5) within 10 */
  CHECK(!quic_cryptolevel_stale_extends(0, 0, 0));   /* empty, nothing new */
}
...
static void test_cryptolevel_unconsumed_on_promote_true(void) {
  CHECK(quic_cryptolevel_unconsumed_on_promote(10, 5));
  CHECK(quic_cryptolevel_unconsumed_on_promote(1, 0));
}

/* Fully drained (or empty) levels promote cleanly. */
static void test_cryptolevel_unconsumed_on_promote_false(void) {
  CHECK(!quic_cryptolevel_unconsumed_on_promote(10, 10));
  CHECK(!quic_cryptolevel_unconsumed_on_promote(0, 0));
}
