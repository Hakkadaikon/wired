...
static void test_ticketguard_single_use(void) {
  ...
  ticketguard_init(&g);
  CHECK(ticketguard_first_use(&g, wired_span_of(a, 32)) == 1);
  CHECK(ticketguard_first_use(&g, wired_span_of(a, 32)) == 0);
  CHECK(ticketguard_first_use(&g, wired_span_of(b, 32)) == 1);
  CHECK(ticketguard_first_use(&g, wired_span_of(tiny, 8)) == 0);
}
...
static void test_ticketguard_ring_window(void) {
  ...
  for (int i = 0; i < QUIC_TICKETGUARD_CAP; i++) {
    ...
    fresh[0]     = (u8)i;
    fresh[1]     = 0x77;
    CHECK(ticketguard_first_use(&g, wired_span_of(fresh, 32)) == 1);
  }
