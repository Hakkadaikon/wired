...
static void test_ticketguard_single_use(void) {
  ...
  CHECK(quic_ticketguard_first_use(&g, wired_span_of(a, 32)) == 1);
  CHECK(quic_ticketguard_first_use(&g, wired_span_of(a, 32)) == 0);
  CHECK(quic_ticketguard_first_use(&g, wired_span_of(b, 32)) == 1);
  CHECK(quic_ticketguard_first_use(&g, wired_span_of(tiny, 8)) == 0);
}
...
static void test_ticketguard_ring_window(void) {
  ...
  quic_ticketguard_init(&g);
  t[0] = 0xEE;
  CHECK(quic_ticketguard_first_use(&g, wired_span_of(t, 32)) == 1);
  for (int i = 0; i < QUIC_TICKETGUARD_CAP; i++) {
    u8 fresh[32] = {0};
    fresh[0]     = (u8)i;
    fresh[1]     = 0x77;
    CHECK(quic_ticketguard_first_use(&g, wired_span_of(fresh, 32)) == 1);
  }
