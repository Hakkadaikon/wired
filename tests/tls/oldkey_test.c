...
void test_oldkey(void) {
  ...
  CHECK(oldkey_retain(t, 1100, pto));
  CHECK(!oldkey_can_discard(t, 1100, pto));

  /* just before the boundary: still retained */
  CHECK(oldkey_retain(t, 1299, pto));

  /* exactly at 3*PTO: window closed, may discard */
  CHECK(!oldkey_retain(t, 1300, pto));
  CHECK(oldkey_can_discard(t, 1300, pto));

  /* past the boundary: discardable */
  CHECK(oldkey_can_discard(t, 1301, pto));

  /* at the update instant: fully retained */
  CHECK(oldkey_retain(t, t, pto));
}
