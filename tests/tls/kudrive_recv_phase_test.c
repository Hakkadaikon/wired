...
void test_kudrive_recv_phase(void) {
  ...
  CHECK(kudrive_key_generation(0, 0, 0) == 0);
  CHECK(kudrive_key_generation(1, 1, 0) == 0);

  /* mismatch -> generation 1 (next), regardless of in-progress flag */
  CHECK(kudrive_key_generation(1, 0, 0) == 1);
  CHECK(kudrive_key_generation(0, 1, 0) == 1);
  CHECK(kudrive_key_generation(1, 0, 1) == 1);
}
