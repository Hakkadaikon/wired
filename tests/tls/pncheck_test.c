...
static void test_pncheck_violation(void) {
  CHECK(keyupdate_pn_violates(10, 11)); /* old-key PN past new phase */
  CHECK(keyupdate_pn_violates(0, 1));
}
...
static void test_pncheck_no_violation(void) {
  CHECK(!keyupdate_pn_violates(10, 10)); /* equal: not "higher" */
  CHECK(!keyupdate_pn_violates(10, 5));  /* reordered, still old-key */
  CHECK(!keyupdate_pn_violates(0, 0));
}
