...
void test_initiate(void) {
  ...
  CHECK(!keyupdate_may_initiate(&(keyupdate_in){0, last, 5000, pto}));

  /* confirmed but 3*PTO not yet elapsed */
  CHECK(!keyupdate_may_initiate(&(keyupdate_in){1, last, 1299, pto}));

  /* confirmed and exactly 3*PTO elapsed: allowed */
  CHECK(keyupdate_may_initiate(&(keyupdate_in){1, last, 1300, pto}));

  /* confirmed and well past: allowed */
  CHECK(keyupdate_may_initiate(&(keyupdate_in){1, last, 2000, pto}));
}
