...
void test_initiate(void) {
  ...
  CHECK(!quic_keyupdate_may_initiate(0, last, 5000, pto));

  /* confirmed but 3*PTO not yet elapsed */
  CHECK(!quic_keyupdate_may_initiate(1, last, 1299, pto));

  /* confirmed and exactly 3*PTO elapsed: allowed */
  CHECK(quic_keyupdate_may_initiate(1, last, 1300, pto));

  /* confirmed and well past: allowed */
  CHECK(quic_keyupdate_may_initiate(1, last, 2000, pto));
}
