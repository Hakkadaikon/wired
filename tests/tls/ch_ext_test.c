...
static usz build_ch(u8 *buf, usz cap)
{
    ...
    return quic_tls_client_hello(buf, cap, random, pub,
...
void test_ch_ext_truncated_returns_zero(void)
{
    ...
    usz dlen;
    usz w = build_ch(buf, sizeof(buf));
    CHECK(quic_salpn_find_extension(buf, 3, QUIC_SALPN_EXT_TYPE, &data, &dlen) == 0);
    CHECK(quic_salpn_find_extension(buf, w - 1, QUIC_SALPN_EXT_TYPE, &data, &dlen) == 0);
}
