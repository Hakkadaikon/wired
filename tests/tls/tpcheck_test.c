...
void test_tpcheck(void) {
  ...
  CHECK(
      quic_tparam_cid_match(z, 0, z, 0) ==
      1); /* both empty (zero-length CID) */
  ...
  CHECK(
      quic_tparam_check_retry_scid(1, 1, a, 4, b, 4) == 1); /* retry, matches */
  CHECK(
      quic_tparam_check_retry_scid(1, 1, a, 4, c, 4) ==
      0); /* retry, mismatch */
  CHECK(
      quic_tparam_check_retry_scid(1, 0, 0, 0, b, 4) ==
      0); /* retry but missing */
  CHECK(
      quic_tparam_check_retry_scid(0, 1, a, 4, 0, 0) ==
      0); /* present but no retry */
  CHECK(
      quic_tparam_check_retry_scid(0, 0, 0, 0, 0, 0) ==
      1); /* no retry, absent */
}
