...
static void test_keyupdate_phase_tracks(void) {
  keyupdate k;
  keyupdate_init(&k);
  CHECK(keyupdate_phase(&k) == 0);
  keyupdate_initiate(&k);
  CHECK(k.gen == 1 && keyupdate_phase(&k) == 1);
  keyupdate_acked(&k);
  keyupdate_initiate(&k);
  CHECK(k.gen == 2 && keyupdate_phase(&k) == 0);
}
...
static void test_keyupdate_blocked_until_acked(void) {
  keyupdate k;
  keyupdate_init(&k);
  CHECK(keyupdate_initiate(&k) == 1 && k.gen == 1);
  /* second initiate while unacknowledged is refused */
  CHECK(keyupdate_initiate(&k) == 0 && k.gen == 1);
  /* following the peer is also refused while our update is pending */
  CHECK(keyupdate_follow(&k) == 0 && k.gen == 1);
  keyupdate_acked(&k);
  CHECK(keyupdate_initiate(&k) == 1 && k.gen == 2); /* now allowed */
}
...
static void test_keyupdate_two_adjacent_generations(void) {
  keyupdate k;
  keyupdate_init(&k);
  keyupdate_initiate(&k); /* gen 1, retains {0,1} */
  CHECK(k.lowest == 0 && k.gen == 1);
  keyupdate_acked(&k);
  keyupdate_initiate(&k); /* gen 2, retains {1,2}, drops 0 */
  CHECK(k.lowest == 1 && k.gen == 2);
  CHECK(keyupdate_can_decrypt(&k, 2) == 1); /* current always retained */
  CHECK(keyupdate_can_decrypt(&k, 1) == 1); /* adjacent retained */
  CHECK(keyupdate_can_decrypt(&k, 0) == 0); /* dropped */
}

/* The next generation (one ahead) decrypts on demand; two ahead is refused. */
static void test_keyupdate_decrypt_window(void) {
  keyupdate k;
  keyupdate_init(&k);                       /* gen 0, retains {0} */
  CHECK(keyupdate_can_decrypt(&k, 1) == 1); /* one ahead: on demand */
  CHECK(keyupdate_can_decrypt(&k, 2) == 0); /* two ahead: KEY_UPDATE_ERROR */
}
