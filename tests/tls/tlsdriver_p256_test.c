 ...
 * tlsdriver, reaching the same shared secret on both sides. */

/* Give d a P-256 key pair for group secp256r1: my_priv is the 32-byte
 * scalar, my_pub the 65-byte SEC1 uncompressed point tlsdriver_init's
...
static void test_tlsdriver_p256_ecdhe_agree(void) {
  ...
  /* tlsdriver_init's my_priv/my_pub arguments are a placeholder 32-byte
  ...
  {
    u8 unused_pair[32] = {0};
    tlsdriver_init(&cl, unused_pair, unused_pair, 0);
    tlsdriver_init(&sv, unused_pair, unused_pair, 1);
  }
