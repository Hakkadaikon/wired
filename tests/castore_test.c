...
{
    quic_x509 c;
    CHECK(quic_x509_parse(quic_castore_root_der,
                          sizeof(quic_castore_root_der), &c) == 1);
...
{
    quic_x509 c;
    CHECK(quic_x509_parse(quic_castore_leaf_der,
                          sizeof(quic_castore_leaf_der), &c) == 1);
...
static void test_add_then_find_by_leaf_issuer(void)
{
    ...
    usz root_len, dn_len;
    quic_castore_init(&s);
    CHECK(quic_castore_add(&s, quic_castore_root_der,
                           sizeof(quic_castore_root_der)) == 1);
