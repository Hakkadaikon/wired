...
static void test_accept_pre_handshake(void)
{
    ...
    CHECK(quic_vndrive_accept(0, QUIC_VERSION_1, offered, 2) == 1);
    CHECK(quic_vndrive_accept(1, QUIC_VERSION_1, offered, 2) == 0);
}
...
static void test_accept_downgrade(void)
{
    ...
    CHECK(quic_vndrive_accept(0, QUIC_VERSION_1, offered, 2) == 0);
    CHECK(quic_vndrive_accept(0, QUIC_VERSION_2, offered, 2) == 0);
}
...
static void test_select_pref(void)
{
    ...
    u32 chosen = 0;
    CHECK(quic_vndrive_select(offered, 2, sup2, 2, &chosen) == 1);
    CHECK(chosen == QUIC_VERSION_2);

    u32 sup1[2] = {QUIC_VERSION_1, QUIC_VERSION_2};
    chosen = 0;
    CHECK(quic_vndrive_select(offered, 2, sup1, 2, &chosen) == 1);
...
static void test_select_none(void)
{
    ...
    u32 chosen = 0xdead;
    CHECK(quic_vndrive_select(offered, 1, sup, 2, &chosen) == 0);
}
...
static void test_should_retry(void)
{
    ...
    CHECK(quic_vndrive_should_retry(QUIC_VERSION_2, 1) == 0); /* already retried */
