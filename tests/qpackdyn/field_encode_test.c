...
static void test_indexed_dynamic_golden(void)
{
    u8 out[4];
    usz len = 0;
    usz w = quic_qdyn_indexed_dynamic(5, out, sizeof(out), &len);
    CHECK(w == len && len == 1 && out[0] == 0x85);
}
...
static void test_indexed_dynamic_prefix_boundary(void)
{
    u8 out[4];
    usz len = 0;
    usz w = quic_qdyn_indexed_dynamic(63, out, sizeof(out), &len);
    CHECK(w == len && len == 2 && out[0] == 0xBF && out[1] == 0x00);
}
