...
static void test_insert_literal_golden(void)
{
    ...
    const u8 value[] = {'c','u','s','t','o','m','-','v','a','l','u','e'};
    u8 out[64];
    usz len = 0;
    usz w = quic_qdyn_insert_literal(name, sizeof(name), value, sizeof(value),
                                     out, sizeof(out), &len);
    CHECK(w == len);
    CHECK(len == 1 + 10 + 1 + 12);
    CHECK(out[0] == 0x4a);
...
static void test_insert_literal_namelen_boundary(void)
{
    ...
    usz w = quic_qdyn_insert_literal(name, sizeof(name), value, 1, out,
                                     sizeof(out), &len);
...
static void test_insert_literal_nofit(void)
{
    ...
    u8 out[2];
    usz len = 99;
    CHECK(quic_qdyn_insert_literal(name, 2, value, 1, out, sizeof(out),
                                   &len) == 0);
}
