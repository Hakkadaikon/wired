...
static void qd_roundtrip(quic_qpack_dyn *t, const char *name, const char *value)
{
    u8 fs[8];
    ...
    CHECK(quic_qpack_dyn_insert(t, (const u8 *)name, eq_len(name),
                                (const u8 *)value, eq_len(value)));
    base = t->dropped + t->count;
    CHECK(quic_qpack_dyn_find(t, (const u8 *)name, eq_len(name),
                              (const u8 *)value, eq_len(value), &abs, &matched));
    rel = base - abs - 1;
...
static void test_dynamic_roundtrip(void)
{
    ...
    /* The older entry still resolves after a second insert (base advanced). */
    {
        ...
        CHECK(quic_qdyn_decode_field(&t, base, fs, fs_len, 0, &dn, &dnl, &dv,
                                     &dvl, &consumed));
...
static void test_static_reference(void)
{
    ...
    CHECK(quic_qdyn_decode_field(&t, 0, fs, 1, 0, &dn, &dnl, &dv, &dvl,
                                 &consumed));
    CHECK(consumed == 1);
...
static void test_dynamic_miss(void)
{
    ...
    CHECK(quic_qdyn_decode_field(&t, 1, fs, fs_len, 0, &dn, &dnl, &dv, &dvl,
                                 &consumed) == 0);
}
