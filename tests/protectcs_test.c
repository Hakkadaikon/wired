...
static void pcs_roundtrip(u16 suite, const u8 *key, usz keylen, u8 byte0)
{
    ...
    u8 pkt[64];
    pkt[0] = byte0;
    pkt[1] = 0; pkt[2] = 0; pkt[3] = 0; pkt[4] = 7; /* pn = 7, pn_len = 4 */
    const u8 payload[] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                          0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
                          0x11, 0x12, 0x13, 0x14};
    for (usz i = 0; i < sizeof(payload); i++) pkt[5 + i] = payload[i];

    usz out_len = 0;
    CHECK(quic_protectcs_seal(suite, key, iv, hp, 7, pkt, 1, 4,
                              sizeof(payload), &out_len) == 1);
    ...
    const u8 *got = 0;
    usz got_len = 0;
    CHECK(quic_protectcs_open(suite, key, iv, hp, pkt, out_len, 1,
                              &got, &got_len) == 1);
...
static void cross_suite(void)
{
    ...
    u8 a[64], c[64];
    for (usz s = 0; s < 64; s++) { a[s] = (u8)s; c[s] = (u8)s; }
    a[0] = 0x43; c[0] = 0x43; /* short header, low 2 bits => pn_len 4 */
    a[4] = 9; c[4] = 9;       /* pn = 9 in the low pn byte */

    usz la = 0, lc = 0;
    CHECK(quic_protectcs_seal(QUIC_TLS_AES_128_GCM_SHA256, key, iv, hp, 9,
                              a, 1, 4, 20, &la) == 1);
    CHECK(quic_protectcs_seal(QUIC_TLS_CHACHA20_POLY1305_SHA256, key, iv, hp, 9,
                              c, 1, 4, 20, &lc) == 1);
    ...
    c[lc - 1] ^= 0xff;
    const u8 *p = 0; usz pl = 0;
    CHECK(quic_protectcs_open(QUIC_TLS_CHACHA20_POLY1305_SHA256, key, iv, hp,
                              c, lc, 1, &p, &pl) == 0);
}
...
void test_protectcs(void)
{
    ...
    usz n = 1;
    CHECK(quic_protectcs_seal(0x0000, cha_key, cha_key, cha_key, 1, pkt, 1, 4,
                              20, &n) == 0);
    const u8 *p = 0; usz pl = 0;
    CHECK(quic_protectcs_open(0x0000, cha_key, cha_key, cha_key, pkt, 41, 1,
                              &p, &pl) == 0);
}
