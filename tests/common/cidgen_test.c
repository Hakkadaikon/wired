...
static void test_cidgen_len_bounds(void) {
  CHECK(cid_len_valid(0) == 0);
  CHECK(cid_len_valid(1) == 1);
  CHECK(cid_len_valid(20) == 1);
  CHECK(cid_len_valid(21) == 0);

  u8 cid[20];
  CHECK(cid_generate(cid, 0) == 0);
  CHECK(cid_generate(cid, 21) == 0);
  CHECK(cid_generate(cid, 1) == 1);
  CHECK(cid_generate(cid, 20) == 1);
}
...
static void test_cidgen_distinct_and_full(void) {
  ...
  CHECK(cid_generate(a, 20) == 1);
  CHECK(cid_generate(b, 20) == 1);
  int differ = 0;
  for (usz i = 0; i < 20; i++)
    if (a[i] != b[i]) differ = 1;
  CHECK(differ);
