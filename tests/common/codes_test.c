...
static void test_codes_standard(void) {
  CHECK(error_is_standard(EC_NO_ERROR) == 1);
  CHECK(error_is_standard(EC_VERSION_NEGOTIATION_ERROR) == 1);
  /* 0x12 is just past the enumerated range: not standard */
  CHECK(error_is_standard(0x12) == 0);
  /* CRYPTO_ERROR range boundaries */
  CHECK(error_is_standard(EC_CRYPTO_LO) == 1);
  CHECK(error_is_standard(EC_CRYPTO_HI) == 1);
...
static void test_codes_unrecognized_name(void) {
  u64 code = err_crypto(TLS_ALERT_UNRECOGNIZED_NAME);
  CHECK(TLS_ALERT_UNRECOGNIZED_NAME == 112);
  CHECK(code == 0x0170);
