...
static void test_debug_fmt_u64(void) {
  ...
  at = 0;
  wired_fmt_u64(buf, &at, &(wired_fmt_u64_in){0, 1});
  CHECK(at == 1 && buf[0] == '0');

  at = 0;
  wired_fmt_u64(buf, &at, &(wired_fmt_u64_in){12345, 1});
  CHECK(at == 5);
  CHECK(buf[0] == '1' && buf[4] == '5');

  /* width pads with leading zeros; the value is longer than width -> no pad. */
  at = 0;
  wired_fmt_u64(buf, &at, &(wired_fmt_u64_in){42, 5});
  CHECK(at == 5);
  CHECK(
  ...
  buf[0] = 'x';
  buf[1] = 'y';
  wired_fmt_u64(buf, &at, &(wired_fmt_u64_in){7, 1});
