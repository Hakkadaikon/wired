#include "test.h"

static void test_sent_ack_accounting(void) {
  sent s;
  sent_init(&s);
  sent_on_send(&s, &(sent_out){1, 100, 0});
  sent_on_send(&s, &(sent_out){2, 200, 0});
  CHECK(s.bytes_in_flight == 300);
  CHECK(sent_on_ack(&s, 1) == 1);
  CHECK(s.bytes_in_flight == 200);
  /* duplicate ack is idempotent: no double decrement */
  CHECK(sent_on_ack(&s, 1) == 0);
  CHECK(s.bytes_in_flight == 200);
}

static void test_sent_largest_acked_monotonic(void) {
  sent s;
  sent_init(&s);
  for (u64 pn = 1; pn <= 5; pn++) sent_on_send(&s, &(sent_out){pn, 10, 0});
  sent_on_ack(&s, 5);
  CHECK(s.largest_acked == 5);
  sent_on_ack(&s, 2); /* older ack must not lower it */
  CHECK(s.largest_acked == 5);
}

/* Packet threshold = 3: a packet exactly 3 below largest_acked is lost,
 * one only 2 below is not (modeler: the >= boundary, off-by-one is a bug). */
static void test_sent_packet_threshold_3(void) {
  sent s;
  sent_init(&s);
  sent_on_send(&s, &(sent_out){1, 10, 0}); /* 4 below 5 -> lost */
  sent_on_send(&s, &(sent_out){2, 10, 0}); /* 3 below 5 -> lost (boundary) */
  sent_on_send(&s, &(sent_out){3, 10, 0}); /* 2 below 5 -> NOT lost */
  sent_on_send(&s, &(sent_out){5, 10, 0});
  sent_on_ack(&s, 5);
  CHECK(sent_detect_loss(&s) == 2);
  CHECK(s.pkts[0].state == PKT_LOST);
  CHECK(s.pkts[1].state == PKT_LOST);
  CHECK(s.pkts[2].state == PKT_INFLIGHT);
}

/* An acked packet is never later marked lost (acked/lost exclusivity). */
static void test_sent_acked_never_lost(void) {
  sent s;
  sent_init(&s);
  sent_on_send(&s, &(sent_out){1, 10, 0});
  sent_on_send(&s, &(sent_out){5, 10, 0});
  sent_on_ack(&s, 1); /* pn 1 acked */
  sent_on_ack(&s, 5);
  sent_detect_loss(&s);
  CHECK(s.pkts[0].state == PKT_ACKED); /* stays acked, not lost */
  /* bytes_in_flight: pn1 acked, pn5 acked -> 0 */
  CHECK(s.bytes_in_flight == 0);
}

void test_sent(void) {
  test_sent_ack_accounting();
  test_sent_largest_acked_monotonic();
  test_sent_packet_threshold_3();
  test_sent_acked_never_lost();
}
