...
static void test_transport_connect(void) {
  udp_transport t;
  u32           peer = addr_from_octets((const u8[4]){127, 0, 0, 1});
  udp_transport_connect(&t, peer, 4433);
...
static void test_transport_loopback(void) {
  udp_transport t;
  if (udp_transport_open(&t, 0) != 0) return;
  ...
  udp_transport_connect(&t, addr_from_octets((const u8[4]){127, 0, 0, 1}), 0);
  u8  msg[4] = {1, 2, 3, 4};
  int sent   = udp_transport_send(&t, msg, sizeof msg);
  CHECK(sent == 0 || sent == 1);
}
