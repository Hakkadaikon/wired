...
static void test_verinfo_roundtrip(void) {
  version_information in = {VERSION_1, 2, {VERSION_1, VERSION_2}};
  u8                  buf[64];
  ...
  CHECK(out.chosen == VERSION_1);
  CHECK(out.count == 2);
  CHECK(out.available[0] == VERSION_1);
  CHECK(out.available[1] == VERSION_2);
}

/* Wire bytes are 4 big-endian bytes per version. */
static void test_verinfo_wire_be(void) {
  version_information in = {VERSION_1, 1, {VERSION_2}};
  u8                  buf[8];
...
static void test_verinfo_chosen_only(void) {
  version_information in = {VERSION_2, 0, {0}};
  u8                  buf[8];
  usz                 n = verinfo_encode(buf, sizeof buf, &in);
  CHECK(n == 4);
  version_information out;
  CHECK(verinfo_decode(buf, n, &out) == 4);
  CHECK(out.chosen == VERSION_2 && out.count == 0);
}
