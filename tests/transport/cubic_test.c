...
static void test_cubic_window_vectors(void) {
  u64 k = cubic_k_ms(100);
  CHECK(k == 4217);
  CHECK(cubic_w(k, k, 100) == 100);
  CHECK(cubic_w(0, k, 100) == 70);
  CHECK(cubic_w(k + 10000, k, 100) == 500);
}
...
static void test_cubic_fast_convergence(void) {
  CHECK(cubic_wmax_fastconv(100, 200) == 85);
  CHECK(cubic_wmax_fastconv(200, 200) == 200);
  CHECK(cubic_wmax_fastconv(300, 200) == 300);
}
