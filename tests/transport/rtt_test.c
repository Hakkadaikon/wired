...
static void test_rtt_constant_stays(void) {
  rtt r;
  rtt_init(&r);
  rtt_sample(&r, 80000, 0, 0, 0);
  for (usz i = 0; i < 20; i++) rtt_sample(&r, 80000, 0, 0, 0);
...
static void test_rtt_pto(void) {
  rtt r;
  rtt_init(&r);
  rtt_sample(&r, 100000, 0, 0, 0); /* smoothed=100000, rttvar=50000 */
  CHECK(rtt_pto(&r, 25000) == 100000 + 200000 + 25000);
}
...
static void test_rtt_sample_uses_ack_delay_after_confirm_when_smaller(void) {
  ...
  rtt_sample(&r, 150000, 10000, 25000, 1);
 ...
 * the current min_rtt -- unlike rtt_sample's ordinary lesser-of update. */
static void test_rtt_on_persistent_sets_min_rtt_to_latest(void) {
  rtt r;
  rtt_init(&r);
  rtt_sample(&r, 50000, 0, 0, 0); /* seed: min_rtt=50000 */
  rtt_on_persistent(&r, 200000);  /* path RTT increased */
  CHECK(r.min_rtt == 200000);
}

/* A smaller newest sample also wins outright: rtt_on_persistent always
 * assigns, never takes the lesser-of. */
static void test_rtt_on_persistent_overrides_smaller_min(void) {
  rtt r;
  rtt_init(&r);
  rtt_sample(&r, 50000, 0, 0, 0);
  rtt_on_persistent(&r, 10000);
