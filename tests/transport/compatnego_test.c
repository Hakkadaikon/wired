...
void test_compatnego(void) {
  ...
  CHECK(version_compat_switch_ok(QUIC_VERSION_1, QUIC_VERSION_2) == 1);
  CHECK(version_needs_retry(QUIC_VERSION_1, QUIC_VERSION_2) == 0);

  /* same version: compatible with itself */
  CHECK(version_compat_switch_ok(QUIC_VERSION_1, QUIC_VERSION_1) == 1);

  /* unknown negotiated version: not compatible, needs retry */
  CHECK(version_compat_switch_ok(QUIC_VERSION_1, 0xdeadbeefu) == 0);
  CHECK(version_needs_retry(QUIC_VERSION_1, 0xdeadbeefu) == 1);
