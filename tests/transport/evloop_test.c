...
static void test_iteration_order_receive_before_send(void) {
  evloop c;
  mk(&c);
  evloop_on_receive(&c, 1); /* ack-eliciting: owes an ACK */
...
static void test_ack_owed_then_cleared(void) {
  evloop c;
  mk(&c);
  evloop_on_receive(&c, 0); /* non-eliciting */
  evloop_step(&c, 0);
  CHECK(c.ack_owed == 0);
  CHECK(c.next_pn == 0); /* nothing owed, nothing to send */

  evloop_on_receive(&c, 1); /* eliciting */
  evloop_step(&c, 0);
...
static void test_timers_independent(void) {
  ...
  c.loss.deadline   = 0;
  c.idle.armed      = 1;
  c.idle.deadline   = 100; /* not yet due */
  evloop_step(&c, 1);
...
static void test_pto_deadline_untouched_without_recv(void) {
  ...
  evloop_step(&c, 5); /* no queued receive */
  CHECK(c.pto.armed == 1);
...
static void test_key_update_after_confirmed_only(void) {
  ...
  CHECK(evloop_initiate_key_update(&c, 100) == 0);
  CHECK(c.key_generation == 0);

  c.gate.handshake_confirmed = 1;
  CHECK(evloop_initiate_key_update(&c, 100) == 1);
...
static void test_old_key_retain_then_discard(void) {
  ...
  CHECK(evloop_old_key_retained(&c, 0) == 0); /* gen 0: nothing prior */

  evloop_initiate_key_update(&c, 100);
  CHECK(c.key_generation == 1);
  CHECK(evloop_old_key_retained(&c, 120) == 1); /* within 30 */
  CHECK(evloop_old_key_retained(&c, 140) == 0); /* past 3*PTO: discarded */
}
