...
static void test_prefaddr_may_migrate(void) {
  CHECK(prefaddr_may_migrate(1, 1) == 1);
  CHECK(prefaddr_may_migrate(0, 1) == 0); /* path not validated */
  CHECK(prefaddr_may_migrate(1, 0) == 0); /* handshake not confirmed */
  CHECK(prefaddr_may_migrate(0, 0) == 0);
}

/* Use the preferred_address CID only when one was supplied. */
static void test_prefaddr_use_cid(void) {
  CHECK(prefaddr_use_cid(1) == 1);
  CHECK(prefaddr_use_cid(0) == 0);
}
