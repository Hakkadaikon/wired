...
void test_stream_limit(void) {
  ...
  CHECK(stream_max_id(&(stream_kind){0, 0}, 0, &id) == 0);

  /* client bidi, max_streams 1: only index 0 -> stream ID 0 */
  CHECK(stream_max_id(&(stream_kind){0, 0}, 1, &id) == 1);
  CHECK(id == 0);

  /* client bidi, max_streams 5: index 4 -> ID 16 (4 << 2) */
  CHECK(stream_max_id(&(stream_kind){0, 0}, 5, &id) == 1);
  CHECK(id == 16);

  /* server uni, max_streams 1: index 0 -> low bits 0b11 = 3 */
  CHECK(stream_max_id(&(stream_kind){1, 1}, 1, &id) == 1);
  CHECK(id == 3);

  /* client uni, max_streams 3: index 2 -> (2<<2)|0b10 = 10 */
  CHECK(stream_max_id(&(stream_kind){0, 1}, 3, &id) == 1);
  CHECK(id == 10);

  /* the legal limit boundary: 2^60 is allowed, 2^60 + 1 is not */
  CHECK(stream_max_streams_ok(((u64)1) << 60) == 1);
  CHECK(stream_max_streams_ok((((u64)1) << 60) + 1) == 0);
  CHECK(stream_max_id(&(stream_kind){0, 0}, (((u64)1) << 60) + 1, &id) == 0);
}
