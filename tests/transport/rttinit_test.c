...
static void test_rttinit_is_first(void) {
  CHECK(rtt_is_first(0) == 1);
  CHECK(rtt_is_first(1) == 0);
}

/* First sample: smoothed_rtt = latest, rttvar = latest/2 (RFC 9002 5.2). */
static void test_rttinit_seed(void) {
  CHECK(rtt_first_srtt(100000) == 100000);
  CHECK(rtt_first_rttvar(100000) == 50000);
  CHECK(rtt_first_rttvar(1) == 0); /* truncating divide */
}
