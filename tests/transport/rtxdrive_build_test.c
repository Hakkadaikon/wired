...
static void test_build_roundtrip(void) {
  ...
  rtxbytes_init(&st);
  rtxbytes_store(&st, 7, wired_span_of(s1, sizeof s1));

  CHECK(rtxdrive_build(&st, 7, &ob) == 1);
...
static void test_build_ack_skipped(void) {
  ...
  rtxbytes_init(&st);
  rtxbytes_store(&st, 8, wired_span_of(ack, sizeof ack));

  CHECK(rtxdrive_build(&st, 8, &ob) == 1);
  CHECK(ob.len == 0);
}
...
static void test_build_pn_not_held(void) {
  ...
  rtxbytes_init(&st);
  CHECK(rtxdrive_build(&st, 5, &ob) == 1);
  CHECK(ob.len == 0);
}
...
static void test_build_no_room(void) {
  ...
  rtxbytes_init(&st);
  rtxbytes_store(&st, 3, wired_span_of(s1, sizeof s1));
  CHECK(rtxdrive_build(&st, 3, &ob) == 0);
}
