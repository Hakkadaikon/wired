...
static void test_stream_id_types(void) {
  ...
  CHECK(stream_is_client_initiated(0) == 1 && stream_is_uni(0) == 0);
  CHECK(stream_is_client_initiated(1) == 0 && stream_is_uni(1) == 0);
  CHECK(stream_is_client_initiated(2) == 1 && stream_is_uni(2) == 1);
  CHECK(stream_is_client_initiated(3) == 0 && stream_is_uni(3) == 1);
}
...
static void test_stream_id_construct(void) {
  ...
  CHECK(stream_id(0, 0, 0) == 0);
  CHECK(stream_id(1, 1, 0) == 3);
  /* index 5, server, uni: (5<<2)|3 = 23 */
  u64 id = stream_id(1, 1, 5);
  CHECK(id == 23);
  CHECK(stream_is_client_initiated(id) == 0 && stream_is_uni(id) == 1);
  /* the next client bidi after stream 0 is stream 4 */
  CHECK(stream_id(0, 0, 1) == 4);
}
