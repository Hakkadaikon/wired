...
void test_initpad(void) {
  ...
  CHECK(pktbuild_init_pad(buf, 100, sizeof(buf)) == 1200);
  for (usz i = 100; i < 1200; i++) CHECK(buf[i] == 0x00);
  CHECK(buf[99] == 0xFF); /* existing bytes untouched */

  /* exactly 1200 -> unchanged */
  CHECK(pktbuild_init_pad(buf, 1200, sizeof(buf)) == 1200);
  /* over 1200 -> unchanged */
  CHECK(pktbuild_init_pad(buf, 1250, sizeof(buf)) == 1250);

  /* cap too small to reach 1200 -> unchanged */
  u8 small[500];
  CHECK(pktbuild_init_pad(small, 100, sizeof(small)) == 100);
}
