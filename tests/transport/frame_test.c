...
static void test_frame_crypto_truncated(void) {
  ...
  crypto_frame in        = {.offset = 0, .length = 3, .data = payload};
  u8           buf[32];
  usz          w = frame_put_crypto(buf, sizeof(buf), &in);
  crypto_frame out;
  CHECK(frame_get_crypto(buf, w - 1, &out) == 0); /* data cut short */
  CHECK(frame_put_crypto(buf, 2, &in) == 0);      /* header no room */
}
...
static void test_frame_stream_truncated(void) {
  ...
  usz          w = frame_put_stream(buf, sizeof(buf), &in);
  stream_frame out;
  CHECK(frame_get_stream(buf, w - 1, &out) == 0);
  CHECK(frame_put_stream(buf, 2, &in) == 0);
}
...
static void test_frame_stream_implicit_length(void) {
  ...
  stream_frame f;
  usz          r = frame_get_stream(buf, sizeof buf, &f);
  CHECK(r == sizeof buf);
  ...
  r               = frame_get_stream(buf2, sizeof buf2, &f);
  CHECK(r == sizeof buf2);
