...
static void test_tokentype_retry(void) {
  ...
  usz        w  = quic_token_tag_retry(&ob, wired_span_of(body, sizeof(body)));
...
static void test_tokentype_newtoken(void) {
  ...
  u8         out[8];
  wired_obuf ob = quic_obuf_of(out, sizeof(out));
  usz w = quic_token_tag_newtoken(&ob, wired_span_of(body, sizeof(body)));
...
static void test_tokentype_empty(void) {
  ...
  CHECK(quic_token_tag_retry(&ob, wired_span_of((const u8*)0, 0)) == 1);
  CHECK(quic_token_is_retry(out, 1) == 1);
  /* no room */
  wired_obuf ob0 = quic_obuf_of(out, 0);
  CHECK(quic_token_tag_retry(&ob0, wired_span_of((const u8*)0, 0)) == 0);
}
