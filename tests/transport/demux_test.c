...
void test_demux(void) {
  ...
  CHECK(
      quic_demux_match(quic_span_of(a, 4), quic_span_of(b, 4)) ==
      1); /* same bytes + length */
  CHECK(
      quic_demux_match(quic_span_of(a, 4), quic_span_of(c, 4)) ==
      0); /* last byte differs */
  CHECK(
      quic_demux_match(quic_span_of(a, 4), quic_span_of(d, 3)) ==
      0); /* length differs */
