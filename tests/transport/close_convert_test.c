...
static void test_close_needs_convert_matrix(void) {
  CHECK(close_needs_convert(1, 1) == 1); /* app close in handshake */
  CHECK(close_needs_convert(1, 0) == 0); /* app close, post-handshake */
  CHECK(close_needs_convert(0, 1) == 0); /* transport close in handshake */
  CHECK(close_needs_convert(0, 0) == 0); /* transport close otherwise */
}
