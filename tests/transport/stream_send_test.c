...
static void test_stream_frame_basic(void) {
  ...
  CHECK(quic_appdata_stream_frame(
      4, 0, data, sizeof(data), 0, out, sizeof(out), &olen));
...
static void test_stream_frame_off_fin(void) {
  ...
  u8       out[32];
  usz      olen = 0;
  CHECK(quic_appdata_stream_frame(7, 100, data, 1, 1, out, sizeof(out), &olen));
  CHECK(
...
static void test_stream_frame_off_nofin(void) {
  ...
  u8       out[32];
  usz      olen = 0;
  CHECK(quic_appdata_stream_frame(3, 5, data, 1, 0, out, sizeof(out), &olen));
...
static void test_stream_frame_empty(void) {
  ...
  usz olen = 0;
  CHECK(quic_appdata_stream_frame(
      8, 0, (const u8 *)"", 0, 1, out, sizeof(out), &olen));
