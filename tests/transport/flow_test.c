...
static void test_reasm_contiguous_only(void) {
  quic_reasm r;
  quic_reasm_init(&r);
  quic_reasm_insert(&r, 0, wired_span_of((const u8*)"abc", 3));
  quic_reasm_insert(&r, 6, wired_span_of((const u8*)"ghi", 3)); /* gap */
  CHECK(quic_reasm_deliver(&r) == 3); /* stops at the hole */
  quic_reasm_insert(&r, 3, wired_span_of((const u8*)"def", 3)); /* fill */
...
static void test_reasm_idempotent(void) {
  quic_reasm r;
  quic_reasm_init(&r);
  quic_reasm_insert(&r, 0, wired_span_of((const u8*)"hello", 5));
  CHECK(quic_reasm_deliver(&r) == 5);
  quic_reasm_insert(&r, 2, wired_span_of((const u8*)"llo", 3)); /* overlap */
