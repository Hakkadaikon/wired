...
/* RFC 9000 19.3: ackgen_build_ranges's gap-encoded output (largest +
 * First ACK Range Length + (Gap, Range Length) pairs) converts to
 * ack_frame's explicit [hi, lo] range array, the format ack_encode
 * requires. */
void test_ackrangeconv(void) {
  /* single contiguous block 3..7: one explicit range {7, 3}. */
  {
    ...
    ack_frame f      = {0};
    CHECK(ackrangeconv_to_frame(7, raw, 1, &f) == 1);
    CHECK(f.n_ranges == 1);
   ...
   * ackrange_test.c): explicit ranges {5,4} then {1,0}. */
  {
    ...
    ack_frame f      = {0};
    CHECK(ackrangeconv_to_frame(5, raw, 3, &f) == 1);
    CHECK(f.n_ranges == 2);
    CHECK(f.ranges[0].hi == 5 && f.ranges[0].lo == 4);
    CHECK(f.ranges[1].hi == 1 && f.ranges[1].lo == 0);
  }

  /* zero raw values (0 ranges written, e.g. ackgen_build_ranges never
   ...
   * fails cleanly instead of overflowing. */
  {
    ...
    CHECK(
        ackrangeconv_to_frame(1000, raw, sizeof(raw) / sizeof(raw[0]), &f) ==
        0);
  }
}
