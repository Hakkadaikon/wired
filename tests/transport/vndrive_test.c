...
static void test_accept_pre_handshake(void) {
  ...
  CHECK(vndrive_accept(0, QUIC_VERSION_1, verlist_of(offered, 2)) == 1);
  CHECK(vndrive_accept(1, QUIC_VERSION_1, verlist_of(offered, 2)) == 0);
}
...
static void test_accept_downgrade(void) {
  ...
  CHECK(vndrive_accept(0, QUIC_VERSION_1, verlist_of(offered, 2)) == 0);
  CHECK(vndrive_accept(0, QUIC_VERSION_2, verlist_of(offered, 2)) == 0);
}
...
static void test_should_retry(void) {
  CHECK(vndrive_should_retry(QUIC_VERSION_2, 0) == 1); /* 1st time */
  CHECK(vndrive_should_retry(QUIC_VERSION_2, 1) == 0); /* already retried */
