...
static void test_stream_read_contiguous(void) {
  ...
  u8 in[3] = {1, 2, 3};
  CHECK(quic_stream_read_push(&s, 0, quic_span_of(in, 3)) == 1);
...
static void test_stream_read_stops_at_gap(void) {
  ...
  CHECK(quic_stream_read_push(&s, 0, quic_span_of(a, 2)) == 1);
  CHECK(quic_stream_read_push(&s, 4, quic_span_of(b, 2)) == 1);
...
static void test_stream_read_fills_gap_then_continues(void) {
  ...
  u8 b[2]   = {20, 21};
  u8 mid[2] = {12, 13};
  quic_stream_read_push(&s, 0, quic_span_of(a, 2));
  quic_stream_read_push(&s, 4, quic_span_of(b, 2));
  ...
  CHECK(quic_stream_read_push(&s, 2, quic_span_of(mid, 2)) == 1);
  quic_stream_read_pull(&s, &ob);
  CHECK(ob.len == 4);
...
static void test_stream_read_respects_cap(void) {
  ...
  quic_stream_read_init(&s);
  u8 in[4] = {1, 2, 3, 4};
  quic_stream_read_push(&s, 0, quic_span_of(in, 4));

  u8        out[8];
  quic_obuf small = quic_obuf_of(out, 2); /* cap of 2 limits the pull */
  quic_stream_read_pull(&s, &small);
  CHECK(small.len == 2);
  CHECK(out[0] == 1 && out[1] == 2);
  quic_obuf rest = quic_obuf_of(out, 8);
