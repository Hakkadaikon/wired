...
static void test_short_bad_args(void)
{
    const u8 dcid[4] = {0};
    u8 buf[16];
    CHECK(quic_short_build(buf, sizeof(buf), dcid, 4, 0, 0, 0, 0) == 0); /* pn_len 0 */
    CHECK(quic_short_build(buf, sizeof(buf), dcid, 4, 0, 0, 0, 5) == 0); /* pn_len 5 */
...
static void test_retry_roundtrip(void)
{
    ...
    u8 buf[64];
    usz w = quic_retry_build(buf, sizeof(buf), 1, dcid, 3, scid, 2,
                             token, 5, tag);
...
static void test_retry_truncated(void)
{
    ...
    u8 buf[64];
    usz w = quic_retry_build(buf, sizeof(buf), 1, dcid, 1, scid, 1, token, 1, tag);
    CHECK(w != 0);
    ...
    CHECK(quic_retry_parse(buf, QUIC_RETRY_TAG_LEN + 6, &r) == 0); /* CID len overruns */
...
static void test_vneg_roundtrip(void)
{
    ...
    CHECK(buf[0] == 0x80);
    CHECK(buf[1] == 0 && buf[2] == 0 && buf[3] == 0 && buf[4] == 0); /* Version 0 */
