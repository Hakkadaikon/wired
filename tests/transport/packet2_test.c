...
static void test_short_build(void) {
  ...
  u8         buf[16];
  short_desc d = {wired_span_of(dcid, 4), 1, 1, 0xABCD, 2};
  usz        w = short_build(buf, sizeof(buf), &d);
  CHECK(w == 1 + 4 + 2);
...
static void test_short_build_min(void) {
  ...
  u8         buf[8];
  short_desc d = {wired_span_of(dcid, 1), 0, 0, 0x7F, 1};
  usz        w = short_build(buf, sizeof(buf), &d);
  CHECK(w == 3);
...
static void test_short_bad_args(void) {
  const u8   dcid[4] = {0};
  u8         buf[16];
  short_desc d0 = {wired_span_of(dcid, 4), 0, 0, 0, 0};
  short_desc d5 = {wired_span_of(dcid, 4), 0, 0, 0, 5};
  short_desc d1 = {wired_span_of(dcid, 4), 0, 0, 0, 1};
  CHECK(short_build(buf, sizeof(buf), &d0) == 0); /* pn_len 0 */
  CHECK(short_build(buf, sizeof(buf), &d5) == 0); /* pn_len 5 */
  CHECK(short_build(buf, 3, &d1) == 0);           /* no room */
}
...
static void test_vneg_roundtrip(void) {
  ...
  u8        buf[64];
  vneg_desc vd = {wired_span_of(dcid, 2), wired_span_of(scid, 3), vers, 2};
  usz       w  = vneg_build(buf, sizeof(buf), &vd);
...
static void test_vneg_respond_swap(void) {
  ...
  usz w = vneg_respond(buf, sizeof(buf), &rv);
  CHECK(w != 0);

  vneg_packet v;
  CHECK(vneg_parse(buf, w, &v) == w);
...
static void test_vneg_bad(void) {
  ...
  const u32 vers[1] = {0x00000001};
  u8        buf[64];
  vneg_desc v0 = {wired_span_of(dcid, 1), wired_span_of(scid, 1), vers, 0};
  vneg_desc v1 = {wired_span_of(dcid, 1), wired_span_of(scid, 1), vers, 1};
  /* count 0 must fail to build */
  CHECK(vneg_build(buf, sizeof(buf), &v0) == 0);

  usz w = vneg_build(buf, sizeof(buf), &v1);
  CHECK(w != 0);
  buf[4] = 1; /* corrupt Version field to non-zero */
  vneg_packet v;
  CHECK(vneg_parse(buf, w, &v) == 0);
}
