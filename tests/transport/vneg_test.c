...
static void test_vneg_downgrade_checks(void) {
  vneg         v;
  version_info vi = {.chosen = QUIC_VERSION_1, .n_available = 2};
  vi.available[0] = QUIC_VERSION_1;
  vi.available[1] = QUIC_VERSION_2;

  init2(&v);
  CHECK(vneg_check_downgrade(&v, &vi, QUIC_VERSION_1) == 1);

  /* Chosen != in_use */
  init2(&v);
  CHECK(vneg_check_downgrade(&v, &vi, QUIC_VERSION_2) == 0);
  CHECK(v.phase == QUIC_VNEG_ERROR);

  /* empty Available */
  init2(&v);
  version_info empty = {.chosen = QUIC_VERSION_1, .n_available = 0};
  CHECK(vneg_check_downgrade(&v, &empty, QUIC_VERSION_1) == 0);

  /* Chosen not in Available */
  init2(&v);
  version_info bad = {.chosen = QUIC_VERSION_2, .n_available = 1};
  bad.available[0] = QUIC_VERSION_1;
  CHECK(vneg_check_downgrade(&v, &bad, QUIC_VERSION_2) == 0);
}
...
static void test_vneg_confirm(void) {
  vneg v;
  init2(&v);
  vneg_confirm(&v, QUIC_VERSION_2);
  CHECK(v.phase == QUIC_VNEG_CONFIRMED && v.negotiated == QUIC_VERSION_2);

  init2(&v);
  version_info bad = {.chosen = QUIC_VERSION_2, .n_available = 0};
  vneg_check_downgrade(&v, &bad, QUIC_VERSION_1); /* -> ERROR */
  vneg_confirm(&v, QUIC_VERSION_1);
