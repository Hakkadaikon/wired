...
static void test_batch_concat(void) {
  ...
  rtxbytes_store(&st, 12, wired_span_of(s2, sizeof s2));

  CHECK(rtxdrive_batch(&st, (lost_pns){lost, 4}, &ob) == 1);
...
static void test_batch_truncates_on_cap(void) {
  ...
  CHECK(rtxdrive_batch(&st, (lost_pns){lost, 2}, &ob) == 1);
...
static void test_batch_empty_store(void) {
  ...
  rtxbytes_init(&st);
  CHECK(rtxdrive_batch(&st, (lost_pns){lost, 3}, &ob) == 1);
  CHECK(ob.len == 0);
}
