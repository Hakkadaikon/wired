...
static void test_batch_concat(void) {
  ...
  quic_rtxbytes_store(&st, 11, wired_span_of(ack, sizeof ack));
  quic_rtxbytes_store(&st, 12, wired_span_of(s2, sizeof s2));

  CHECK(quic_rtxdrive_batch(&st, (quic_lost_pns){lost, 4}, &ob) == 1);
...
static void test_batch_truncates_on_cap(void) {
  ...
  CHECK(quic_rtxdrive_batch(&st, (quic_lost_pns){lost, 2}, &ob) == 1);
