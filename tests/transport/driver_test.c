...
static void test_driver_handshake_completes(void) {
  ...
  for (round = 0; round < 32; round++) {
    ...
    pump(&cl, &sv);
    pump(&sv, &cl);
    if (driver_handshake_complete(&cl) && driver_handshake_complete(&sv)) break;
  }
...
static void test_driver_rejects_uninstalled_level(void) {
  ...
  driver_step(&cl); /* client emits ClientHello */
  n = driver_take(&cl, dg, sizeof(dg));
  CHECK(n != 0);
  driver_feed(&sv, dg, n);
  driver_step(&sv); /* server consumes ClientHello */
  driver_step(&sv); /* server emits ServerHello */
