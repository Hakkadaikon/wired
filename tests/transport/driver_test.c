...
static void test_driver_handshake_completes(void) {
  ...
  quic_driver cl, sv;
  int         round;
  quic_driver_init(&cl, 0, wired_span_of(dcid, 8));
  quic_driver_init(&sv, 1, wired_span_of(dcid, 8));
...
static void test_driver_rejects_uninstalled_level(void) {
  ...
  CHECK(
      quic_connio_recv(&cl.io, QUIC_LEVEL_HANDSHAKE, wired_mspan_of(dg, n)) ==
      0);
...
static void test_driver_run_halts_at_max(void) {
  ...
  quic_driver cl;
  usz         steps;
  quic_driver_init(&cl, 0, wired_span_of(dcid, 4));
