...
static void test_ipv4_checksum(void) {
  ...
  quic_ipv4_build(hdr, 1200, 0x7f000001, 0x7f000002, QUIC_IP_PROTO_UDP);
...
static void test_udp_checksum(void) {
  ...
  usz      n    = quic_udp4_build(
      dg, sizeof(dg), 0x1234, 0x4321, 0x7f000001, 0x7f000002, pl, 4);
  CHECK(n == QUIC_UDP_HDR + 4);
  CHECK(quic_udp4_check(dg, n, 0x7f000001, 0x7f000002) == 1);
  dg[QUIC_UDP_HDR + 1] ^= 0x10;
  CHECK(quic_udp4_check(dg, n, 0x7f000001, 0x7f000002) == 0);
}
...
static void test_net_datagram_over_link(void) {
  ...
  usz      un   = quic_udp4_build(
      udp, sizeof(udp), 9000, 443, 0x0a000001, 0x0a000002, pl, 5);
  quic_ipv4_build(
      ip, (u16)(QUIC_IPV4_HDR + un), 0x0a000001, 0x0a000002, QUIC_IP_PROTO_UDP);
  ...
  CHECK(quic_udp4_check(rx + QUIC_IPV4_HDR, un, 0x0a000001, 0x0a000002) == 1);
