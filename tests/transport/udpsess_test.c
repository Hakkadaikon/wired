...
static void test_udpsess_init_active_path(void) {
  ...
  t.peer_port              = 4433;
  udpsess_init(&s, &t, wired_span_of(dcid0, 3));
  CHECK(s.active == 0);
...
static void test_udpsess_set_peer_keeps_old(void) {
  ...
  t.peer_port              = 4433;
  udpsess_init(&s, &t, wired_span_of(dcid0, 1));
  udpsess_set_peer(&s, 1, &(udpsess_peer){0x0b000002, 5555});
...
static void test_udpsess_migrate_gate(void) {
  ...
  udpsess_init(&s, &t, wired_span_of(dcid0, 1));
  udpsess_set_peer(&s, 1, &(udpsess_peer){0x0b000002, 5555});
  CHECK(udpsess_can_migrate(&s, 0) == 0);
  CHECK(udpsess_can_migrate(&s, 1) == 1);
  CHECK(udpsess_migrate(&s, 1, 0) == 0); /* unvalidated: refused */
...
static void test_udpsess_migrate_needs_peer(void) {
  ...
  t.peer_port              = 4433;
  udpsess_init(&s, &t, wired_span_of(dcid0, 1));
  CHECK(udpsess_migrate(&s, 1, 1) == 0); /* path 1 peer unset */
  CHECK(s.active == 0);
}
...
static void test_udpsess_dcid_per_path(void) {
  ...
  t.peer_addr = 0x0a000001;
  t.peer_port = 4433;
  udpsess_init(&s, &t, wired_span_of(dcid0, 2));
  udpsess_set_dcid(&s, 1, wired_span_of(dcid1, 3));
  CHECK(udpsess_dcid_for_path(&s, 0, &out) == 1);
  CHECK(out.p == dcid0 && out.n == 2);
  CHECK(udpsess_dcid_for_path(&s, 1, &out) == 1);
...
static void test_udpsess_dcid_unset(void) {
  ...
  t.peer_port = 4433;
  udpsess_init(&s, &t, wired_span_of(dcid0, 1));
  CHECK(udpsess_dcid_for_path(&s, 1, &out) == 0); /* unset */
  CHECK(udpsess_dcid_for_path(&s, 2, &out) == 0); /* out of range */
}
