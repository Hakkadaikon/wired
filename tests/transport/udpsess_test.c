...
static void test_udpsess_set_peer_keeps_old(void)
{
    ...
    t.peer_port = 4433;
    quic_udpsess_init(&s, &t, dcid0, 1);
    quic_udpsess_set_peer(&s, 1, 0x0b000002, 5555);
...
static void test_udpsess_dcid_per_path(void)
{
    ...
    t.peer_addr = 0x0a000001;
    t.peer_port = 4433;
    quic_udpsess_init(&s, &t, dcid0, 2);
    quic_udpsess_set_dcid(&s, 1, dcid1, 3);
    CHECK(quic_udpsess_dcid_for_path(&s, 0, &out, &len) == 1);
    CHECK(out == dcid0 && len == 2);
    CHECK(quic_udpsess_dcid_for_path(&s, 1, &out, &len) == 1);
    CHECK(out == dcid1 && len == 3);
}
...
static void test_udpsess_dcid_unset(void)
{
    ...
    t.peer_port = 4433;
    quic_udpsess_init(&s, &t, dcid0, 1);
    CHECK(quic_udpsess_dcid_for_path(&s, 1, &out, &len) == 0); /* unset */
    CHECK(quic_udpsess_dcid_for_path(&s, 2, &out, &len) == 0); /* out of range */
}
