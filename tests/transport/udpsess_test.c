...
static void test_udpsess_init_active_path(void) {
  ...
  t.peer_port                 = 4433;
  quic_udpsess_init(&s, &t, wired_span_of(dcid0, 3));
  CHECK(s.active == 0);
...
static void test_udpsess_set_peer_keeps_old(void) {
  ...
  t.peer_port                 = 4433;
  quic_udpsess_init(&s, &t, wired_span_of(dcid0, 1));
...
static void test_udpsess_dcid_per_path(void) {
  ...
  t.peer_addr = 0x0a000001;
  t.peer_port = 4433;
  quic_udpsess_init(&s, &t, wired_span_of(dcid0, 2));
  quic_udpsess_set_dcid(&s, 1, wired_span_of(dcid1, 3));
