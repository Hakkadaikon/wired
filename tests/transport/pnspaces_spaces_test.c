...
static void test_pnspaces_spaces_independent_numbering(void) {
  ...
  CHECK(pnspaces_next_pn(&s, 0) == 0);
  CHECK(pnspaces_next_pn(&s, 0) == 1);
  CHECK(pnspaces_next_pn(&s, 0) == 2);

  /* Allocating in Initial did not advance Handshake or Application. */
  CHECK(pnspaces_next_pn(&s, 1) == 0); /* Handshake still 0 */
  CHECK(pnspaces_next_pn(&s, 2) == 0); /* Application still 0 */

  /* Cross-checks: spaces stay independent after interleaving. */
  CHECK(pnspaces_next_pn(&s, 1) == 1);
  CHECK(pnspaces_next_pn(&s, 0) == 3); /* Initial resumes from 3 */
  CHECK(pnspaces_next_pn(&s, 2) == 1);
}
