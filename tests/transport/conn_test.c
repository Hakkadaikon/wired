...
static void test_conn_handshake(void) {
  conn c;
  conn_init(&c);
  CHECK(c.phase == PHASE_INITIAL);
  CHECK(conn_step(&c, CONN_EV_HS_PROGRESS) && c.phase == PHASE_HANDSHAKE);
  CHECK(conn_step(&c, CONN_EV_HS_CONFIRMED) && c.phase == PHASE_CONFIRMED);
}
...
static void test_conn_no_regress(void) {
  conn c;
  conn_init(&c);
  conn_step(&c, CONN_EV_HS_PROGRESS);
  conn_step(&c, CONN_EV_HS_CONFIRMED);
  /* no event drives Confirmed back to Handshake/Initial */
  CHECK(!conn_step(&c, CONN_EV_HS_PROGRESS) && c.phase == PHASE_CONFIRMED);
}
...
static void test_conn_close(void) {
  ...
  conn_init(&c);
  conn_step(&c, CONN_EV_HS_PROGRESS);
  CHECK(conn_step(&c, CONN_EV_CLOSE) && c.phase == PHASE_CLOSING);
  CHECK(conn_step(&c, CONN_EV_CLOSED) && c.phase == PHASE_CLOSED);
  CHECK(!conn_step(&c, CONN_EV_HS_PROGRESS) && c.phase == PHASE_CLOSED);
}
...
static void test_conn_pn_monotonic_independent(void) {
  ...
  conn_init(&c);
  u64 a, b, d;
  CHECK(conn_next_pn(&c, PN_INITIAL, &a) && a == 0);
  CHECK(conn_next_pn(&c, PN_INITIAL, &b) && b == 1);
  /* a different space is unaffected by Initial's counter */
  CHECK(conn_next_pn(&c, PN_HANDSHAKE, &d) && d == 0);
}
...
static void test_conn_app_space_gated(void) {
  ...
  conn_init(&c);
  u64 pn;
  CHECK(!conn_next_pn(&c, PN_APPLICATION, &pn)); /* Initial phase */
  conn_step(&c, CONN_EV_HS_PROGRESS);
  CHECK(!conn_next_pn(&c, PN_APPLICATION, &pn)); /* Handshake phase */
  conn_step(&c, CONN_EV_HS_CONFIRMED);
  CHECK(conn_next_pn(&c, PN_APPLICATION, &pn) && pn == 0);
}
