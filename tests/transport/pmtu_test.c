...
static void test_pmtu_max_probes_above_validated_caps_ceiling_only(void) {
  ...
  usz probe = pmtu_next_probe(&p, 0);
  int i;
  for (i = 0; i <= QUIC_PMTU_MAX_PROBES; i++) pmtu_on_loss(&p, probe);
  CHECK(p.ceiling == probe);
  CHECK(p.validated == QUIC_PMTU_BASE);
...
static void test_pmtu_black_hole_lowers_validated(void) {
  ...
  int i;
  for (i = 0; i <= QUIC_PMTU_MAX_PROBES; i++) pmtu_on_loss(&p, grown);
...
static void test_pmtu_mps(void) {
  pmtu p;
  pmtu_init(&p);
  CHECK(pmtu_mps(&p) == QUIC_PMTU_BASE - QUIC_PMTU_OVERHEAD);
  pmtu_on_ack(&p, QUIC_PMTU_BASE + QUIC_PMTU_STEP);
  CHECK(pmtu_mps(&p) == QUIC_PMTU_BASE + QUIC_PMTU_STEP - QUIC_PMTU_OVERHEAD);
}
...
static void test_pmtu_probe_timer_not_due_without_probe(void) {
  pmtu p;
  pmtu_init(&p);
  CHECK(pmtu_probe_timer_due(&p, QUIC_PMTU_PROBE_TIMER_US * 10) == 0);
}
...
static void test_pmtu_probe_timer_due_after_elapsed(void) {
  ...
  CHECK(pmtu_probe_timer_due(&p, 1000 + QUIC_PMTU_PROBE_TIMER_US - 1) == 0);
  CHECK(pmtu_probe_timer_due(&p, 1000 + QUIC_PMTU_PROBE_TIMER_US) == 1);
}
...
static void test_pmtu_probe_timer_canceled_on_resolve(void) {
  ...
  pmtu_on_ack(&p, probe);
  CHECK(pmtu_probe_timer_due(&p, 1000 + QUIC_PMTU_PROBE_TIMER_US) == 0);
}
...
static void test_pmtu_raise_timer_not_due_while_searching(void) {
  pmtu p;
  pmtu_init(&p);
  CHECK(pmtu_raise_timer_due(&p, (u64)QUIC_PMTU_RAISE_TIMER_US * 10) == 0);
}
...
static void test_pmtu_raise_timer_due_after_elapsed(void) {
  ...
  CHECK(p.searching == 0);
  CHECK(pmtu_raise_timer_due(&p, 100 + QUIC_PMTU_RAISE_TIMER_US - 1) == 0);
  CHECK(pmtu_raise_timer_due(&p, 100 + QUIC_PMTU_RAISE_TIMER_US) == 1);
}
