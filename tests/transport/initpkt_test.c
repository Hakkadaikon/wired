...
static void test_initpkt_keys_rfc(void) {
  ...
  const u8 want_iv[12]  = {0xfa, 0x04, 0x4b, 0x2f, 0x42, 0xa3,
...
static void test_initpkt_roundtrip(void) {
  ...
  u8  pkt[1300];
  usz total = 0;
  CHECK(quic_initpkt_build(
      dcid, 8, scid, 4, ch, sizeof(ch), 2, pkt, sizeof(pkt), &total));
...
static void test_initpkt_tamper(void) {
  ...
  usz      total = 0;
  CHECK(quic_initpkt_build(
      dcid, 8, 0, 0, ch, sizeof(ch), 7, pkt, sizeof(pkt), &total));
  pkt[total - 1] ^= 0x01;
  const u8 *crypto = 0;
  usz       clen   = 0;
