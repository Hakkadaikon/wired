...
static void test_cidnego_adopt(void) {
  ...
  CHECK(ob.len == 5);
  CHECK(
      quic_cidnego_match(quic_span_of(dcid, ob.len), quic_span_of(scid, 5)) ==
      1);
}
...
static void test_cidnego_len_bounds(void) {
  ...
  quic_obuf ob = quic_obuf_of(dcid, sizeof(dcid));
  CHECK(
      quic_cidnego_peer_dcid(quic_span_of(scid, 20), &ob) == 1 && ob.len == 20);
...
static void test_cidnego_match_neg(void) {
  ...
  u8 b[3] = {1, 2, 4};
  CHECK(
      quic_cidnego_match(quic_span_of(a, 3), quic_span_of(b, 3)) ==
      0); /* last byte differs */
  CHECK(
      quic_cidnego_match(quic_span_of(a, 3), quic_span_of(a, 2)) ==
      0); /* length differs */
