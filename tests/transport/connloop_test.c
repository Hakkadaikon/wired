...
static void test_send_level_never_regresses(void) {
  ...
  keyset_install(&c.keys, LEVEL_HANDSHAKE, &k);

  CHECK(
      connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 0, 10}) == 1);
  int before = c.send_level;
  CHECK(before == LEVEL_INITIAL);
  /* promote up to Handshake: allowed */
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){LEVEL_HANDSHAKE, 1, 1, 10}) ==
      1);
  CHECK(c.send_level == LEVEL_HANDSHAKE);
  /* regress back to Initial: refused, level unchanged (before/after) */
  int hi = c.send_level;
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 2, 10}) == 0);
...
static void test_no_app_data_before_handshake_complete(void) {
  ...
  connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 0, 10});
  connloop_on_send(&c, &(connloop_send_in){LEVEL_HANDSHAKE, 1, 1, 10});

  /* handshake not complete: 1-RTT send refused */
  CHECK(connloop_on_send(&c, &(connloop_send_in){LEVEL_ONERTT, 1, 2, 10}) == 0);
  CHECK(c.send_level == LEVEL_HANDSHAKE);

  /* after completion: 1-RTT send allowed */
  c.handshake_complete = 1;
  CHECK(connloop_on_send(&c, &(connloop_send_in){LEVEL_ONERTT, 1, 2, 10}) == 1);
  CHECK(c.send_level == LEVEL_ONERTT);
}
...
static void test_server_send_capped_3x_recv(void) {
  ...
  connloop_on_recv(&c, LEVEL_INITIAL, 100); /* budget = 300 */
  CHECK(c.recv_bytes == 100);
  /* exactly 3x allowed */
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 0, 300}) == 1);
  CHECK(c.sent_bytes == 300);
  /* one more byte exceeds 3x: refused, sent_bytes unchanged (before/after) */
  u64 before = c.sent_bytes;
  CHECK(connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 1, 1}) == 0);
...
static void test_validate_lifts_3x_cap(void) {
  ...
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 0, 300}) == 1);
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 1, 1}) ==
      0); /* capped */

  connloop_validate(&c); /* address validated: cap lifted */
  CHECK(c.validated == 1);
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 1, 9999}) ==
      1);
}
...
static void test_recv_drops_packet_without_key(void) {
  ...
  CHECK(connloop_on_recv(&c, LEVEL_HANDSHAKE, 50) == 0);
...
static void test_closed_processes_no_packet(void) {
  ...
  initial_keys k = {0};
  keyset_install(&c.keys, LEVEL_INITIAL, &k);
  c.phase    = CONNLOOP_CLOSED;
  u64 before = c.recv_bytes;
  CHECK(connloop_on_recv(&c, LEVEL_INITIAL, 50) == 0);
...
static void test_ack_removes_tracked_packet(void) {
  ...
  keyset_install(&c.keys, LEVEL_INITIAL, &k);
  connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 0, 10});
  connloop_on_send(&c, &(connloop_send_in){LEVEL_INITIAL, 1, 1, 10});
  CHECK(sentpkt_count(&c.sent) == 2);

  /* spurious ACK for pn 9 (never sent): removes nothing */
  u64 spur = 0;
  CHECK(connloop_on_ack(&c, &(connloop_ack_in){9, &spur, 1}) == 0);
  CHECK(sentpkt_count(&c.sent) == 2);

  /* ACK pn 0..1 (largest 1, first range covers down to 0): removes both */
  u64 r      = 1;
  usz before = sentpkt_count(&c.sent);
  CHECK(connloop_on_ack(&c, &(connloop_ack_in){1, &r, 1}) == 2);
  CHECK(sentpkt_count(&c.sent) < before);
...
static void test_pto_sends_probe_keeps_inflight(void) {
  ...
  CHECK(connloop_on_pto(&c, &(connloop_pto_in){LEVEL_INITIAL, 0, 10}) == 0);
  CHECK(c.pto_armed == 0);
  ...
  CHECK(connloop_on_pto(&c, &(connloop_pto_in){LEVEL_INITIAL, 1, 10}) == 1);
  CHECK(sentpkt_count(&c.sent) >= before);
  CHECK(sentpkt_count(&c.sent) == before + 1);
}
...
static void test_closing_never_returns_open(void) {
  connloop c;
  connloop_init(&c, 1);
  CHECK(c.phase == CONNLOOP_ACTIVE);

  connloop_close(&c, 0); /* local close */
  int after_local = c.phase;
  CHECK(after_local == CONNLOOP_CLOSING);

  connloop_close(&c, 0); /* advance closing -> draining */
  CHECK(c.phase == CONNLOOP_DRAINING);
  connloop_close(&c, 0); /* draining -> closed */
  CHECK(c.phase == CONNLOOP_CLOSED);
...
static void test_closing_sends_no_app_data(void) {
  ...
  initial_keys k       = {0};
  keyset_install(&c.keys, LEVEL_ONERTT, &k);
  c.send_level = LEVEL_HANDSHAKE;
  connloop_close(&c, 0); /* -> closing */
  CHECK(connloop_on_send(&c, &(connloop_send_in){LEVEL_ONERTT, 1, 0, 10}) == 0);
}
...
static void test_no_recv_at_discarded_level(void) {
  ...
  initial_keys k = {0};
  keyset_install(&c.keys, LEVEL_INITIAL, &k);
  CHECK(connloop_on_recv(&c, LEVEL_INITIAL, 10) == 1);
  keyset_discard(&c.keys, LEVEL_INITIAL);
  /* after discard: processing at that level refused */
  CHECK(connloop_on_recv(&c, LEVEL_INITIAL, 10) == 0);
}
...
static void test_handshake_progress_reaches_confirmed(void) {
  ...
  initial_keys k = {0};
  int          lv;
  for (lv = LEVEL_INITIAL; lv <= LEVEL_ONERTT; lv++) {
    keyset_install(&cl.keys, lv, &k);
    keyset_install(&sv.keys, lv, &k);
  }
  /* Initial then Handshake exchange */
  CHECK(
      connloop_on_send(&cl, &(connloop_send_in){LEVEL_INITIAL, 1, 0, 20}) == 1);
  CHECK(connloop_on_recv(&sv, LEVEL_INITIAL, 20) == 1);
  CHECK(
      connloop_on_send(&sv, &(connloop_send_in){LEVEL_INITIAL, 1, 0, 20}) == 1);
  CHECK(
      connloop_on_send(&cl, &(connloop_send_in){LEVEL_HANDSHAKE, 1, 1, 20}) ==
      1);
  CHECK(connloop_on_recv(&sv, LEVEL_HANDSHAKE, 20) == 1);

  /* handshake completes on both ends, then app data over 1-RTT */
  cl.handshake_complete = 1;
  sv.handshake_complete = 1;
  CHECK(
      connloop_on_send(&cl, &(connloop_send_in){LEVEL_ONERTT, 1, 2, 20}) == 1);
  CHECK(cl.send_level == LEVEL_ONERTT);
  CHECK(connloop_on_recv(&sv, LEVEL_ONERTT, 20) == 1);

  /* client's in-flight drains to empty via ACKs (pn 0..2) */
  CHECK(sentpkt_count(&cl.sent) == 3);
  u64 r = 2;
  connloop_on_ack(&cl, &(connloop_ack_in){2, &r, 1});
  CHECK(sentpkt_count(&cl.sent) == 0);
...
static void test_auth_fail_reaches_limit_aesgcm(void) {
  connloop c;
  connloop_init(&c, 1);
  c.auth_fail_count = AEAD_INTEGRITY_LIMIT_AESGCM - 1;
  connloop_on_auth_fail(&c, 0);
  CHECK(c.auth_fail_count == AEAD_INTEGRITY_LIMIT_AESGCM);
...
static void test_auth_fail_reaches_limit_chacha(void) {
  connloop c;
  connloop_init(&c, 1);
  c.auth_fail_count = AEAD_INTEGRITY_LIMIT_CHACHA - 1;
  connloop_on_auth_fail(&c, 1);
  CHECK(c.aead_limit == 1);
}
