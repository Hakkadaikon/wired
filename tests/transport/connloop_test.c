...
static void test_send_level_never_regresses(void) {
  ...
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 0, 10}) ==
      1);
  ...
  CHECK(
      connloop_on_send(
          &c, &(connloop_send_in){QUIC_LEVEL_HANDSHAKE, 1, 1, 10}) == 1);
  CHECK(c.send_level == QUIC_LEVEL_HANDSHAKE);
  /* regress back to Initial: refused, level unchanged (before/after) */
  int hi = c.send_level;
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 2, 10}) ==
      0);
...
static void test_no_app_data_before_handshake_complete(void) {
  ...
  connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 0, 10});
  connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_HANDSHAKE, 1, 1, 10});

  /* handshake not complete: 1-RTT send refused */
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_ONERTT, 1, 2, 10}) ==
      0);
  CHECK(c.send_level == QUIC_LEVEL_HANDSHAKE);
...
static void test_server_send_capped_3x_recv(void) {
  ...
  connloop_on_recv(&c, QUIC_LEVEL_INITIAL, 100); /* budget = 300 */
  CHECK(c.recv_bytes == 100);
  /* exactly 3x allowed */
  CHECK(
      connloop_on_send(
          &c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 0, 300}) == 1);
  CHECK(c.sent_bytes == 300);
  /* one more byte exceeds 3x: refused, sent_bytes unchanged (before/after) */
  u64 before = c.sent_bytes;
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 1, 1}) ==
      0);
...
static void test_validate_lifts_3x_cap(void) {
  ...
  CHECK(
      connloop_on_send(
          &c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 1, 9999}) == 1);
}
...
static void test_recv_drops_packet_without_key(void) {
  ...
  CHECK(connloop_on_recv(&c, QUIC_LEVEL_HANDSHAKE, 50) == 0);
...
static void test_closed_processes_no_packet(void) {
  ...
  CHECK(connloop_on_recv(&c, QUIC_LEVEL_INITIAL, 50) == 0);
...
static void test_ack_removes_tracked_packet(void) {
  ...
  keyset_install(&c.keys, QUIC_LEVEL_INITIAL, &k);
  connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 0, 10});
  connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 1, 10});
...
static void test_pto_sends_probe_keeps_inflight(void) {
  ...
  CHECK(
      connloop_on_pto(&c, &(connloop_pto_in){QUIC_LEVEL_INITIAL, 0, 10}) == 0);
  CHECK(c.pto_armed == 0);
  ...
  CHECK(
      connloop_on_pto(&c, &(connloop_pto_in){QUIC_LEVEL_INITIAL, 1, 10}) == 1);
  CHECK(sentpkt_count(&c.sent) >= before);
...
static void test_closing_sends_no_app_data(void) {
  ...
  CHECK(
      connloop_on_send(&c, &(connloop_send_in){QUIC_LEVEL_ONERTT, 1, 0, 10}) ==
      0);
}
...
static void test_no_recv_at_discarded_level(void) {
  ...
  initial_keys k = {0};
  keyset_install(&c.keys, QUIC_LEVEL_INITIAL, &k);
  CHECK(connloop_on_recv(&c, QUIC_LEVEL_INITIAL, 10) == 1);
  keyset_discard(&c.keys, QUIC_LEVEL_INITIAL);
  /* after discard: processing at that level refused */
  CHECK(connloop_on_recv(&c, QUIC_LEVEL_INITIAL, 10) == 0);
}
...
static void test_handshake_progress_reaches_confirmed(void) {
  ...
  CHECK(
      connloop_on_send(
          &cl, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 0, 20}) == 1);
  CHECK(connloop_on_recv(&sv, QUIC_LEVEL_INITIAL, 20) == 1);
  CHECK(
      connloop_on_send(
          &sv, &(connloop_send_in){QUIC_LEVEL_INITIAL, 1, 0, 20}) == 1);
  CHECK(
      connloop_on_send(
          &cl, &(connloop_send_in){QUIC_LEVEL_HANDSHAKE, 1, 1, 20}) == 1);
  CHECK(connloop_on_recv(&sv, QUIC_LEVEL_HANDSHAKE, 20) == 1);

  /* handshake completes on both ends, then app data over 1-RTT */
  cl.handshake_complete = 1;
  sv.handshake_complete = 1;
  CHECK(
      connloop_on_send(&cl, &(connloop_send_in){QUIC_LEVEL_ONERTT, 1, 2, 20}) ==
      1);
  CHECK(cl.send_level == QUIC_LEVEL_ONERTT);
  CHECK(connloop_on_recv(&sv, QUIC_LEVEL_ONERTT, 20) == 1);
