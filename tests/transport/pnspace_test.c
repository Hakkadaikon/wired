...
static void test_pnspace_starts_at_zero(void) {
  pnspace s;
  pnspace_init(&s);
  CHECK(pnspace_next(&s, PNS_INITIAL) == 0);
  CHECK(pnspace_next(&s, PNS_HANDSHAKE) == 0);
  CHECK(pnspace_next(&s, PNS_APP) == 0);
}
...
static void test_pnspace_independent(void) {
  ...
  pnspace_init(&s);
  CHECK(pnspace_next(&s, PNS_INITIAL) == 0);
  CHECK(pnspace_next(&s, PNS_INITIAL) == 1);
  CHECK(pnspace_next(&s, PNS_INITIAL) == 2);
  /* Handshake untouched by Initial activity */
  CHECK(pnspace_next(&s, PNS_HANDSHAKE) == 0);
  CHECK(pnspace_next(&s, PNS_HANDSHAKE) == 1);
  /* App still independent */
  CHECK(pnspace_next(&s, PNS_APP) == 0);
  /* Initial resumes from where it left off */
  CHECK(pnspace_next(&s, PNS_INITIAL) == 3);
}
...
static void test_pnspace_exhausted_at_limit(void) {
  pnspace s;
  pnspace_init(&s);
  CHECK(pnspace_exhausted(&s, PNS_APP) == 0);
  s.next[PNS_APP] = PN_LIMIT; /* about to issue the last legal pn */
  CHECK(pnspace_exhausted(&s, PNS_APP) == 0);
  CHECK(pnspace_next(&s, PNS_APP) == PN_LIMIT);
  /* the space just issued 2^62-1, the last legal packet number */
  CHECK(pnspace_exhausted(&s, PNS_APP) == 1);
  /* other spaces are untouched */
  CHECK(pnspace_exhausted(&s, PNS_INITIAL) == 0);
}
