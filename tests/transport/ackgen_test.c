...
static void test_ackgen_generic(void) {
  ...
  CHECK(quic_ackgen_should_ack(&(quic_ackgen_state){0, 0, 100}, 25) == 0);

  /* first ack-eliciting packet: not due until max_ack_delay elapses */
  CHECK(
      quic_ackgen_should_ack(&(quic_ackgen_state){1, 0, 24}, 25) ==
      0); /* 24 < 25 */
  CHECK(
      quic_ackgen_should_ack(&(quic_ackgen_state){1, 0, 25}, 25) ==
      1); /* 25 >= 25 */

  /* second ack-eliciting packet while one is pending: immediate ack */
  CHECK(quic_ackgen_should_ack(&(quic_ackgen_state){1, 1, 0}, 25) == 1);

  /* a pending ack-eliciting packet, no new packet: delay still governs */
  CHECK(quic_ackgen_should_ack(&(quic_ackgen_state){0, 1, 24}, 25) == 0);
  CHECK(quic_ackgen_should_ack(&(quic_ackgen_state){0, 1, 25}, 25) == 1);
}
 ...
 * (quic_frame_ack_eliciting(QUIC_FK_DATAGRAM), the same predicate
 * quic_framedispatch_handle sets st->ack_eliciting from) rather than a
...
static void test_ackgen_datagram_only_packet_delayed(void) {
  int dgram_eliciting = quic_frame_ack_eliciting(QUIC_FK_DATAGRAM);
  CHECK(dgram_eliciting == 1);

  /* within max_ack_delay: not yet due */
  CHECK(
      quic_ackgen_should_ack(
          &(quic_ackgen_state){dgram_eliciting, 0, 24}, 25) == 0);
  /* max_ack_delay elapsed: due */
  CHECK(
      quic_ackgen_should_ack(
          &(quic_ackgen_state){dgram_eliciting, 0, 25}, 25) == 1);
}
