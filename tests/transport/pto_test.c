...
static void test_pto_backoff(void) {
  ...
  CHECK(
      quic_pto_backoff(QUIC_PTO_BACKOFF_MAX) ==
      ((u64)1 << QUIC_PTO_BACKOFF_MAX));
...
static void test_pto_backoff_scales(void) {
  u64 base = quic_pto_duration((quic_pto_rtt){100000, 10000}, 5000, 0);
  CHECK(quic_pto_duration((quic_pto_rtt){100000, 10000}, 5000, 3) == base * 8);
}
