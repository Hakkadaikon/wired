...
static void test_pto_var_dominates(void) {
  ...
  CHECK(
      pto_duration((pto_rtt){100000, 10000}, 5000, 0) == 100000 + 40000 + 5000);
}
...
static void test_pto_backoff_scales(void) {
  u64 base = pto_duration((pto_rtt){100000, 10000}, 5000, 0);
  CHECK(pto_duration((pto_rtt){100000, 10000}, 5000, 3) == base * 8);
}
