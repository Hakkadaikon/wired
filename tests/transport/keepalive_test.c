...
static void test_keepalive_interval_half(void) {
  CHECK(keepalive_interval(30000) == 15000);
  CHECK(keepalive_interval(1) == 0); /* integer division floor */
}
...
static void test_keepalive_due_boundary(void) {
  ...
  CHECK(!keepalive_due(0, 14999, 30000)); /* just before half */
  CHECK(keepalive_due(0, 15000, 30000));  /* exactly half */
  CHECK(keepalive_due(0, 20000, 30000));  /* past half */
}

/* Boundary is measured from last activity, not absolute time. */
static void test_keepalive_due_from_last_activity(void) {
  CHECK(!keepalive_due(10000, 24999, 30000));
  CHECK(keepalive_due(10000, 25000, 30000));
}
