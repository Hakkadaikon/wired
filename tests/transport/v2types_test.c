...
static void test_v2_wire_values(void) {
  CHECK(v2_packet_type(QUIC_LT_INITIAL) == 1);
  CHECK(v2_packet_type(QUIC_LT_0RTT) == 2);
  CHECK(v2_packet_type(QUIC_LT_HANDSHAKE) == 3);
  CHECK(v2_packet_type(QUIC_LT_RETRY) == 0);
}

/* v1 wire values equal the logical ordering (RFC 9000 17.2). */
static void test_v1_wire_values(void) {
  CHECK(v1_packet_type(QUIC_LT_INITIAL) == 0);
  CHECK(v1_packet_type(QUIC_LT_0RTT) == 1);
  CHECK(v1_packet_type(QUIC_LT_HANDSHAKE) == 2);
  CHECK(v1_packet_type(QUIC_LT_RETRY) == 3);
}
...
static void test_invalid(void) {
  CHECK(v2_packet_type(QUIC_LT_INVALID) == -1);
  CHECK(v1_packet_type((logical_type)4) == -1);
  CHECK(v2_logical_type(4) == QUIC_LT_INVALID);
  CHECK(v1_logical_type(-1) == QUIC_LT_INVALID);
}
