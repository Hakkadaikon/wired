...
static void test_lhdr_initial(void) {
  ...
  usz       tkl, pn_off;
  u64       length;
  int       ok = quic_lhdr_parse(
      p, n, 1, &dcid, &dcl, &scid, &scl, &token, &tkl, &length, &pn_off);
  CHECK(ok == 1);
...
static void test_lhdr_handshake(void) {
  ...
  u64       length;
  int       ok = quic_lhdr_parse(
      p, n, 0, &dcid, &dcl, &scid, &scl, &token, &tkl, &length, &pn_off);
  CHECK(ok == 1);
...
static void test_lhdr_not_long(void) {
  ...
  CHECK(
      quic_lhdr_parse(
          p, 8, 1, &dcid, &dcl, &scid, &scl, &token, &tkl, &length, &pn_off) ==
      0);
}
