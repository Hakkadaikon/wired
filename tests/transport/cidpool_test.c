...
static void test_cidpool_limit(void) {
  ...
  cidpool_init(&p, 2);
  u64 seq;
  CHECK(cidpool_issue(&p, &seq) == 1 && seq == 0);
  CHECK(cidpool_issue(&p, &seq) == 1 && seq == 1);
  CHECK(cidpool_active_count(&p) == 2);
  /* at the limit: the next issue is refused */
  CHECK(cidpool_issue(&p, &seq) == 0);
  /* retire seq 0, freeing one slot */
  CHECK(cidpool_retire_prior_to(&p, 1) == 1);
  CHECK(cidpool_active_count(&p) == 1);
  CHECK(cidpool_issue(&p, &seq) == 1 && seq == 2);
  CHECK(cidpool_active_count(&p) == 2);
}
...
static void test_cidpool_retire(void) {
  ...
  cidpool_init(&p, 5);
  u64 seq;
  for (int i = 0; i < 3; i++) cidpool_issue(&p, &seq); /* seqs 0,1,2 */
  /* retiring past next_seq (3) is a protocol violation */
  CHECK(cidpool_retire_prior_to(&p, 4) == 0);
  /* retire_prior_to exactly next_seq retires all issued so far */
  CHECK(cidpool_retire_prior_to(&p, 3) == 1);
  CHECK(cidpool_active_count(&p) == 0);
