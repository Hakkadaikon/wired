...
static void test_coalorder_long_any(void) {
  CHECK(coalesce_short_must_be_last(0xC0, 0) == 1); /* not last, ok */
  CHECK(coalesce_short_must_be_last(0xC0, 1) == 1); /* last, ok */
}

/* A short header (high bit clear) is only allowed as the last packet. */
static void test_coalorder_short_last_only(void) {
  CHECK(coalesce_short_must_be_last(0x40, 1) == 1); /* last, ok */
  CHECK(coalesce_short_must_be_last(0x40, 0) == 0); /* not last, bad */
}
