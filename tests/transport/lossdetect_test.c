...
static void test_loss_by_packet_boundary(void) {
  CHECK(loss_by_packet(10, 7) == 1);  /* gap 3 */
  CHECK(loss_by_packet(10, 8) == 0);  /* gap 2 */
  CHECK(loss_by_packet(10, 10) == 0); /* same packet */
  CHECK(loss_by_packet(3, 0) == 1);   /* gap 3 from zero */
}
...
static void test_loss_by_time_boundary(void) {
  ...
  CHECK(
      loss_by_time((loss_when){9000, 0}, 8000, 8000) ==
      1); /* exactly at threshold */
  CHECK(loss_by_time((loss_when){8999, 0}, 8000, 8000) == 0); /* just under */
}

/* Uses the larger of srtt and latest_rtt. */
static void test_loss_by_time_uses_max(void) {
  /* max(4000, 8000)=8000 -> threshold 9000 */
  CHECK(loss_by_time((loss_when){9000, 0}, 4000, 8000) == 1);
  CHECK(loss_by_time((loss_when){9000, 0}, 8000, 4000) == 1);
}

/* now before sent_time: elapsed is zero, not lost (threshold > 0). */
static void test_loss_by_time_no_underflow(void) {
  CHECK(loss_by_time((loss_when){100, 5000}, 8000, 8000) == 0);
}
