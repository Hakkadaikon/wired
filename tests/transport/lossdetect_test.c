...
static void test_loss_by_time_uses_max(void) {
  ...
  CHECK(quic_loss_by_time((quic_loss_when){9000, 0}, 4000, 8000) == 1);
  CHECK(quic_loss_by_time((quic_loss_when){9000, 0}, 8000, 4000) == 1);
}

/* now before sent_time: elapsed is zero, not lost (threshold > 0). */
static void test_loss_by_time_no_underflow(void) {
  CHECK(quic_loss_by_time((quic_loss_when){100, 5000}, 8000, 8000) == 0);
}
