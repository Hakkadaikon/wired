...
static void test_draining_period_is_3pto(void) {
  CHECK(draining_period(0) == 0);
  CHECK(draining_period(1) == 3);
  CHECK(draining_period(100) == 300);
}
...
static void test_draining_done_boundary(void) {
  ...
  CHECK(draining_done(1000, 1299, 100) == 0); /* one tick early */
  CHECK(draining_done(1000, 1300, 100) == 1); /* exactly at limit */
  CHECK(draining_done(1000, 1301, 100) == 1); /* past limit */
  CHECK(draining_done(1000, 1000, 100) == 0); /* just entered */
}

/* Sending is forbidden during draining, allowed otherwise. */
static void test_draining_may_send(void) {
  CHECK(draining_may_send(1) == 0); /* in draining: cannot send */
  CHECK(draining_may_send(0) == 1); /* not draining: may send */
}
