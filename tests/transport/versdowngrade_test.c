...
void test_versdowngrade(void) {
  ...
  u32 av[] = {VERSION_2, VERSION_1};
  CHECK(vers_no_downgrade(VERSION_2, av, 2) == 1);

  /* negotiated v1 while v2 (usable) precedes it -> downgrade */
  CHECK(vers_no_downgrade(VERSION_1, av, 2) == 0);

  /* GREASE before negotiated is ignored -> still ok */
  u32 grease[] = {0x0a0a0a0au, VERSION_1};
  CHECK(vers_no_downgrade(VERSION_1, grease, 2) == 1);

  /* negotiated absent from server's list -> downgrade */
  u32 only_v2[] = {VERSION_2};
  CHECK(vers_no_downgrade(VERSION_1, only_v2, 1) == 0);
}
