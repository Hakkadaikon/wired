...
static void test_loss_delay_nine_eighths(void) {
  ...
  CHECK(lossdrive_loss_delay(8000, 8000, 1000) == 9000);
  /* uses the larger rtt: max(4000,8000)=8000 -> 9000 */
  CHECK(lossdrive_loss_delay(4000, 8000, 1000) == 9000);
}
...
static void test_loss_delay_granularity_floor(void) {
  ...
  CHECK(lossdrive_loss_delay(0, 0, 1000) == 1000);
  /* 500*9/8 = 562 < 1000 -> 1000 */
  CHECK(lossdrive_loss_delay(500, 500, 1000) == 1000);
}
