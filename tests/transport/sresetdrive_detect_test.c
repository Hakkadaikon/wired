...
void test_sresetdrive_detect(void) {
  ...
  CHECK(sresetdrive_is_reset(pkt, QUIC_SRESETDRIVE_MIN, token) == 1);

  pkt[QUIC_SRESETDRIVE_MIN - 1] ^= 0x01; /* corrupt last token byte */
  CHECK(sresetdrive_is_reset(pkt, QUIC_SRESETDRIVE_MIN, token) == 0);

  /* one byte under the 21-byte minimum is never a reset */
  CHECK(sresetdrive_is_reset(pkt, QUIC_SRESETDRIVE_MIN - 1, token) == 0);
}
