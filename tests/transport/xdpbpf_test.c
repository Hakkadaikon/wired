...
/* Golden 41-instruction program for map_fd=5, port=4433 (0x1151, byte-swapped
 ...
 * from the original filter except every jump now targets idx39, the
 * relocated XDP_PASS trap; idx15-31 are the core-routing block, unchanged
 * from the previous revision; idx32 is the rx_queue_index fallback key load;
 * idx33 is the new DCID-byte range check (JGE_IMM r2,64 -> fallback, closing
 * the map-miss-drops-to-XDP_PASS handshake bug); idx34-40 are the shared
 * redirect-map epilogue + XDP_PASS trap, each one slot later than before). */
static const u64 xbt_golden[XDPBPF_PROG_LEN] = {
    0x0000000000001261ULL, 0x0000000000041361ULL, 0x00000000000025bfULL,
    0x0000002a00000507ULL, 0x000000000022352dULL, 0x00000000000c2469ULL,
    0x0000000800200455ULL, 0x00000000000e2471ULL, 0x00000045001e0455ULL,
    0x0000000000142469ULL, 0x0000ff3f001c0445ULL, 0x0000000000172471ULL,
    0x00000011001a0455ULL, 0x0000000000242469ULL, 0x0000511100180455ULL,
    ...
    0x00000040fffe0235ULL, 0x0000000500001118ULL, 0x0000000000000000ULL,
 ...
 * including the byte-swapped port at idx14 and the map fd at idx34. */
 ...
 * idx34 (the map-fd LDDW); every other instruction is byte-identical to the
 * golden program. */
...
static void test_xdpbpf_core_routing_branch_offsets_correct(void) {
  ...
  xbt_check_branch(prog, 4, 39);
  xbt_check_branch(prog, 6, 39);
  xbt_check_branch(prog, 8, 39);
  xbt_check_branch(prog, 10, 39);
  xbt_check_branch(prog, 12, 39);
  xbt_check_branch(prog, 14, 39);
   ...
   * through to read the DCID byte then jumps to the key-range check(33). */
  xbt_check_branch(prog, 21, 24);
  xbt_check_branch(prog, 23, 33);

  /* long path: boundary-miss -> fallback(32); dcid_len != 0 -> has_dcid(30);
   * dcid_len == 0 (fallthrough) -> fallback(32); has_dcid -> the key-range
   * check(33). */
  ...
  /* key-range check at idx33: key >= 64 -> fallback(32); key < 64
   * (fallthrough) -> redirect(34). */
  xbt_check_branch(prog, 33, 32);
}
 ...
 * idx32) is never the register the map-fd LDDW writes (r1, idx34) nor the
...
static void test_xdpbpf_core_routing_no_register_clobber(void) {
  ...
  map_fd_load   = xbt_decode(prog[34]); /* LDDW r1,map_fd */
  redirect_call = xbt_decode(prog[37]); /* CALL bpf_redirect_map */
 ...
 * test above) to land on idx33 with that byte in r2. This test pins idx33
...
static void test_xdpbpf_short_and_long_paths_share_key_check(void) {
  u64 prog[XDPBPF_PROG_LEN];
  CHECK(xdpbpf_prog_build(prog, 5, 4433) == XDPBPF_PROG_LEN);
  xbt_check_branch(prog, 23, 33); /* short path's JA */
  xbt_check_branch(prog, 31, 33); /* long path's JA */
}
