...
static void test_inittoken_roundtrip(void) {
  ...
  u8         buf[32];
  wired_span out;
  usz w = quic_inittoken_put(buf, sizeof(buf), wired_span_of(tok, sizeof(tok)));
...
static void test_inittoken_empty(void) {
  u8         buf[8];
  wired_span out;
  usz w = quic_inittoken_put(buf, sizeof(buf), wired_span_of((const u8*)0, 0));
  usz r = quic_inittoken_get(buf, w, &out);
...
static void test_inittoken_bounds(void) {
  ...
  CHECK(quic_inittoken_put(buf, 2, wired_span_of(tok, sizeof(tok))) == 0);
}
