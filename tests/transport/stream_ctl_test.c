...
static void test_reset_stream(void) {
  ...
  CHECK(reset_stream_decode(buf, w - 1, &out) == 0);
}
...
static void test_stop_sending(void) {
  ...
  CHECK(stop_sending_decode(buf, w - 1, &out) == 0);
}
...
static void test_reset_stream_at(void) {
  ...
  CHECK(reset_stream_at_decode(buf, w - 1, &out) == 0);

  /* boundary: reliable_size == final_size */
  in.reliable_size = in.final_size;
  w                = reset_stream_at_encode(buf, sizeof(buf), &in);
  CHECK(w != 0);
  CHECK(reset_stream_at_decode(buf, w, &out) == w);
  ...
  CHECK(reset_stream_at_encode(buf, sizeof(buf), &in) == 0);
