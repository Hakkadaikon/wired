...
static void test_retrydrive_accept(void) {
  CHECK(retrydrive_accept(0, 1) == 1); /* first + valid tag */
  CHECK(retrydrive_accept(1, 1) == 0); /* second Retry */
  CHECK(retrydrive_accept(0, 0) == 0); /* invalid tag */
  CHECK(retrydrive_accept(1, 0) == 0); /* second + invalid */
}
...
static void test_retrydrive_initial_token(void) {
  ...
  retrydrive_initial_token(&s, &t, &len);
