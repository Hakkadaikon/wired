...
static void test_retry_reencode_on_change(void) {
  CHECK(version_retry_reencode(VERSION_1, VERSION_2) == 1);
  CHECK(version_retry_reencode(VERSION_2, VERSION_1) == 1);
}

/* No change of version: tag stays valid. */
static void test_retry_no_reencode_same(void) {
  CHECK(version_retry_reencode(VERSION_1, VERSION_1) == 0);
...
static void test_0rtt_keep_compatible(void) {
  CHECK(version_0rtt_keep(VERSION_1, VERSION_2) == 1);
  CHECK(version_0rtt_keep(VERSION_1, VERSION_1) == 1);
}

/* Incompatible switch (unknown version) discards 0-RTT keys. */
static void test_0rtt_drop_incompatible(void) {
  CHECK(version_0rtt_keep(VERSION_1, 0xdeadbeefu) == 0);
