...
static void test_pto_no_backoff(void) {
  ...
  CHECK(
      quic_lossdrive_pto(
          (quic_pto_rtt){10000, 1000},
          &(quic_lossdrive_ptoctx){2000, 0, 1000}) == 16000);
}
...
static void test_pto_backoff_four(void) {
  CHECK(
      quic_lossdrive_pto(
          (quic_pto_rtt){10000, 1000},
          &(quic_lossdrive_ptoctx){2000, 2, 1000}) == 64000);
}
...
static void ptobackoff_granularity_floor(void) {
  ...
  CHECK(
      quic_lossdrive_pto(
          (quic_pto_rtt){5000, 0}, &(quic_lossdrive_ptoctx){0, 0, 1000}) ==
      6000);
}
