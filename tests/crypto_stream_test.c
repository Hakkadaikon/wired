...
static void test_emit_splits(void)
{
    ...
    u8 out[128];
    usz out_len = 0;
    CHECK(quic_crypto_stream_emit(src, sizeof src, 100, 8, out,
    ...
    while (pos < out_len) {
        ...
        for (u64 i = 0; i < f.length; i++)
            CHECK(f.data[i] == src[total + i]);
        expect_off += f.length;
        total += f.length;
        pos += n;
        frames++;
...
static void test_ecdhe_symmetric(void)
{
    ...
    for (usz i = 0; i < 32; i++) { cpriv[i] = (u8)(i + 1); spriv[i] = (u8)(64 - i); }
...
static void test_clienthello_roundtrip(void)
{
    ...
    for (usz i = 0; i < 32; i++) { random[i] = (u8)i; priv[i] = (u8)(i * 3 + 1); }
    quic_x25519_base(pub, priv);

    u8 ch[1024];
    usz ch_len = quic_tls_client_hello(ch, sizeof ch, random, pub,
                                       (const u8 *)"example.com", 11, tp, sizeof tp);
    CHECK(ch_len != 0);

    u8 frames[2048];
    usz flen = 0;
    CHECK(quic_crypto_stream_emit(ch, ch_len, 0, 40, frames,
                                  sizeof frames, &flen) == 1);
    ...
    while (pos < flen) {
        ...
        CHECK(n != 0);
        offs[nf] = (usz)f.offset; lens[nf] = (usz)f.length; datp[nf] = f.data;
        pos += n; nf++;
    }
    for (usz i = nf; i-- > 0;)
