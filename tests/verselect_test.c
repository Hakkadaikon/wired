...
{
    quic_version_information vi = {QUIC_VERSION_1, 2,
                                  {QUIC_VERSION_1, QUIC_VERSION_2}};
    u32 we[] = {QUIC_VERSION_2, QUIC_VERSION_1}; /* prefer v2 */
    u32 out = 0;
    CHECK(quic_verinfo_pick_compatible(&vi, we, 2, &out) == 1);
...
static void test_verselect_none(void)
{
    ...
    u32 we[] = {0xdeadbeefu};
    u32 out = 0xffffffffu;
    CHECK(quic_verinfo_pick_compatible(&vi, we, 1, &out) == 0);
