...
static void test_der_short_form(void) {
  ...
  quic_der_tlv t;
  CHECK(quic_der_read(wired_span_of(in, sizeof(in)), &t) == 1);
  CHECK(
...
static void test_der_truncated(void) {
  ...
  CHECK(quic_der_read(wired_span_of(a, sizeof(a)), &t) == 0);
  /* too short for any TLV */
  const u8 b[] = {0x02};
  CHECK(quic_der_read(wired_span_of(b, 1), &t) == 0);
  /* long form 0x82 announced but length octets missing */
  const u8 c[] = {0x04, 0x82, 0x01};
  CHECK(quic_der_read(wired_span_of(c, sizeof(c)), &t) == 0);
  /* unsupported indefinite/reserved length octet 0x80 */
  const u8 d[] = {0x04, 0x80, 0x00};
  CHECK(quic_der_read(wired_span_of(d, sizeof(d)), &t) == 0);
}
