...
static void p256field_n_off(p256_fe r, i64 delta) {
  quic_fp_set(r, quic_p256_n);
  r[0] = (u64)((i64)r[0] + delta);
}

/* quic_fp_reduce_n agrees with the generic long-division reducer on the
...
static void test_p256field_n_reduce_matches_generic(void) {
  ...
  quic_fp_set(cases[3], quic_p256_n);
  p256field_n_off(cases[4], 1);
  for (usz i = 0; i < 4; i++) cases[5][i] = ~(u64)0;
  for (usz i = 0; i < 6; i++) {
    p256_fe fast, slow;
    quic_fp_reduce_n(fast, cases[i]);
    quic_fp_reduce(slow, cases[i], quic_p256_n);
    CHECK(quic_fp_eq(fast, slow));
  }
  for (usz i = 0; i < 200; i++) {
    p256_fe a, fast, slow;
    p256field_n_rand(a);
    quic_fp_reduce_n(fast, a);
    quic_fp_reduce(slow, a, quic_p256_n);
    CHECK(quic_fp_eq(fast, slow));
  }
}

/* quic_fp_mul_n agrees with the generic modular multiply for operands < n:
...
static void test_p256field_n_mul_matches_generic(void) {
  ...
  for (usz i = 0; i < 3; i++)
    for (usz j = 0; j < 3; j++) {
      p256_fe fast, slow;
      quic_fp_mul_n(fast, (quic_fpab){bounds[i], bounds[j]});
      quic_fp_mul(slow, (quic_fpab){bounds[i], bounds[j]}, quic_p256_n);
      CHECK(quic_fp_eq(fast, slow));
    }
  for (usz i = 0; i < 200; i++) {
    ...
    p256field_n_rand(a);
    p256field_n_rand(b);
    quic_fp_reduce_n(a, a);
    quic_fp_reduce_n(b, b);
    quic_fp_mul_n(fast, (quic_fpab){a, b});
    quic_fp_mul(slow, (quic_fpab){a, b}, quic_p256_n);
    CHECK(quic_fp_eq(fast, slow));
  }
}
