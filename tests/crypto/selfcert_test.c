...
static void test_der_tlv_roundtrip(void) {
  ...
  CHECK(
      quic_selfcert_der_tlv(QUIC_DER_INTEGER, v, 2, out, sizeof(out), &n) == 1);
...
static void test_der_tlv_longform(void) {
  ...
  CHECK(
      quic_selfcert_der_tlv(
          QUIC_DER_OCTET_STRING, big, 200, out, sizeof(out), &n) == 1);
...
static void test_der_tlv_overflow(void) {
  ...
  const u8 v[] = {1, 2, 3, 4};
  CHECK(
      quic_selfcert_der_tlv(QUIC_DER_INTEGER, v, 4, out, sizeof(out), &n) == 0);
}
