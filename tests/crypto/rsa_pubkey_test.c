...
static void test_rsa_pubkey_extract(void) {
  ...
  CHECK(quic_x509_rsa_pubkey(key, klen, &n, &n_len, &e, &e_len) == 1);
  CHECK(n_len == 256 && n[0] == 0xc1 && n[255] == 0x01); /* pad stripped */
...
static void test_rsa_pubkey_min_n(void) {
  u8        nv[257], key[600];
  const u8 *n, *e;
  usz       n_len, e_len, klen;
  klen = rpk_key(key, nv, rpk_padded_n(nv, 255), rpk_e_f4, sizeof(rpk_e_f4));
  CHECK(quic_x509_rsa_pubkey(key, klen, &n, &n_len, &e, &e_len) == 0);
