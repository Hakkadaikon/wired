...
static void test_p384_field_fast_matches_generic(void) {
  ...
  p384_fe              pm1, a, b;
  fp384_sub(pm1, (fp384ab){zero, one}, p384_p); /* p - 1 */
...
static void test_p384_field_addsub(void) {
  ...
  pf_rand(a);
  pf_rand(b);
  fp384_add(s, (fp384ab){a, b}, p384_p);
  fp384_sub(d, (fp384ab){s, b}, p384_p);
  CHECK(fp384_eq(d, a) == 1);
  fp384_sqr_p(sq, a);
  fp384_mul_p(mm, a, a);
  CHECK(fp384_eq(sq, mm) == 1);
}
...
static void test_p384_field_inv(void) {
  ...
  pf_rand(a);
  fp384_inv_p(inv, a);
  fp384_mul_p(prod, a, inv);
  CHECK(fp384_eq(prod, one) == 1);
}
...
static void test_p384_field_mont_inv_n(void) {
  ...
  static const p384_fe one = {1, 0, 0, 0, 0, 0};
  for (usz i = 0; i < 6; i++) t[i] = pf_rng();
  fp384_reduce(a, t, p384_n);
  mont384_inv(inv, a, &p384_mont_n);
  fp384_mul(prod, (fp384ab){a, inv}, p384_n);
  CHECK(fp384_eq(prod, one) == 1);
}
