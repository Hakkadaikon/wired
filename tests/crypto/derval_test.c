...
static void test_derval_oid_equal(void) {
  ...
  wired_span e     = wired_span_of(exp, sizeof(exp));
  CHECK(der_oid_equal(wired_span_of(rsa, sizeof(rsa)), e) == 1);
  /* differing last byte */
  const u8 other[] = {0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x0b};
  CHECK(der_oid_equal(wired_span_of(other, sizeof(other)), e) == 0);
  /* length mismatch */
  CHECK(der_oid_equal(wired_span_of(rsa, sizeof(rsa) - 1), e) == 0);
}

static void test_derval_uint(void) {
  u64      out;
  const u8 one[] = {0x01};
  CHECK(der_uint(one, 1, &out) == 1 && out == 1);
  /* 0x0100 = 256 */
  const u8 t256[] = {0x01, 0x00};
  CHECK(der_uint(t256, 2, &out) == 1 && out == 256);
  /* leading 0x00 pad keeps 0x80 positive: value 128 */
  const u8 p128[] = {0x00, 0x80};
  CHECK(der_uint(p128, 2, &out) == 1 && out == 128);
  /* full 8 octets */
  const u8 max8[] = {0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
  CHECK(der_uint(max8, 8, &out) == 1 && out == 0x7fffffffffffffffULL);
  /* 9 significant octets after pad strip -> too wide */
  const u8 wide[] = {0x00, 0x01, 0, 0, 0, 0, 0, 0, 0, 0};
  CHECK(der_uint(wide, sizeof(wide), &out) == 0);
}
...
static void test_derval_uint_reject(void) {
  ...
  const u8 neg[] = {0x80};
  CHECK(der_uint(neg, 1, &out) == 0);
  /* empty value */
  CHECK(der_uint(neg, 0, &out) == 0);
}
