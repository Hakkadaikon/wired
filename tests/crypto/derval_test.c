...
static void test_derval_oid_equal(void) {
  ...
  CHECK(quic_der_oid_equal(wired_span_of(rsa, sizeof(rsa)), e) == 1);
  /* differing last byte */
  const u8 other[] = {0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x0b};
  CHECK(quic_der_oid_equal(wired_span_of(other, sizeof(other)), e) == 0);
  /* length mismatch */
  CHECK(quic_der_oid_equal(wired_span_of(rsa, sizeof(rsa) - 1), e) == 0);
}
