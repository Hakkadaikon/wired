...
static void test_x509_parse_golden(void) {
  ...
  CHECK(c.tbs == quic_x509_golden + 4 && c.tbs_len == 305);
  /* signatureAlgorithm OID is ecdsa-with-SHA256. */
  CHECK(
      quic_der_oid_equal(
          c.sig_alg_oid, c.sig_alg_len, quic_oid_ecdsa_sha256,
          sizeof(quic_oid_ecdsa_sha256)) == 1);
  /* signatureValue BIT STRING value is 71 octets (at offset 323). */
  CHECK(c.sig == quic_x509_golden + 323 && c.sig_len == 71);
}

static void test_x509_truncated(void) {
  quic_x509 c;
  CHECK(quic_x509_parse(quic_x509_golden, 10, &c) == 0);
  CHECK(quic_x509_parse(quic_x509_golden, 0, &c) == 0);
}
...
static void test_x509_not_tbs_seq(void) {
  ...
  quic_x509 c;
  CHECK(quic_x509_parse(bad, sizeof(bad), &c) == 0);
}
