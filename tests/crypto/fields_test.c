...
static void test_fields_match_parts(void) {
  quic_tbscert t;
  wired_span   tbs = wired_span_of(quic_x509_golden + 4, 305);
...
static void test_fields_truncated(void) {
  ...
  CHECK(quic_tbscert_parse(wired_span_of(quic_x509_golden + 4, 3), &t) == 0);
}
...
static void test_fields_short_tbs(void) {
  ...
  quic_tbscert t;
  CHECK(quic_tbscert_parse(wired_span_of(tbs, sizeof(tbs)), &t) == 0);
}
