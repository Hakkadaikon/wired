...
static int pc_sig_rs(wired_span der, u8 r[32], u8 s[32]) {
  ...
  derseq     c;
  if (!der_seq(der, &seq)) return 0;
  derseq_init(&c, seq);
  if (!derseq_next(&c, &tag, &rv)) return 0;
  if (!derseq_next(&c, &tag, &sv)) return 0;
  pc_be32(r, rv);
  pc_be32(s, sv);
  return 1;
}
...
static int pc_split_spki(wired_span spki, wired_span* oid, wired_span* key) {
  ...
  wired_span seq, alg;
  derseq     c, a;
  if (!der_seq(spki, &seq)) return 0;
  derseq_init(&c, seq);
  if (!derseq_next(&c, &tag, &alg)) return 0;
  if (!derseq_next(&c, &tag, key)) return 0;
  derseq_init(&a, alg);
  return derseq_next(&a, &tag, oid);
}
...
static void test_spki_roundtrip(void) {
  ...
  u8         spki[128];
  wired_obuf o = obuf_of(spki, sizeof(spki));
  CHECK(p256cert_spki(x, y, &o) == 1);
  ...
  u8 rx[32], ry[32];
  CHECK(x509_ec_pubkey(key, rx, ry) == 1);
...
static void test_p256cert_parse(void) {
  ...
  x509 c;
  CHECK(x509_parse(wired_span_of(cert, clen), &c) == 1);
...
static void test_cert_selfsigned(void) {
  ...
  CHECK(ecdsa_p256_verify(x, y, r, s, hash) == 1);

  /* A flipped TBS byte must break verification. */
  u8 saved = c.tbs.p[0];
  ((u8*)c.tbs.p)[0] ^= 0xff;
  wired_sha256(c.tbs.p, c.tbs.n, hash);
  CHECK(ecdsa_p256_verify(x, y, r, s, hash) == 0);
  ((u8*)c.tbs.p)[0] = saved;
}
...
static void test_cert_san_localhost(void) {
  ...
  CHECK(x509_san_matches(c.tbs, wired_span_of((const u8*)"localhost", 9)) == 1);
  CHECK(
      x509_san_matches(c.tbs, wired_span_of((const u8*)"example.com", 11)) ==
      0);
}
 ...
 * x509_san_matches only checks dNSName entries, not iPAddress. */
...
static void test_cert_san_ipv4_omitted(void) {
  ...
  x509       c;
  wired_span san;
  CHECK(x509_parse(wired_span_of(cert, o.len), &c) == 1);
  CHECK(x509_find_ext(c.tbs, wired_span_of(pct_oid_san, 3), &san) == 1);
  {
...
static void test_cert_now_secs_14day_window(void) {
  ...
  /* x509_validity_ok takes YYYYMMDDHHMMSS, not raw epoch seconds --
   * round every probe through the same converter the cert builder used. */
  {
    u64 nb   = clock_epoch_to_ymdhms(nb_secs);
    u64 na   = clock_epoch_to_ymdhms(nb_secs + fourteen_d);
    u64 nb_m = clock_epoch_to_ymdhms(nb_secs - 1);
    u64 na_p = clock_epoch_to_ymdhms(nb_secs + fourteen_d + 1);
    /* now itself and both inclusive edges are inside the window */
    CHECK(x509_validity_ok(c.tbs, clock_epoch_to_ymdhms(now)) == 1);
    CHECK(x509_validity_ok(c.tbs, nb) == 1);
    CHECK(x509_validity_ok(c.tbs, na) == 1);
    /* one second outside either edge */
    CHECK(x509_validity_ok(c.tbs, nb_m) == 0);
    CHECK(x509_validity_ok(c.tbs, na_p) == 0);
  }
  /* the fixed 2020-2030 window (no now_secs) would have accepted this --
   * confirms the window actually narrowed, not just shifted. */
  CHECK(x509_validity_ok(c.tbs, 20280101000000ULL) == 0);
}
 ...
 * a zero-initialized p256cert_key. */
static void test_cert_now_secs_zero_keeps_fixed_window(void) {
  ...
  x509 c;
  CHECK(x509_parse(wired_span_of(cert, o.len), &c) == 1);
  CHECK(x509_validity_ok(c.tbs, 20200101000000ULL) == 1);
  CHECK(x509_validity_ok(c.tbs, 20300101000000ULL) == 1);
}
