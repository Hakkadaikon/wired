...
static void test_hs_message_short(void)
{
    ...
    CHECK(quic_hs_message_ready((const u8 *)"\x01\x00\x00\x03" "ab", 6, &n) == 0);
...
{
    usz n = 0;
    CHECK(quic_hs_message_ready((const u8 *)"\x01\x00\x00\x03" "abc", 7, &n) == 1);
    CHECK(n == 7);
}
...
{
    usz n = 0;
    CHECK(quic_hs_message_ready(
        (const u8 *)"\x01\x00\x00\x03" "abc" "\x02\x00\x00\x00", 11, &n) == 1);
    CHECK(n == 7);
}
