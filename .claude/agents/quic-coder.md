...
1. **libc-free.** No standard headers (`<stdio.h>`, `<string.h>`, `<stdlib.h>`, `<stdint.h>`, etc.). The only allowed includes are `sys/syscall.h` and the repo's `util/*.h`. Builds run with `-ffreestanding -nostdlib -fno-builtin -Werror`. Use the repo's own fixed-width types and helpers from `src/util/` (`be.h`, `bytes.h`, `ct.h`, `num.h`). Never write inline asm, ISA builtins (`__asm__`, `__builtin_ia32_*`), or raw syscalls (`syscallN(SYS_*)`) in a domain file — those live only in the arch adapter `src/common/arch/` behind `wired_arch_*` names; call the typed wrappers from `common/arch/sysops.h`.
...
4. **Prefix encodes the audience.** Application-facing API (Stable table in `docs/api-stability.md`, or anything the examples call) is `wired_<rest>`; internal non-static API carries its module token (`sdrv_*`, `moqctl_*`, `bn_*`). MECE splits responsibility, not the global namespace — all public symbols link into one binary and must be globally unique. Before naming a function, `grep -rn '<module>_<name>' src/` to confirm no existing collision (historically `quic_sent_*` collided between sentpkt and recovery; `quic_h3_control_open` between h3run and h3 — distinct module tokens are the fix and now the rule). Prevent it at naming time, not at wiring time.
...
6. **Adding a struct member? Sweep every constructor.** `grep -rn '<struct name>'` and update every initialization site (tests and examples included) in the same change — prefer designated initializers / `= {0}` so new members start zeroed. The same "new field left uninitialized at one call site" fix was made twice within an hour once; freestanding code has no implicit zeroing to save you.
