...
typedef struct { u8 *buf; usz cap; usz off; int ok; } quic_p256cert_enc;

/* Append one TLV at the cursor, advancing off. Latches ok=0 on overflow. */
static inline void quic_p256cert_put(quic_p256cert_enc *e, u8 tag,
                                     const u8 *val, usz len)
{
    usz n;
    if (e->ok && quic_selfcert_der_tlv(tag, val, len, e->buf + e->off,
                                       e->cap - e->off, &n))
        e->off += n;
    else
        e->ok = 0;
...
static inline void quic_p256cert_put_pre(quic_p256cert_enc *e, const u8 *tlv,
                                         usz n)
{
...
static inline usz quic_p256cert_wrap(quic_p256cert_enc *e, u8 tag, u8 *out,
                                     usz cap)
{
    usz n;
