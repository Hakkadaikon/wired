...
static usz pc_build_gennames(u8 *out, usz cap)
{
    u8 inner[32];
    quic_p256cert_enc e = {inner, sizeof(inner), 0, 1};
    quic_p256cert_put(&e, PC_SAN_DNSNAME_TAG, pc_cn_value,
                      sizeof(pc_cn_value) - 1);
    return quic_p256cert_wrap(&e, QUIC_DER_SEQUENCE, out, cap);
}
...
{
    u8 gn[48], ext[64];
    quic_p256cert_enc eg = quic_p256cert_loaded(gn, pc_build_gennames(gn,
                                                                sizeof(gn)));
...
{
    u8 ext[64], seq[80];
    quic_p256cert_enc ee = quic_p256cert_loaded(ext, pc_build_san_ext(ext,
                                                              sizeof(ext)));
    quic_p256cert_enc es = quic_p256cert_loaded(seq, quic_p256cert_wrap(&ee,
                              QUIC_DER_SEQUENCE, seq, sizeof(seq)));
