...
void quic_hkdf_extract(const u8 *salt, usz salt_len,
                       const u8 *ikm, usz ikm_len,
 ...
 * (prev_len = 0 for T(1)). */
static void expand_block(const u8 *prk, const u8 *prev, usz prev_len,
                         const u8 *info, usz info_len, u8 counter,
...
static usz build_label(u8 *info, const char *label, usz label_len,
                       const u8 *ctx, usz ctx_len, usz len)
{
