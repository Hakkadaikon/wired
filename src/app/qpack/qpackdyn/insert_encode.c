...
static usz encode_name(u8 *out, usz cap, const u8 *name, usz name_len) {
  usz off = quic_qpack_int_encode(out, cap, 5, QPACK_INSERT_LITNAME, name_len);
  if (off == 0) return 0;
  if (!quic_put_bytes(out, cap, &off, name, name_len)) return 0;
  return off;
}

usz quic_qdyn_insert_literal(
    const u8 *name,
    usz       name_len,
    const u8 *value,
    usz       value_len,
  ...
  usz w = off ? quic_qpack_string_encode(out + off, cap - off, value, value_len)
              : 0;
  if (w == 0) return 0;
  *out_len = off + w;
  return *out_len;
