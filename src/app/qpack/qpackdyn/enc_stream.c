...
static usz apply_capacity(
    quic_qpack_dyn* t, u64 capacity, u64 max_table_capacity, u16* err) {
  if (!quic_qpack_capacity_within_limit(capacity, max_table_capacity)) {
    *err = QUIC_QPACK_ENCODER_STREAM_ERROR;
    return 0;
  }
  quic_qpack_dyn_set_capacity(t, (usz)capacity);
  return 1;
}

/* 1 if the decoded instruction is a Set Dynamic Table Capacity (used != 0,
 * kind matches) -- split out so quic_qdyn_enc_apply_capacity's own branch
 * count stays at the CCN gate. */
static int is_set_capacity(usz used, quic_qpack_enc_kind kind) {
