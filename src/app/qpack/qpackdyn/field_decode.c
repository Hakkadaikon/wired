...
int quic_qdyn_decode_field(
  ...
  int is_static;
  usz used =
      quic_qpack_indexed_decode(fs + pos, fs_len - pos, &index, &is_static);
  if (used == 0) return 0;
  *consumed = used;
  if (is_static) return resolve_static(index, name, name_len, value, value_len);
  return resolve_dynamic(table, base, index, name, name_len, value, value_len);
}
