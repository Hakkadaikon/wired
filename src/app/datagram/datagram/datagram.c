...
static int put_datagram_head(
    u8 *buf, usz cap, usz *off, const quic_datagram_frame *f, int with_len) {
...
static usz decode_no_len(
    const u8 *buf, usz n, usz off, quic_datagram_frame *f) {
  f->length = n - off;
  f->data   = buf + off;
  return n;
}
