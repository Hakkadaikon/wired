...
int quic_h3reqdrive_send_method(
  ...
  u8  fs[256];
  usz fs_len = 0;
  if (!quic_h3req_enc_method(
          method, m_len, path, p_len, authority, a_len, fs, sizeof(fs),
          &fs_len))
    return 0;
  return quic_h3conn_send_request(
      stream_id, fs, fs_len, body, body_len, out, cap, out_len);
}
...
int quic_h3reqdrive_send_get(
  ...
  return quic_h3reqdrive_send_method(
      stream_id, method, 3, path, p_len, authority, a_len, 0, 0, out, cap,
      out_len);
}
...
static int scan_lines(
    const u8 *fs, usz n, u8 *scr, usz scap, usz off, quic_h3reqdrive_req *r) {
  usz used = 0;
  while (off < n)
    if (!step_line(fs, n, scr, scap, &off, &used, r)) return 0;
  return 1;
}
...
static int decode_lines(
    const u8 *fs, usz n, u8 *scr, usz scap, quic_h3reqdrive_req *r) {
...
int quic_h3reqdrive_recv_get(
  ...
  *r               = (quic_h3reqdrive_req){0};
  if (!quic_h3reqdrive_request_sections(stream_data, len, &fs, &fs_len, r))
    return 0;
  if (!decode_lines(fs, fs_len, scratch, scap, r)) return 0;
  return 1;
}
