...
static usz put_headers(u16 status, u8 *out, usz cap) {
  u8  field[16];
  usz flen;
  if (!quic_h3resp_encode_status(status, field, sizeof field, &flen)) return 0;
  return quic_h3_frame_put(out, cap, QUIC_H3_FRAME_HEADERS, field, flen);
}

/* RFC 9114 4.1 */
int quic_h3resp_build(
    u16 status, const u8 *body, usz body_len, u8 *out, usz cap, usz *out_len) {
