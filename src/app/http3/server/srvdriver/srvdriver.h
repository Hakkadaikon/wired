...
/** Resolved driver selection and its knobs, filled by wired_srvdriver_parse
 * (or by hand for a caller that skips CLI parsing). */
typedef struct {
...
/** Parse `--workers`/`--ifindex`/`--queue`/`--ip`/`--skb-mode`/`--cores`/
 * `--control-core`/`--pin-core` from argv into *opt, applying the same
 * exclusive-combination rules as examples/word_list's driver_select:
 ...
 *
 * opt->run/opt->workers/opt->xdp/opt->threads are populated only for the
 * fields this parses; a caller that also wants qlog/keylog paths, app
 * callbacks (wt_on_datagram, etc.), or cert-reload paths sets those directly
 * on opt->run and the wired_srvrun_obs it passes to wired_srvdriver_run
 * separately.
 *
...
/** Dispatch to the driver selected by opt->kind:
