...
/* --ifindex/--ip/--queue/--skb-mode into opt->xdp. Shared by WIRED_SRVDRIVER_
 * XDP and WIRED_SRVDRIVER_THREADS (the latter when --ifindex was also given,
 * selecting AF_XDP multi-queue mode: worker i serves queue i). */
static int srvdriver_load_xdp(
    int argc, char** argv, u16 port, wired_srvxdp_cfg* xdp) {
...
static int srvdriver_load_threads_xdp(
    int argc, char** argv, u16 port, wired_srvdriver_opt* opt) {
  if (wired_cliargs_int(argc, argv, "--ifindex", -1) < 0) {
    opt->threads.xdp = 0;
    return 1;
  }
  if (!srvdriver_load_xdp(argc, argv, port, &opt->xdp)) return 0;
...
static int srvdriver_load_threads(
  ...
  return srvdriver_load_threads_xdp(argc, argv, port, opt);
}
...
static int srvdriver_load_kind_xdp(
    int argc, char** argv, u16 port, wired_srvdriver_opt* opt) {
  return srvdriver_load_xdp(argc, argv, port, &opt->xdp);
}

typedef int (*srvdriver_load_fn)(int, char**, u16, wired_srvdriver_opt*);
...
static int srvdriver_load_kind(
    int argc, char** argv, u16 port, wired_srvdriver_opt* opt) {
  return SRVDRIVER_LOAD_BY_KIND[opt->kind](argc, argv, port, opt);
}

/* opt is zeroed and its kind/pin_core resolved up front (both trivially
...
int wired_srvdriver_parse(int argc, char** argv, wired_srvdriver_opt* opt) {
  ...
  u16 port        = (u16)wired_cliargs_int(argc, argv, "--port", 0);
  ...
  *opt      = (wired_srvdriver_opt){0};
  ...
  if (!srvdriver_load_pin_core(argc, argv, opt->kind, &opt->pin_core))
    return 0;
  return srvdriver_load_kind(argc, argv, port, opt);
}
...
static int srvdriver_run_plain(
  ...
  return wired_server_run_opt(port, id, h, obs, &opt->run);
}
...
static int srvdriver_run_workers(
  ...
  return wired_srvworkers_run(port, id, h, wobs, &opt->workers) >= 0;
}
...
static int srvdriver_run_xdp(
  ...
  int ok;
  if (wired_srvxdp_open(&xdp, &opt->xdp) < 0)
    wired_die("AF_XDP open failed\n");
  run     = opt->run;
  run.xdp = &xdp;
  ok      = wired_server_run_opt(port, id, h, obs, &run);
...
static int srvdriver_run_threads(
    ...
    wired_srvrun_obs           obs,
    const wired_srvdriver_opt* opt) {
  return wired_srvthreads_run(port, id, h, obs, &opt->threads);
}

typedef int (*srvdriver_run_fn)(
    u16, wired_srvboot_id*, wired_srvrun_handler, wired_srvrun_obs,
...
int wired_srvdriver_run(
    ...
    wired_srvrun_obs           obs,
    const wired_srvdriver_opt* opt) {
  return SRVDRIVER_RUN_BY_KIND[opt->kind](port, id, h, obs, opt);
}
