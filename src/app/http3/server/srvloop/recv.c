 ...
 * walks it). largest_pn is unused outside the 1-RTT space (the Initial uses a
 * 4-byte PN). */
...
static int recv_at_level(
  ...
  static int (*const open_at[])(
      quic_server *, u8 *, usz, u64, const u8 **, usz *) = {
      recv_initial,
      recv_handshake,
      recv_onertt,
  };
...
int quic_srvloop_recv(
    ...
    const u8   **payload,
    usz         *payload_len) {
  if (len == 0 || !quic_connrunner_packet_level(dgram[0], level)) return 0;
