 ...
 * app-facing mmsg_buf shape. Delegates every actual mechanism to its
...
typedef struct {
  ...
  u16 bind_flags;   /**< passed through to xsk_cfg */
...
typedef struct {
  xsk           xsk;        /**< AF_XDP socket, UMEM and four rings */
  xskumem_alloc txpool;     /**< TX frame pool, UMEM frames 64..127 */
  xdpmac        macs;       /**< peer ip -> mac reflect cache */
 ...
 * wired_srvxdpbpf_open on cfg->ifindex, then xsksetup_open, then point
 ...
 * per-interface BPF object: xsksetup_open, then register the new
 ...
 * (xdpframe_parse), its payload copied into bufs[i].buf, its source
 ...
 * xsksetup_close the socket/rings/UMEM.
 * @param x driver instance to close */
void wired_srvxdp_close(wired_srvxdp* x);

/** Read XDP_STATISTICS for fd (xsksetup_stats) and log one line per
