...
static i64 srvxdp_open_xsk(
  ...
  xsk_cfg xc = {cfg->ifindex, cfg->queue_id, cfg->bind_flags};
  i64     r  = xsksetup_open(&x->xsk, &xc);
  if (r < 0) return r;
  r = wired_srvxdpbpf_register(bpf, cfg->queue_id, (u32)x->xsk.fd);
  if (r < 0) xsksetup_close(&x->xsk);
  return r;
}
...
static u32 srvxdp_reap_comp(wired_srvxdp* x) {
  u32 idx;
  u32 n = xskring_cons_peek(&x->xsk.comp, x->xsk.comp.size, &idx);
  for (u32 i = 0; i < n; i++)
    xskumem_alloc_put(&x->txpool, *xskring_addr_at(&x->xsk.comp, idx + i));
  if (n) xskring_cons_release(&x->xsk.comp, n);
  return n;
}

/* Learn the peer's MAC (and, the first time, our own) from one parsed RX
 * frame. */
static void srvxdp_learn(wired_srvxdp* x, const xdpframe_rx* rx) {
  xdpmac_learn(&x->macs, wired_udp_addr4_be(&rx->src), rx->peer_mac);
  if (!x->have_mac) {
...
static int srvxdp_rx_one(wired_srvxdp* x, u64 addr, u32 len, mmsg_buf* out) {
  xdpframe_rx rx;
  wired_span  frame = wired_span_of(x->xsk.umem + addr, len);
  if (!xdpframe_parse(frame, &rx)) return 0;
...
static void srvxdp_refill(wired_srvxdp* x, u32 rx_idx, u32 nframes) {
  u32 fill_idx;
  u32 got = xskring_prod_reserve(&x->xsk.fill, nframes, &fill_idx);
  for (u32 i = 0; i < got; i++) {
    xdp_desc* d = xskring_desc_at(&x->xsk.rx, rx_idx + i);
    *xskring_addr_at(&x->xsk.fill, fill_idx + i) = d->addr;
  }
  xskring_prod_submit(&x->xsk.fill, got);
}

/* Process every peeked RX frame [rx_idx, rx_idx+n) into bufs, returning the
 * number of slots filled (<= n). */
static u32 srvxdp_rx_drain(wired_srvxdp* x, u32 rx_idx, u32 n, mmsg_buf* bufs) {
  u32 filled = 0;
  for (u32 i = 0; i < n; i++) {
    xdp_desc* d = xskring_desc_at(&x->xsk.rx, rx_idx + i);
...
i64 wired_srvxdp_rx_burst(wired_srvxdp* x, mmsg_buf* bufs, usz nbufs) {
  u32 rx_idx;
  u32 filled;
  u32 n;
  srvxdp_reap_comp(x);
  n      = xskring_cons_peek(&x->xsk.rx, (u32)nbufs, &rx_idx);
  filled = srvxdp_rx_drain(x, rx_idx, n, bufs);
  if (!n) return filled;
  srvxdp_refill(x, rx_idx, n);
  xskring_cons_release(&x->xsk.rx, n);
  return filled;
}
...
static i64 srvxdp_txpool_get(wired_srvxdp* x) {
  i64 addr = xskumem_alloc_get(&x->txpool);
  if (addr >= 0) return addr;
  srvxdp_reap_comp(x);
  return xskumem_alloc_get(&x->txpool);
}

/* Fill in m's addressing: peer_mac/our_mac plus the ports and addresses
 * xdpframe_build needs, all in host order. */
...
static i64 srvxdp_tx_submit(wired_srvxdp* x, i64 addr, usz n) {
  u32       tx_idx;
  xdp_desc* d;
  if (xskring_prod_reserve(&x->xsk.tx, 1, &tx_idx) != 1) {
    xskumem_alloc_put(&x->txpool, (u64)addr);
    return 0;
  }
  d          = xskring_desc_at(&x->xsk.tx, tx_idx);
  d->addr    = (u64)addr;
  d->len     = (u32)n;
  d->options = 0;
  xskring_prod_submit(&x->xsk.tx, 1);
  xsksetup_kick_tx(x->xsk.fd);
  return 1;
}

i64 wired_srvxdp_send(wired_srvxdp* x, const sockaddr* dst, wired_span pkt) {
  u8          peer_mac[6];
  xdpframe_tx m;
  i64         addr;
  usz         n;

  if (!xdpmac_lookup(&x->macs, wired_udp_addr4_be(dst), peer_mac)) return 0;
...
/* Names for xsksetup_stats's out[6], in its documented order
...
void wired_srvxdp_print_stats(i64 fd) {
  u64 stats[6];
  if (xsksetup_stats(fd, stats) < 0) return;
  for (usz i = 0; i < 6; i++)
