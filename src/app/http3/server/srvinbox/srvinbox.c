 ...
 * producer's own first look shows full -- drop is decided against the
 * reloaded value, never a stale look alone. Every load of the shared cons
 * cell is ACQUIRE, never a plain non-atomic read (which would race the
 * consumer's RELEASE store and carry no synchronizes-with edge to the slot
 * data the consumer is about to read). wired_srvinbox_ring has no
 * xskring.c-style per-side cached_cons field (the push/pop API is stateless
 * across calls, unlike xskring's reserve/submit split), so there is no
 * cheaper look to fall back to here. Returns 1 (still full after any
 * reload) or 0 (room). */
 ...
 * consumer's own first look shows empty -- mirrors srvinbox_prod_full's
 * every-load-is-ACQUIRE rationale (a synchronizes-with edge to the
 * producer's slot write is required before the slot read below, not just a
 * fresh-looking value). */
