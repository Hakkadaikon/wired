...
static int prio_before(const quic_h3prio_candidate* c, usz a, usz b) {
...
static int shift_needed(
    const quic_h3prio_candidate* c, const usz* order, usz key, usz j) {
...
static void order_insert_one(
    const quic_h3prio_candidate* c, usz* order, usz i) {
  usz key = order[i];
  usz j   = i;
...
static void order_insert(const quic_h3prio_candidate* c, usz* order, usz n) {
