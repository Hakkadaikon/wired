...
#include "app/http3/core/h3/grease.h" /* h3_grease_value */
 ...
 * (h3_connect_protocol_ok, wired via srvrun_is_wt_connect) before
 ...
 * (h3_grease_value of the whole byte) to send. A failed RNG read skips
...
static h3settings_in control_settings_in(int advertise_wt) {
  h3settings_in in = {
...
/* RFC 9114 6.2.1 */
int h3settings_control_stream(
    int advertise_wt, u8* out, usz cap, usz* out_len) {
  usz           pre = 0;
  h3settings_in in  = control_settings_in(advertise_wt);
  wired_obuf    ob;
  if (!h3settings_control_prefix(out, cap, &pre)) return 0;
  ob = obuf_of(out + pre, cap - pre);
  if (!h3settings_build(&in, &ob)) return 0;
  *out_len = pre + ob.len;
  return 1;
}
