...
static int sfv_bare(wired_span v, usz i) {
  return i + 1 >= v.n || v.p[i + 1] == ',' || v.p[i + 1] == ' ';
}

/* 1 when v[i..] is `X=?` with a fourth byte to read. */
static int sfv_has_flag(wired_span v, usz i) {
  return i + 3 < v.n && v.p[i + 1] == '=' && v.p[i + 2] == '?';
}

void quic_h3_priority_set_incremental(
    quic_h3_priority* p, wired_span v, usz i) {
  if (sfv_bare(v, i)) {
    p->incremental = 1;
    return;
  }
