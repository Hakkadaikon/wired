...
static int put_head(h3frame_wcur* w, u64 type, u64 len) {
  if (!varint_put(wired_mspan_of(w->buf, w->cap), &w->off, type)) return 0;
  return varint_put(wired_mspan_of(w->buf, w->cap), &w->off, len);
}
...
static int get_head(h3frame_rcur* r, u64* type, u64* len) {
  if (!varint_take(wired_span_of(r->buf, r->n), &r->off, type)) return 0;
  return varint_take(wired_span_of(r->buf, r->n), &r->off, len);
}
...
static usz put_one(wired_obuf* out, u64 type, u64 v) {
  h3frame_wcur w = {out->p, out->cap, 0};
  if (!put_head(&w, type, varint_len(v))) return 0;
  if (!varint_put(wired_mspan_of(w.buf, w.cap), &w.off, v)) return 0;
  out->len = w.off;
  return w.off;
}
...
static int get_one_body(h3frame_rcur* r, u64 len, u64* v) {
  usz vstart = r->off;
  if (!varint_take(wired_span_of(r->buf, r->n), &r->off, v)) return 0;
...
static int frame_put_pair(h3frame_wcur* w, const u64* id, const u64* value) {
  if (!varint_put(wired_mspan_of(w->buf, w->cap), &w->off, *id)) return 0;
  return varint_put(wired_mspan_of(w->buf, w->cap), &w->off, *value);
}
...
static int frame_take_pair(h3frame_rcur* r, h3_settings* s) {
  if (!take_pair_id(r, s)) return 0;
  if (!varint_take(wired_span_of(r->buf, r->n), &r->off, &s->pairs[s->n].value))
    return 0;
  s->n++;
  return 1;
}
...
static usz settings_body_len(const h3_settings* s) {
  usz total = 0;
  for (usz i = 0; i < s->n; i++)
    total += varint_len(s->pairs[i].id) + varint_len(s->pairs[i].value);
  return total;
}

/* Append every (Identifier Value) pair after the head. Returns 1 ok, 0. */
static int put_settings_pairs(h3frame_wcur* w, const h3_settings* s) {
  int ok = 1;
  for (usz i = 0; i < s->n; i++)
 ...
 * truncation/overflow. */
static int get_settings_pairs(h3frame_rcur* r, usz end, h3_settings* s) {
  s->n = 0;
  while (r->off < end)
