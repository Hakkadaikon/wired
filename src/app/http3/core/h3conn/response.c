...
int quic_h3conn_send_response(u64 stream_id, u16 status,
                              ...
                              u8 *out, usz cap, usz *out_len)
{
    ...
    if (!quic_h3resp_build(status, body, body_len, h3, sizeof(h3), &h3_len))
        return 0;
    return quic_appdata_stream_frame(stream_id, 0, h3, h3_len, 1,
                                     out, cap, out_len);
}
...
static int status_from_indexed(const u8 *buf, usz n, u16 *status)
{
    ...
    const char *name = 0;
    const char *value = 0;
    if (!quic_qpack_indexed_decode(buf, n, &index, &is_static))
        return 0;
    if (!quic_qpack_static_get((usz)index, &name, &value))
        return 0;
...
static int status_from_literal(const u8 *buf, usz n, u16 *status)
{
    ...
    u8 val[8];
    usz vlen = 0;
    if (!quic_qpack_literal_namref_decode(buf, n, &index, &is_static, &never,
                                          val, sizeof(val), &vlen))
        return 0;
    if (vlen != 3)
        return 0;
...
static int decode_status(const u8 *fs, usz n, u16 *status)
{
    ...
    if (!off)
        return 0;
    if (status_from_indexed(fs + off, n - off, status))
        return 1;
    return status_from_literal(fs + off, n - off, status);
}

/* RFC 9114 4.1 / RFC 9000 19.8 */
int quic_h3conn_recv_response(const u8 *stream_data, usz len, u16 *status,
                              const u8 **body, usz *body_len)
{
    quic_stream_frame f;
    const u8 *headers = 0;
    ...
    if (!quic_frame_get_stream(stream_data, len, &f))
        return 0;
    if (!quic_h3req_resp_parse(f.data, (usz)f.length, &headers, &h_len,
                               body, body_len))
        return 0;
