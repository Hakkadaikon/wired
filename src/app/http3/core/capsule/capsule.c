...
static wired_mspan capsule_out_tail(wired_obuf* out) {
  return wired_mspan_of(out->p + out->len, out->cap - out->len);
}
 ...
 * single byte of it. */
static int capsule_fits(const wired_obuf* out, usz total) {
...
int capsule_decode(wired_span data, usz* at, u64* type, wired_span* value) {
  ...
  *type  = t;
  *value = wired_span_of(data.p + value_start, len);
  *at    = value_start + len;
  return 1;
}
