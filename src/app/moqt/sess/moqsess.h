#ifndef QUIC_MOQSESS_H
#define QUIC_MOQSESS_H
 ...
 * done so). */
typedef enum {
  QUIC_MOQSESS_IDLE        = 0, /* neither SETUP sent nor peer's received */
...
typedef enum {
  QUIC_MOQSESS_EV_SENT_SETUP     = 0, /* own SETUP finished sending */
  QUIC_MOQSESS_EV_RECV_SETUP     = 1, /* peer's SETUP received */
  QUIC_MOQSESS_EV_CTRL_CLOSED    = 2, /* control stream closed at transport */
  QUIC_MOQSESS_EV_UNKNOWN_UNI    = 3, /* unknown uni stream type byte */
  QUIC_MOQSESS_EV_SECOND_CTRL    = 4, /* 2nd control stream from same peer */
  QUIC_MOQSESS_EV_MALFORMED_CTRL = 5, /* unknown msg type / len mismatch */
  QUIC_MOQSESS_EV_SEND_GOAWAY    = 6,
  QUIC_MOQSESS_EV_RECV_GOAWAY    = 7,
  QUIC_MOQSESS_EV_RECV_GOAWAY_BAD_URI = 8, /* server: nonzero URI length */
  QUIC_MOQSESS_EV_GOAWAY_TIMEOUT      = 9, /* peer did not close in time */
  QUIC_MOQSESS_EV_BAD_FIRST = 10,          /* new request stream's head is not a
...
#define QUIC_MOQSESS_CLOSE_PROTOCOL_VIOLATION 1
#define QUIC_MOQSESS_CLOSE_INVALID_REQUEST_ID 2
#define QUIC_MOQSESS_CLOSE_GOAWAY_TIMEOUT 3
#define QUIC_MOQSESS_CLOSE_NO_ERROR 4
...
typedef enum {
  ...
  QUIC_MOQSUB_EV_OK           = 1, /* SUBSCRIBE_OK / PUBLISH_OK (REQUEST_OK) */
  QUIC_MOQSUB_EV_ERROR        = 2, /* REQUEST_ERROR sent or received */
  QUIC_MOQSUB_EV_UPDATE       = 3, /* REQUEST_UPDATE: no state change */
  QUIC_MOQSUB_EV_STOP_SENDING = 4, /* STOP_SENDING sent or received */
...
#define QUIC_MOQSUB_TERM_NONE 0
#define QUIC_MOQSUB_TERM_STOP_SENDING 1 /* cancel via STOP_SENDING */
#define QUIC_MOQSUB_TERM_PUBLISH_DONE 2 /* publisher finished normally */
#define QUIC_MOQSUB_TERM_ERROR 3        /* REQUEST_ERROR (reject or fault) */
#define QUIC_MOQSUB_TERM_EARLY_FIN 4    /* peer FIN'd before responding */
