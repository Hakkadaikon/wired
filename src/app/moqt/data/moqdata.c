...
static const struct {
  ...
  int kind;
    ...
    {MOQDATA_TYPE_SETUP, MOQDATA_STREAM_CONTROL},
    {MOQDATA_TYPE_FETCH_HEADER, MOQDATA_STREAM_FETCH},
    {MOQDATA_TYPE_PADDING, MOQDATA_STREAM_PADDING},
};
...
static int moqdata_classify_kind(u64 v) {
  ...
  if (moqdata_type_valid(v)) return MOQDATA_STREAM_SUBGROUP;
  return MOQDATA_STREAM_UNKNOWN;
}

int moqdata_classify(wired_span in, usz* off) {
  usz at = 0;
  u64 v;
  if (!moqvi_take(in, &at, &v)) return MOQDATA_STREAM_INSUFFICIENT;
  *off = at;
...
static int moqdata_subhdr_take_body(
  ...
  if (!moqdata_subhdr_take_ids(buf, at, h)) return MOQDATA_INSUFFICIENT;
  if (!moqdata_subhdr_take_prio(buf, at, h)) return MOQDATA_INSUFFICIENT;
  *off = *at;
  return MOQDATA_OK;
}
...
int moqdata_subhdr_take(wired_span buf, usz* off, moqdata_subhdr* h) {
  ...
  if (!moqdata_type_valid(type)) return MOQDATA_VIOLATION;
  *h      = (moqdata_subhdr){0};
  h->type = type;
...
int moqdata_subhdr_put(wired_mspan buf, usz* off, const moqdata_subhdr* h) {
  usz at = *off;
  if (!moqdata_type_valid(h->type)) return MOQDATA_VIOLATION;
  if (!moqdata_subhdr_put_rest(buf, &at, h)) return MOQDATA_INSUFFICIENT;
  *off = at;
  return MOQDATA_OK;
}
...
static int moqdata_obj_take_id(
  ...
  u64 delta;
  if (!moqvi_take(buf, at, &delta)) return MOQDATA_INSUFFICIENT;
  if (moqdata_id_overflow(seq->have_prev, seq->prev_id, delta))
    return MOQDATA_VIOLATION;
...
static int moqdata_status_known(u64 s) {
  return s == MOQDATA_STATUS_NORMAL || s == MOQDATA_STATUS_END_OF_GROUP ||
         s == MOQDATA_STATUS_END_OF_TRACK;
}

static int moqdata_obj_props_ok(u64 props_len, u64 status) {
  return props_len == 0 || status == MOQDATA_STATUS_NORMAL;
}

static int moqdata_obj_take_status(wired_span buf, usz* at, u64* status) {
  if (!moqvi_take(buf, at, status)) return MOQDATA_INSUFFICIENT;
  if (!moqdata_status_known(*status)) return MOQDATA_VIOLATION;
  return MOQDATA_OK;
}
...
static int moqdata_obj_take_empty(
  ...
  if (!moqdata_obj_props_ok(props_len, o->status)) return MOQDATA_VIOLATION;
...
static int moqdata_obj_put_body(wired_mspan buf, usz* at, wired_span payload) {
  ...
  if (payload.n == 0) return moqvi_put(buf, at, MOQDATA_STATUS_NORMAL);
...
int moqdata_obj_put(
  ...
  usz at = *off;
  if (!moqvi_put(buf, &at, id_delta)) return MOQDATA_INSUFFICIENT;
  if (!moqdata_obj_put_body(buf, &at, payload)) return MOQDATA_INSUFFICIENT;
  *off = at;
  return MOQDATA_OK;
}
...
int moqdata_msg_build(wired_mspan buf, usz* off, const moqdata_msg* m) {
  ...
  if (moqdata_subhdr_put(buf, &at, &h) != MOQDATA_OK)
    return MOQDATA_INSUFFICIENT;
  if (moqdata_obj_put(buf, &at, 0, m->payload) != MOQDATA_OK)
    return MOQDATA_INSUFFICIENT;
  *off = at;
  return MOQDATA_OK;
}
