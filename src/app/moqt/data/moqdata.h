...
int quic_moqdata_type_props(u64 type);
u64 quic_moqdata_type_sgid_mode(u64 type);
int quic_moqdata_type_end_of_group(u64 type);
int quic_moqdata_type_default_priority(u64 type);
int quic_moqdata_type_first_object(u64 type);
 ...
 * quic_moqdata_obj_take calls on the same SUBGROUP_HEADER. */
typedef struct {
  int has_props;
  int have_prev;
  u64 prev_id;
