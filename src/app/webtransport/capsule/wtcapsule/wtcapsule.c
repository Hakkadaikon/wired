...
int quic_wtcapsule_encode_close(quic_obuf* out, u32 app_error_code, quic_span message) {
  ...
  for (i = 0; i < message.n; i++) body[QUIC_WTCAPSULE_CLOSE_CODE_LEN + i] = message.p[i];
  return quic_capsule_encode(out, QUIC_WTCAPSULE_TYPE_CLOSE,
                              quic_span_of(body, QUIC_WTCAPSULE_CLOSE_CODE_LEN + message.n));
}

int quic_wtcapsule_encode_drain(quic_obuf* out) {
  return quic_capsule_encode(out, QUIC_WTCAPSULE_TYPE_DRAIN, quic_span_of(0, 0));
}
...
static int wtcapsule_is_close(u64 type, quic_span value) {
  return type == QUIC_WTCAPSULE_TYPE_CLOSE && value.n >= QUIC_WTCAPSULE_CLOSE_CODE_LEN &&
         value.n - QUIC_WTCAPSULE_CLOSE_CODE_LEN <= QUIC_WTCAPSULE_CLOSE_MESSAGE_MAX;
}
...
static void wtcapsule_take_close(quic_span value, u32* app_error_code, quic_span* message) {
  *app_error_code = quic_get_be32(value.p);
  *message = quic_span_of(value.p + QUIC_WTCAPSULE_CLOSE_CODE_LEN,
