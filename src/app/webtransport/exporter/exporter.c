...
static int ctx_fields_fit(quic_span label, quic_span app_context) {
...
static int put_len8_field(u8* out, usz cap, usz* off, quic_span field) {
...
static usz put_ctx_fields(
    u8* out, usz cap, quic_span label, quic_span app_context) {
  usz off = 8;
...
int quic_wt_exporter(
  ...
  return quic_tls_exporter(
      exporter_secret, quic_span_of(exporter_label, sizeof exporter_label - 1),
      quic_span_of(ctx, ctx_len), okm);
}
