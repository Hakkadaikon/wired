...
#include "srvfin/verify.h"
#include "srvfin/hsdone.h"
                      ...
                      const u8 *cert_der, usz cert_len)
{
    ...
    quic_sdrv_init(&s->sdrv, server_priv_x25519, server_pub_x25519,
                   cert_seed, cert_der, cert_len);
...
static int srv_install_hs_keys(quic_server *s)
{
    ...
    if (!quic_keysched_advance_handshake(&s->sched, ecdhe, sizeof(ecdhe),
                                         s->tr, s->tr_through_sh))
        return 0;
...
static int srv_after_flight(quic_server *s, const u8 *sh, usz shl,
                            const u8 *fl, usz fll)
{
...
static int srv_emit_flight(quic_server *s, const u8 *server_random,
                           u8 *sh_out, usz sh_cap, usz *sh_len,
                           u8 *fl_out, usz fl_cap, usz *fl_len)
{
    if (!quic_sdrv_build_server_flight(&s->sdrv, server_random,
                                       sh_out, sh_cap, sh_len,
                                       fl_out, fl_cap, fl_len))
        return 0;
                             ...
                             u8 *flight_out, usz flight_cap, usz *flight_len)
{
    ...
    if (!srv_emit_flight(s, server_random, sh_out, sh_cap, sh_len,
                         flight_out, flight_cap, flight_len))
        return 0;
...
static int srv_verify_finished(quic_server *s, const u8 *msg, usz len)
{
    ...
    quic_tls_derive_secret(hs, "c hs traffic", 12, s->tr, s->tr_through_sh,
                           c_traffic);
...
{
    if (!quic_srvfin_should_send_handshake_done(s->fin.confirmed,
                                                s->hs_done_sent))
        return 0;
