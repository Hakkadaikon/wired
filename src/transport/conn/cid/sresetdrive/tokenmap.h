#ifndef SRESETDRIVE_TOKENMAP_H
#define SRESETDRIVE_TOKENMAP_H
...
#define SRESETDRIVE_MAX_CID                                        \
  20                      /* RFC 9000 5.1: CID is at most 20 bytes \
                           */
#define SRESETDRIVE_CAP 8 /* concurrent CIDs tracked per peer */

/** One connection ID's remembered Stateless Reset Token (RFC 9000 10.3). */
typedef struct {
  u8 cid[SRESETDRIVE_MAX_CID];
  u8 cid_len;
...
typedef struct {
  sresetdrive_entry e[SRESETDRIVE_CAP];
  usz               count;
} sresetdrive_map;
