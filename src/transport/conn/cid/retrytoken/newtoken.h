#ifndef RETRYTOKEN_NEWTOKEN_H
#define RETRYTOKEN_NEWTOKEN_H
...
#define NEWTOKEN_KEY 32   /* HMAC-SHA256 key */
#define NEWTOKEN_NONCE 16 /* CSPRNG bytes making each token unique */
#define NEWTOKEN_MAC 32   /* HMAC-SHA256 output */
#define NEWTOKEN_WIRE_LEN (8 + NEWTOKEN_NONCE + NEWTOKEN_MAC)
...
#define NEWTOKEN_MAX_AGE_SECS (7ULL * 24 * 3600)
 ...
 * NEWTOKEN_WIRE_LEN bytes to token. Returns 1 on success, 0 if the
 * CSPRNG failed. */
 ...
 * time. Accepts only if: the token is exactly NEWTOKEN_WIRE_LEN bytes,
 * its HMAC matches (constant-time), and now_secs is within
 * NEWTOKEN_MAX_AGE_SECS of the token's issued_at (RFC 9000 8.1.3
