...
#define NEWTOKEN_MSG 64 /* addr + issued_at + nonce, bounded small */
...
static void newtoken_msg_body(
  ...
  bytes_put(buf, off, wired_span_of(nonce, NEWTOKEN_NONCE));
}
...
static usz newtoken_build_msg(
  ...
  usz off = 0;
  if (addr.n + 8 + NEWTOKEN_NONCE > NEWTOKEN_MSG) return 0;
  newtoken_msg_body(
      wired_mspan_of(msg, NEWTOKEN_MSG), &off, addr, issued_at, nonce);
  return off;
}

static void newtoken_mac(
    const u8   key[NEWTOKEN_KEY],
    wired_span addr,
    u64        issued_at,
    const u8   nonce[NEWTOKEN_NONCE],
    u8         mac[NEWTOKEN_MAC]) {
  u8  msg[NEWTOKEN_MSG];
  usz n = newtoken_build_msg(msg, addr, issued_at, nonce);
  hmac_sha256(wired_span_of(key, NEWTOKEN_KEY), wired_span_of(msg, n), mac);
}
...
int newtoken_wire_make(
  ...
  u8* nonce = token + 8;
  if (!rng_bytes(nonce, NEWTOKEN_NONCE)) return 0;
  be_put_be64(token, now_secs);
  newtoken_mac(key, addr, now_secs, nonce, token + 8 + NEWTOKEN_NONCE);
  return 1;
}
 ...
 * nor expired past NEWTOKEN_MAX_AGE_SECS. */
static int within_lifetime(u64 issued_at, u64 now_secs) {
  if (now_secs < issued_at) return 0;
  return now_secs - issued_at <= NEWTOKEN_MAX_AGE_SECS;
}
...
static int mac_ok(
    ...
    u64        issued_at,
    wired_span token) {
  u8 want[NEWTOKEN_MAC];
  newtoken_mac(key, addr, issued_at, token.p + 8, want);
  return ct_diff32(want, token.p + 8 + NEWTOKEN_NONCE) == 0;
}
...
static int newtoken_valid(
    ...
    u64        now_secs,
    u64*       issued_at) {
  if (token.n != NEWTOKEN_WIRE_LEN) return 0;
...
int newtoken_wire_verify(
  ...
  *nonce = wired_span_of(token.p + 8, NEWTOKEN_NONCE);
  return 1;
}
