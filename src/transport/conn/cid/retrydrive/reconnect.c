...
int quic_retrydrive_apply(
  ...
  quic_put_bytes(quic_mspan_of(out->token, sizeof out->token), &off, quic_span_of(retry_token.p, retry_token.n));
  out->token_len = retry_token.n;
  off            = 0;
  quic_put_bytes(quic_mspan_of(out->dcid, sizeof out->dcid), &off, quic_span_of(retry_scid.p, retry_scid.n));
