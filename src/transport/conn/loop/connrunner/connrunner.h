...
typedef struct {
  i64      fd;
  sockaddr peer;
  evloop   loop;     /* the deciding state machine */
  connio   io;       /* the real crypto / frame dispatch */
  rtxbytes rtx;      /* RFC 9002 13.3: real frame bytes kept for resend */
  sentmeta sent;     /* RFC 9002 A.1: real sent-packet metadata + in-flight */
