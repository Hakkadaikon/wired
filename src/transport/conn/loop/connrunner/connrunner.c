#include "transport/conn/loop/connrunner/connrunner.h"

#include "transport/conn/loop/connrunner/keyupdate.h"
#include "transport/conn/loop/connrunner/pmtudrive.h"
#include "transport/conn/loop/connrunner/reconnect.h"
#include "transport/conn/loop/connrunner/recv.h"
#include "transport/conn/loop/connrunner/send.h"
#include "transport/io/socket/poll/deadline.h"
#include "transport/io/socket/poll/wait.h"
#include "transport/io/udp/udploop/rxloop.h"
#include "transport/io/udp/udploop/txloop.h"

void connrunner_init(
    connrunner* r, wired_span dcid, const connrunner_init_in* in) {
  connio_init_in cin = {in->is_server, in->byte0, in->initial_max_data};
  evloop_init_in ein = {in->level, in->cwnd, in->send_len};
  r->fd              = in->fd;
  r->peer            = *in->peer;
  evloop_init(&r->loop, &ein);
  rtxbytes_init(&r->rtx);
  sentmeta_init(&r->sent);
  r->rtx_held = 0;
  connio_init(&r->io, dcid, &cin);
  connrunner_keyupdate_init(r);
  connrunner_reconnect_init(r);
  connrunner_pmtu_init(r);
}

/* RFC 9000 19.7/19.20 via connio.h: a violation flagged while dispatching the
 * just-processed datagram (e.g. a server dispatching NEW_TOKEN, RFC 9000
 * 19.7) outranks the loop's own send choice -- if flush_sends already sent
 * nothing this step, seal the CONNECTION_CLOSE into r->txbuf instead so a
 * protocol violation is never silently dropped for lack of anything else to
 * send. */
static usz advance_close_on_violation(connrunner* r, usz out) {
  wired_obuf ob;
  if (out) return out;
  ob = obuf_of(r->txbuf, sizeof(r->txbuf));
  return connio_close_on_violation(&r->io, &ob);
}

/* RFC 9001 6.6: same pattern as advance_close_on_violation, for the AEAD
 * integrity limit latched by connio_recv's decrypt-failure path -- only
 * runs when nothing else was already chosen to send this step. */
static usz advance_close_on_aead_limit(connrunner* r, usz out) {
  wired_obuf ob;
  if (out) return out;
  ob = obuf_of(r->txbuf, sizeof(r->txbuf));
  return connio_close_on_aead_limit(&r->io, &ob);
}

/* RFC 9000 3.5: same pattern as advance_close_on_violation, for the
 * RESET_STREAM a STOP_SENDING obligates -- only runs when nothing else was
 * already chosen to send this step. */
static usz advance_stop_sending_reset(connrunner* r, usz out) {
  wired_obuf ob;
  if (out) return out;
  ob = obuf_of(r->txbuf, sizeof(r->txbuf));
  return connio_send_stop_sending_reset(&r->io, &ob);
}

/* RFC 8899: reconcile the outstanding probe against this round's ack/loss
 * results, mirroring connrunner_track_acks/track_loss's own detection. */
static void advance_pmtu_reconcile(connrunner* r, u64 now) {
  u64 lost[QUIC_SENTMETA_CAP];
  usz n;
  connrunner_track_loss_ex(r, now, lost, &n);
  connrunner_pmtu_reconcile(r, lost, n, now);
}

/* RFC 8899 3.2/5.1: once the handshake is confirmed and nothing else was
 * chosen to send this iteration, opportunistically send one PLPMTU probe as
 * its own datagram -- a probe must exceed the normal PLPMTU, so it is never
 * coalesced with a regular packet built to send_len. */
static usz advance_pmtu_probe(connrunner* r, usz out, u64 now) {
  usz sealed;
  if (out || !r->loop.gate.handshake_confirmed) return out;
  {
    wired_obuf ob = obuf_of(r->txbuf, sizeof(r->txbuf));
    sealed        = connrunner_pmtu_build_probe(r, &ob, now);
  }
  return sealed;
}

/* RFC 9000 12: the fixed-order core of one iteration, with the datagram already
 * in hand (or empty). Drain receives, step the loop (timers + one send
 * decision), then seal whatever the loop chose -- recv before step before send.
 * Returns the sealed datagram length to transmit, or 0. Socket-free. */
usz connrunner_advance(connrunner* r, u64 now, wired_mspan dgram) {
  u64 sent_before;
  int kind;
  usz out;
  if (dgram.n) connrunner_process_datagram(r, dgram);
  connrunner_track_acks(r);          /* RFC 9002 A.2.2: drop acked bytes */
  kind = connrunner_pending_kind(r); /* before step clears it */
  connrunner_capture_rtx(r);         /* lost pn before step drains it */
  advance_pmtu_reconcile(r, now);    /* RFC 8899: probe ack/loss */
  sent_before = r->loop.next_pn;
  evloop_step(&r->loop, now);
  out = connrunner_flush_sends(r, sent_before, kind);
  out = advance_close_on_violation(r, out);
  out = advance_close_on_aead_limit(r, out);
  out = advance_stop_sending_reset(r, out);
  {
    connrunner_sent_in sin = {now, kind, out};
    connrunner_track_sent(r, &sin); /* RFC 9002 A.1: in-flight */
  }
  out = advance_pmtu_probe(r, out, now);
  connrunner_pmtu_track_sent(r, now, out); /* RFC 9002 6.1 visibility */
  return out;
}

/* Append an armed timer's deadline to ds (RFC 9002 6: unarmed timers do not
 * bound the wait). */
static void push_armed(const evloop_timer* t, u64* ds, usz* n) {
  if (t->armed) ds[(*n)++] = t->deadline;
}

/* RFC 9002 6.2 / 6.1 / RFC 9000 10.1: the next wakeup is the soonest armed
 * timer. Returns 0 if none armed. */
static u64 next_deadline(const connrunner* r) {
  u64 ds[3];
  usz n = 0;
  push_armed(&r->loop.pto, ds, &n);
  push_armed(&r->loop.loss, ds, &n);
  push_armed(&r->loop.idle, ds, &n);
  return poll_min_deadline(ds, n);
}

/* Milliseconds to wait until the next deadline; 0 means block (no timer). */
static u64 wait_timeout(const connrunner* r, u64 now) {
  u64 dl = next_deadline(r);
  if (!dl) return 0;
  return poll_timeout_until(now, dl);
}

/* Wait for and read one datagram into rxbuf; returns its length, or 0. */
static usz wait_and_recv(connrunner* r, u64 now) {
  i64 got;
  if (poll_wait_readable(r->fd, wait_timeout(r, now)) != 1) return 0;
  got = wired_udp_recvfrom(
      r->fd, wired_mspan_of(r->rxbuf, sizeof(r->rxbuf)), &r->peer);
  if (got <= 0) return 0;
  return (usz)got;
}

void connrunner_iterate(connrunner* r, u64 now) {
  usz len = wait_and_recv(r, now);
  usz out = connrunner_advance(r, now, wired_mspan_of(r->rxbuf, len));
  if (out) {
    usz        one = out;
    udpdst     dst = {r->fd, &r->peer};
    pktsrc     src = {r->txbuf, &one, 1};
    wired_obuf ob  = obuf_of(r->rxbuf, sizeof(r->rxbuf));
    udploop_tx(&dst, &src, &ob);
  }
}

/* Progress halts once closed; otherwise iterate up to the bound. */
static int run_done(const connrunner* r, usz i, usz max) {
  return i >= max || r->loop.gate.phase == QUIC_CONNLOOP_CLOSED;
}

void connrunner_run(connrunner* r, u64 now, usz max_iterations) {
  usz i;
  for (i = 0; !run_done(r, i, max_iterations); i++) connrunner_iterate(r, now);
}
