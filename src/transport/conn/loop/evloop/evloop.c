...
void evloop_init(evloop* c, const evloop_init_in* in) {
  connloop_init(&c->gate, 1);
  c->level         = in->level;
  c->next_pn       = 0;
  c->rx_n          = 0;
  c->ack_owed      = 0;
...
static void drain_one(evloop* c, const evloop_rx* r) {
  connloop_on_recv(&c->gate, c->level, c->send_len);
...
static void pto_reset_on_recv(evloop* c, u64 now) {
...
static void phase_receive(evloop* c, u64 now) {
  usz i;
...
static int timer_fires(const evloop_timer* t, u64 now) {
 ...
 * an unvalidated path. */
static int antiamp_ok(const evloop* c) {
  pathbudget b = {c->gate.recv_bytes, c->gate.sent_bytes, c->gate.validated};
  return udploop_send_allowed(&b, c->send_len);
}
 ...
 */
static void pto_act(evloop* c) {
  connloop_pto_in in = {c->level, c->next_pn, c->send_len};
  if (!antiamp_ok(c)) return;
  if (connloop_on_pto(&c->gate, &in)) c->next_pn++;
}

/* RFC 9000 10.1: idle expiry moves the connection toward draining. */
static void idle_act(evloop* c) { connloop_close(&c->gate, 0); }

/* Disarm-then-act one timer: clear armed first so a stale arming cannot fire
 * the action twice. */
static void run_timer(
    evloop_timer* t, u64 now, void (*act)(evloop*), evloop* c) {
...
static void phase_timers(evloop* c, u64 now) {
...
static void evloop_emit(evloop* c) {
  connloop_send_in in = {c->level, 1, c->next_pn, c->send_len};
  connloop_on_send(&c->gate, &in);
  c->next_pn++;
...
static void phase_send(evloop* c) {
...
void evloop_run(evloop* c, u64 now, usz max_iterations) {
  usz i;
  for (i = 0; !step_done(c, i, max_iterations); i++) evloop_step(c, now);
}

int evloop_initiate_key_update(evloop* c, u64 now) {
  keyupdate_in in = {
...
int evloop_old_key_retained(const evloop* c, u64 now) {
  if (c->key_generation == 0) return 0; /* no prior generation exists yet */
  return oldkey_retain(c->key_update_time, now, c->pto_period);
}

void evloop_close(evloop* c, int peer_closed) {
  connloop_close(&c->gate, peer_closed);
}

void evloop_discard_keys(evloop* c, int level) {
  keyset_discard(&c->gate.keys, level);
  c->pto.armed       = 0;
  c->loss.armed      = 0;
  c->bytes_in_flight = 0;
  sentpkt_init(&c->gate.sent);
}
