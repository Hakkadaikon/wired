...
static int antiamp_ok(const quic_evloop *c) {
  return quic_udploop_send_allowed(
      c->gate.recv_bytes, c->gate.sent_bytes, c->gate.validated, c->send_len);
}
...
static void idle_act(quic_evloop *c) { quic_connloop_close(&c->gate, 0); }

/* Disarm-then-act one timer: clear armed first so a stale arming cannot fire
 * the action twice. */
static void run_timer(
    quic_evloop_timer *t, u64 now, void (*act)(quic_evloop *), quic_evloop *c) {
...
void quic_evloop_run(quic_evloop *c, u64 now, usz max_iterations) {
  usz i;
  for (i = 0; !step_done(c, i, max_iterations); i++) quic_evloop_step(c, now);
}
