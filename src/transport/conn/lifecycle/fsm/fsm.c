...
static int row_matches(const fsm_row* r, u8 from, u8 ev) {
  return r->from == from && r->ev == ev;
}

/* Look up (from, ev). Returns the target state, or FSM_NONE if absent. */
static u8 lookup(const fsm_table* table, u8 from, u8 ev) {
