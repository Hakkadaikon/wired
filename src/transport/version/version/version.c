...
/* Write the id and length varints; return 1 ok with *off advanced, 0 if no
 * room or too many Available Versions. */
static int put_vi_head(
    u8 *buf, usz cap, usz *off, const quic_version_info *vi, usz vlen) {
  if (vi->n_available > QUIC_VI_MAX_AVAILABLE) return 0;
  if (!quic_varint_put(buf, cap, off, QUIC_TP_VERSION_INFORMATION)) return 0;
  return quic_varint_put(buf, cap, off, vlen);
}
...
static int take_vi_id(const u8 *buf, usz n, usz *off) {
  u64 id;
...
static int take_vi_head(const u8 *buf, usz n, usz *off, u64 *vlen) {
  if (!take_vi_id(buf, n, off)) return 0;
  if (!quic_varint_take(buf, n, off, vlen)) return 0;
  return vlen_ok(*vlen);
}

/* Fill chosen + available from the value bytes (count = available count). */
static void read_versions(
    const u8 *buf, usz off, quic_version_info *vi, usz count) {
  vi->chosen      = version_rd_be32(buf + off);
  vi->n_available = count;
  for (usz i = 0; i < count; i++)
    vi->available[i] = version_rd_be32(buf + off + 4 + 4 * i);
}
...
usz quic_version_info_decode(const u8 *buf, usz n, quic_version_info *vi) {
  usz off = 0;
  u64 vlen;
  if (!take_vi_head(buf, n, &off, &vlen)) return 0;
