...
static int put_vi_head(const version_wcursor* w, const quic_version_info* vi) {
  ...
  return quic_varint_put(
      wired_mspan_of(w->buf, w->cap), w->off, vi_value_len(vi));
}
...
static int take_vi_id(const version_rcursor* r) {
  u64 id;
  if (!quic_varint_take(wired_span_of(r->buf, r->n), r->off, &id)) return 0;
...
static int take_vi_head(const version_rcursor* r, u64* vlen) {
  ...
  if (!quic_varint_take(wired_span_of(r->buf, r->n), r->off, vlen)) return 0;
  return vlen_ok(*vlen);
}
