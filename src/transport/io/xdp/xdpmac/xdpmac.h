...
typedef struct {
  u32 ip[QUIC_XDPMAC_CAP];     /* IPv4 key, network byte order; valid if used */
  u8  mac[QUIC_XDPMAC_CAP][6]; /* Ethernet MAC learned for ip */
  u8  used[QUIC_XDPMAC_CAP];   /* 1 if the slot holds an entry */
  u32 clock;                   /* next round-robin eviction victim */
} quic_xdpmac;
