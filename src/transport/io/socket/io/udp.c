...
i64 quic_udp_send(i64 fd, const quic_sockaddr_in *sa, const u8 *buf, usz len) {
  return syscall6(SYS_sendto, fd, (i64)buf, (i64)len, 0, (i64)sa, sizeof(*sa));
}
...
i64 quic_udp_recvfrom(i64 fd, u8 *buf, usz len, quic_sockaddr_in *src) {
  ...
  return syscall6(
      SYS_recvfrom, fd, (i64)buf, (i64)len, 0, (i64)src, (i64)&addrlen);
}

i64 quic_udp_close(i64 fd) { return syscall1(SYS_close, fd); }
