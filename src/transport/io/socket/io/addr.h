...
/* Pack octets[0..3] = a.b.c.d into a big-endian u32 (a is the most
 * significant byte). */
u32 addr_from_octets(const u8 octets[4]);

/* Unpack a big-endian u32 into out[0..3] = a,b,c,d. */
void addr_to_octets(u32 addr, u8 out[4]);

/* Convert a host-order port to network byte order. */
u16 port_be(u16 port);
