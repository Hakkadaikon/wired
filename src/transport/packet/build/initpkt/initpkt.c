...
static int build_payload(
  ...
  if (!quic_crypto_stream_emit(
          crypto_payload, payload_len, 0, payload_len, buf, cap, &n))
    return 0;
...
int quic_initpkt_build(
  ...
  if (!build_payload(
          crypto_payload, payload_len, pad_target(dcid_len, scid_len), payload,
          sizeof(payload), &plen))
    return 0;
  total = quic_tx_packet(
      &ck, &hp, 0xc3, dcid, dcid_len, scid, scid_len, 1, (const u8 *)0, 0, pn,
