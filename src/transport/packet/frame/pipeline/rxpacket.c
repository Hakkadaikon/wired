                   ...
                   usz *frames_len)
{
    ...
    u64 length;
    if (!quic_lhdr_parse(pkt, pkt_len, is_initial, &dcid, &dcid_len, &scid,
                         &scid_len, &token, &token_len, &length, &pn_off))
        return 0;
    return quic_vpn_open(keys, hp, pkt, pkt_len, pn_off, length, frames,
                         frames_len);
}
