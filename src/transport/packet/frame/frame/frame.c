...
static int put_crypto_hdr(
    u8 *buf, usz cap, usz *off, const quic_crypto_frame *f) {
...
static int take_crypto_hdr(
    const u8 *buf, usz n, usz *off, quic_crypto_frame *f) {
...
static int put_stream_prefix(
    u8 *buf, usz cap, usz *off, const quic_stream_frame *f) {
...
static int take_opt_offset(
    const u8 *buf, usz n, usz *off, u8 type, u64 *offset) {
  *offset = 0;
...
static int take_stream_hdr(
    const u8 *buf, usz n, usz *off, u8 type, quic_stream_frame *f) {
...
static int put_opt_frame_type(
    u8 *buf, usz cap, usz *off, const quic_conn_close_frame *f) {
...
static int take_opt_frame_type(
    const u8 *buf, usz n, usz *off, quic_conn_close_frame *f) {
  f->frame_type = 0;
  if (f->is_app) return 1;
