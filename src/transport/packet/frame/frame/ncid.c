...
static int put_ncid_head(quic_obuf *o, const quic_ncid_frame *f) {
  if (!quic_varint_put(
          quic_mspan_of(o->p, o->cap), &o->len, QUIC_FRAME_NEW_CID))
    return 0;
  if (!quic_varint_put(quic_mspan_of(o->p, o->cap), &o->len, f->seq)) return 0;
  return quic_varint_put(
      quic_mspan_of(o->p, o->cap), &o->len, f->retire_prior_to);
}
...
static int put_ncid_body(quic_obuf *o, const quic_ncid_frame *f) {
  ...
  if (!quic_put_bytes(
          ...
          quic_span_of(f->cid, f->cid_len)))
    return 0;
  return quic_put_bytes(
      quic_mspan_of(o->p, o->cap), &o->len,
      quic_span_of(f->token, QUIC_NCID_TOKEN));
}
...
static int take_ncid_body(quic_span in, usz *off, quic_ncid_frame *f) {
  ...
  f->cid_len = in.p[(*off)++];
  if (!quic_take_bytes(
          quic_span_of(in.p, in.n), off, quic_mspan_of(f->cid, f->cid_len)))
    return 0;
  return quic_take_bytes(
      quic_span_of(in.p, in.n), off, quic_mspan_of(f->token, QUIC_NCID_TOKEN));
}
