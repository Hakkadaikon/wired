#ifndef FRAME_CONNCTL_H
#define FRAME_CONNCTL_H

#include "common/platform/sys/syscall.h"

/* RFC 9000 19: connection and path management frames. */

#define FRAME_NEW_TOKEN 0x07      /* 19.7  */
#define FRAME_RETIRE_CID 0x19     /* 19.16 */
#define FRAME_PATH_CHALLENGE 0x1a /* 19.17 */
#define FRAME_PATH_RESPONSE 0x1b  /* 19.18 */
#define FRAME_HANDSHAKE_DONE 0x1e /* 19.20 */

#define PATH_DATA 8 /* PATH_CHALLENGE/RESPONSE payload length */

/** NEW_TOKEN (19.7): a token length and a view into the token (not copied).
 * length 0 is permitted. */
typedef struct {
  /** Token length in bytes (0 is permitted). */
  u64 length;
  /** View into the token bytes; not copied, the buffer must outlive uses. */
  const u8* token;
} new_token_frame;

/* Encode/decode NEW_TOKEN. Returns bytes written/consumed, 0 on
 * overflow or malformed/truncated input. */
usz new_token_encode(u8* buf, usz cap, const new_token_frame* f);
usz new_token_decode(const u8* buf, usz n, new_token_frame* f);

/* RETIRE_CONNECTION_ID (19.16): a single sequence number. */
usz retire_cid_encode(u8* buf, usz cap, u64 seq);
usz retire_cid_decode(const u8* buf, usz n, u64* seq);

/* PATH_CHALLENGE/PATH_RESPONSE (19.17/19.18): an 8-byte payload. type is
 * FRAME_PATH_CHALLENGE or FRAME_PATH_RESPONSE. */
usz path_encode(u8* buf, usz cap, u8 type, const u8 data[PATH_DATA]);
usz path_decode(const u8* buf, usz n, u8 type, u8 data[PATH_DATA]);

/* HANDSHAKE_DONE (19.20): type byte only, no body. */
usz handshake_done_encode(u8* buf, usz cap);
usz handshake_done_decode(const u8* buf, usz n);

#endif
