...
static int put_rs_head(quic_obuf *o, const quic_reset_stream_frame *f) {
  if (!quic_varint_put(o->p, o->cap, &o->len, QUIC_FRAME_RESET_STREAM))
    return 0;
...
static int put_rs_tail(quic_obuf *o, const quic_reset_stream_frame *f) {
  if (!quic_varint_put(o->p, o->cap, &o->len, f->error_code)) return 0;
  return quic_varint_put(o->p, o->cap, &o->len, f->final_size);
}
...
static int take_rs_head(quic_span in, usz *off, quic_reset_stream_frame *f) {
  ...
  return quic_varint_take(in.p, in.n, off, &f->error_code);
}
...
static int put_ss_head(quic_obuf *o, const quic_stop_sending_frame *f) {
  if (!quic_varint_put(o->p, o->cap, &o->len, QUIC_FRAME_STOP_SENDING))
    return 0;
  return quic_varint_put(o->p, o->cap, &o->len, f->stream_id);
}
