...
static int short_ok(usz cap, const short_desc* d) {
...
usz short_build(u8* buf, usz cap, const short_desc* d) {
  usz off = 1;
  if (!short_ok(cap, d)) return 0;
  buf[0] = short_byte0(d->spin, d->key_phase, d->pn_len);
  bytes_put(
      wired_mspan_of(buf, cap), &off,
      wired_span_of(d->dcid.p, d->dcid.n)); /* room checked */
