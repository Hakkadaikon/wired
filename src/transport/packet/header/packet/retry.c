...
static int retry_put_cid(wired_obuf* out, wired_span cid) {
...
static int retry_take_cid(wired_span buf, usz* off, wired_mspan* dst) {
  u8 len;
...
static int retry_parse_after_head(const u8* buf, usz n, retry_packet* r) {
  ...
  r->version = ((u32)buf[1] << 24) | ((u32)buf[2] << 16) | ((u32)buf[3] << 8) |
               (u32)buf[4];
  if (!retry_take_cids(wired_span_of(buf, n), &off, r)) return 0;
