 ...
 * scid_len(1) | scid. */
static void locate_cids(
    const u8 *pkt, const quic_header *h, const u8 **dcid, const u8 **scid) {
  *dcid = pkt + 6;
...
static usz quic_lhdr_parse_prefix(
    const u8 *pkt, usz len, quic_header *h, const u8 **dcid, const u8 **scid) {
  usz off;
...
static int read_token_and_length(
    const u8  *pkt,
    usz        len,
    int        is_initial,
    usz       *off,
...
int quic_lhdr_parse(
  ...
  *dcid_len = h.dcid_len;
  *scid_len = h.scid_len;
  if (!read_token_and_length(
          pkt, len, is_initial, &off, token, token_len, length))
    return 0;
  *pn_off = off;
  return 1;
}
