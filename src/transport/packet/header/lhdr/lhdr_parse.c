 ...
 * header. */
static int is_long_header(wired_span pkt) {
 ...
 * scid_len(1) | scid. */
static void locate_cids(const u8* pkt, const wired_header* h, lhdr* out) {
  out->dcid = wired_span_of(pkt + 6, h->dcid_len);
  out->scid = wired_span_of(pkt + 7 + h->dcid_len, h->scid_len);
}
...
static int read_token(const lhdrparse_src* s, usz* off, lhdr* out) {
  usz used;
  if (!s->is_initial) {
    out->token = wired_span_of((const u8*)0, 0);
    return 1;
  }
  used = inittoken_get(s->pkt.p + *off, s->pkt.n - *off, &out->token);
  if (used == 0) return 0;
  *off += used;
  return 1;
}
...
static int take_length(wired_span pkt, usz* off, lhdr* out) {
  if (!varint_take(wired_span_of(pkt.p, pkt.n), off, &out->length)) return 0;
  out->pn_off = *off;
  return 1;
}
...
static usz lhdr_take_prefix(wired_span pkt, lhdr* out) {
  wired_header h;
  usz          off;
...
usz lhdr_pn_len(u8 byte0_unprotected) {
  return (usz)(byte0_unprotected & 0x03) + 1;
}

int lhdr_parse(wired_span pkt, int is_initial, lhdr* out) {
