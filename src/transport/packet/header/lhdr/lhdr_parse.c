 ...
 * scid_len(1) | scid. */
static void locate_cids(const u8 *pkt, const quic_header *h, quic_lhdr *out) {
...
static int take_length(quic_span pkt, usz *off, quic_lhdr *out) {
  if (!quic_varint_take(pkt.p, pkt.n, off, &out->length)) return 0;
  out->pn_off = *off;
  return 1;
}
...
static usz lhdr_take_prefix(quic_span pkt, quic_lhdr *out) {
  ...
  off = quic_header_parse(pkt.p, pkt.n, &h);
  if (off == 0) return 0;
