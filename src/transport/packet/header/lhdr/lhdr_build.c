...
static usz lhdr_put_prefix(u8 *out, usz cap, u8 byte0, u32 version, const u8 *dcid,
                      u8 dcid_len, const u8 *scid, u8 scid_len, u8 pn_len)
{
    quic_header h = {0};
    usz w;
...
static int put_token(u8 *out, usz cap, usz *off, int is_initial,
...
static int put_len_pn(u8 *out, usz cap, usz *off, usz payload_len, u64 pn,
                      u8 pn_len, usz *length_off_out)
{
...
static int put_body(u8 *out, usz cap, usz *off, int is_initial,
...
usz quic_lhdr_build(u8 byte0, u32 version, const u8 *dcid, u8 dcid_len,
                    ...
                    usz *length_off_out)
{
    usz off = lhdr_put_prefix(out, cap, byte0, version, dcid, dcid_len, scid,
                         scid_len, pn_len);
    if (off == 0) return 0;
    if (!put_body(out, cap, &off, is_initial, token, token_len, payload_len,
                  pn, pn_len, length_off_out))
        return 0;
    *hdr_len_out = off;
    return off;
}
