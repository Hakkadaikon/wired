...
static void conv_first(u64 largest, u64 first_len, ack_frame* f) {
...
static void conv_pair(const u64* raw, usz i, ack_range* prev, ack_range* cur) {
  u64 gap = raw[2 * i - 1];
  u64 len = raw[2 * i];
...
static void conv_all_pairs(const u64* raw, usz n, ack_frame* f) {
  usz i;
  for (i = 1; i <= (n - 1) / 2; i++)
