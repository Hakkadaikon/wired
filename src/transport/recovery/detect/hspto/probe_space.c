...
static int early_space(int initial_inflight, int handshake_inflight) {
  if (initial_inflight) return HSPTO_SPACE_INITIAL;
  if (handshake_inflight) return HSPTO_SPACE_HANDSHAKE;
  return HSPTO_SPACE_APPLICATION;
}

int hspto_probe_space(
    int initial_inflight, int handshake_inflight, int handshake_confirmed) {
  if (handshake_confirmed) return HSPTO_SPACE_APPLICATION;
