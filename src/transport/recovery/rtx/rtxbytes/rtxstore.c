...
void rtxbytes_init(rtxbytes* st) {
  st->next = 0;
  for (usz i = 0; i < RTXBYTES_SLOTS; i++) st->s[i].used = 0;
}
...
int rtxbytes_store(rtxbytes* st, u64 pn, wired_span frame) {
  ...
  if (frame.n > RTXBYTES_FRAME) return 0;
  slot = &st->s[st->next];
  if (!bytes_put(
          wired_mspan_of(slot->data, RTXBYTES_FRAME), &off,
          wired_span_of(frame.p, frame.n)))
    return 0;
  slot->pn   = pn;
  slot->len  = frame.n;
  slot->used = 1;
  st->next   = (st->next + 1) % RTXBYTES_SLOTS;
  return 1;
}
...
int rtxbytes_get(const rtxbytes* st, u64 pn, wired_span* out) {
  for (usz i = 0; i < RTXBYTES_SLOTS; i++) {
