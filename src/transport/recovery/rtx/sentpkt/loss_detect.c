...
static int by_packet(u64 largest_acked, u64 pn) {
  return largest_acked >= pn + SENTPKT_PACKET_THRESHOLD;
}
...
static int inflight(const sentpkt* t, usz i) {
  return t->e[i].used && t->e[i].state == SP_INFLIGHT;
}

/* True when an in-flight packet has crossed either threshold. */
static int past_threshold(const sentpkt_entry* p, const loss_params* lp) {
...
static int loss_is_lost(const sentpkt* t, usz i, const loss_params* lp) {
