...
static u8 hs_pos(const quic_driver *d)
{
    return (u8)(d->tx_sent + d->rx_done);
}
...
static int do_recv(quic_driver *d)
{
    ...
    level = g_order[pos][1];
    derive_for(d, g_order[pos][0]); /* keys before this level opens (RFC 9001 4) */
...
static int do_send(quic_driver *d)
{
    ...
    stf.stream_id = 0; stf.offset = d->tx_off; stf.length = 1;
    stf.data = &msg; stf.fin = 0;
    fl = quic_frame_put_stream(frames, sizeof(frames), &stf);
    n = quic_connio_send(&d->io, level, frames, fl, d->out_buf, sizeof(d->out_buf));
    if (n == 0) return 0;
...
{
    usz i = 0;
    while (driver_run_continues(d, i, max_steps) && quic_driver_step(d))
        i++;
    return i;
}
