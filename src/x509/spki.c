...
static int spki_alg_oid(const u8 *alg, usz alg_len, const u8 **oid, usz *oid_len)
{
    quic_derseq c;
...
static int split_spki(const u8 *spki, usz spki_len, const u8 **oid,
                      usz *oid_len, const u8 **key, usz *key_len)
{
    quic_derseq c;
    const u8 *alg;
    usz alg_len;
...
{
    quic_derseq c;
    return tbs_cursor(tbs, tbs_len, &c)
        && skip_n(&c, SPKI_SKIP)
