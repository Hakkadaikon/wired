...
usz quic_h3_frame_put(u8 *buf, usz cap, u64 type, const u8 *payload,
                      usz payload_len)
{
    usz off = 0;
...
usz quic_h3_frame_get(const u8 *buf, usz n, u64 *type, const u8 **payload,
                      u64 *payload_len)
{
    usz off = 0;
...
static int get_one_head(const u8 *buf, usz n, usz *off, u64 want_type, u64 *len)
{
    u64 type;
...
static usz settings_body_len(const quic_h3_settings *s)
{
    usz total = 0;
    for (usz i = 0; i < s->n; i++)
        total += quic_varint_len(s->pairs[i].id) + quic_varint_len(s->pairs[i].value);
    return total;
}

/* Append every (Identifier Value) pair after the head. Returns 1 ok, 0. */
static int put_settings_pairs(u8 *buf, usz cap, usz *off, const quic_h3_settings *s)
{
    int ok = 1;
    for (usz i = 0; i < s->n; i++)
...
static int put_settings_head(u8 *buf, usz cap, usz *off, const quic_h3_settings *s)
{
    if (s->n > QUIC_H3_SETTINGS_MAX) return 0;
...
static int get_settings_pairs(const u8 *buf, usz end, usz *off, quic_h3_settings *s)
{
    s->n = 0;
    while (*off < end)
