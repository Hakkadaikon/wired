...
static const struct { const char *name; quic_h3_ph_kind kind; } known[] = {
    { ":method", QUIC_H3_PH_METHOD },
    { ":scheme", QUIC_H3_PH_SCHEME },
    { ":authority", QUIC_H3_PH_AUTHORITY },
    { ":path", QUIC_H3_PH_PATH },
...
void quic_h3_ph_field(quic_h3_ph_set *p, const u8 *name, usz len)
{
    ...
    if (k == QUIC_H3_PH_NONE) { p->saw_regular = 1; return; }
    if (k == QUIC_H3_PH_UNKNOWN) { p->ok = 0; return; }
    on_pseudo(p, k);
}
...
{
    u8 req = BIT(QUIC_H3_PH_METHOD) | BIT(QUIC_H3_PH_SCHEME) | BIT(QUIC_H3_PH_PATH);
    return p->ok && has_all(p, req);  /* :authority is conditional (RFC 9114 4.3.1) */
}
