...
static int second_eliciting(int ack_eliciting_received, int ack_already_pending)
{
    return ack_eliciting_received && ack_already_pending;
}

/* Any ack-eliciting packet, received now or still pending, is outstanding. */
static int has_outstanding(int ack_eliciting_received, int ack_already_pending)
{
    return ack_eliciting_received || ack_already_pending;
}
