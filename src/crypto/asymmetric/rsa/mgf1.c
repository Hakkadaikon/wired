...
static void mgf1_block(wired_span seed, u32 counter, u8 out[32]) {
  u8 c[4];
  be_put_be32(c, counter);
  sha256_ctx s;
  sha256_init(&s);
  sha256_update(&s, seed.p, seed.n);
  sha256_update(&s, c, 4);
  sha256_final(&s, out);
}
...
/* RFC 8017 B.2.1. */
void mgf1_sha256(wired_span seed, wired_mspan mask) {
  u8  t[32];
  usz off = 0;
