...
static int der_len(wired_span lf, usz* len, usz* hdr) {
  if (lf.n < 1) return 0;
  if (lf.p[0] < 0x80) {
    *len = lf.p[0];
    *hdr = 1;
...
static int der_header(wired_span buf, usz* len, usz* head) {
  usz hdr;
  if (buf.n < 2) return 0;
  if (!der_len(wired_span_of(buf.p + 1, buf.n - 1), len, &hdr)) return 0;
  *head = 1 + hdr;
  return 1;
}
...
int der_read(wired_span buf, der_tlv* out) {
  ...
  out->tag  = buf.p[0];
  out->val  = wired_span_of(buf.p + head, len);
  out->used = head + len;
  return 1;
}
