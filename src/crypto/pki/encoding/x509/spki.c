...
int x509_is_ec(wired_span alg_oid) {
  return der_oid_equal(alg_oid, wired_span_of(oid_ec, sizeof(oid_ec)));
}

int x509_is_rsa(wired_span alg_oid) {
  return der_oid_equal(alg_oid, wired_span_of(oid_rsa, sizeof(oid_rsa)));
}

int x509_is_x25519(wired_span alg_oid) {
  return der_oid_equal(alg_oid, wired_span_of(oid_x25519, sizeof(oid_x25519)));
}

int x509_is_x448(wired_span alg_oid) {
  return der_oid_equal(alg_oid, wired_span_of(oid_x448, sizeof(oid_x448)));
}

int x509_is_ed25519(wired_span alg_oid) {
  return der_oid_equal(
      alg_oid, wired_span_of(spki_oid_ed25519, sizeof(spki_oid_ed25519)));
}

int x509_is_ed448(wired_span alg_oid) {
  return der_oid_equal(alg_oid, wired_span_of(oid_ed448, sizeof(oid_ed448)));
}

int x509_is_p256(wired_span oid) {
  return der_oid_equal(oid, wired_span_of(oid_p256, sizeof(oid_p256)));
}
...
int x509_is_ecdh(wired_span alg_oid) {
  return der_oid_equal(alg_oid, wired_span_of(oid_ecdh, sizeof(oid_ecdh)));
}

int x509_is_ecmqv(wired_span alg_oid) {
  return der_oid_equal(alg_oid, wired_span_of(oid_ecmqv, sizeof(oid_ecmqv)));
}
...
static int is_ec_restricted(wired_span alg_oid) {
  return x509_is_ecdh(alg_oid) || x509_is_ecmqv(alg_oid);
}
...
int x509_ec_restricted_params_ok(wired_span tbs) {
  ...
  if (!x509_public_key(tbs, &alg_oid, &key)) return 0;
  if (!is_ec_restricted(alg_oid)) return 1;
  return x509_ec_curve(tbs, &curve_oid);
}
