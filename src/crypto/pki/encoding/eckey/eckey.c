...
static int eckey_open(wired_span der, quic_derseq* c, u64* v) {
  wired_span body, iv;
...
static int eckey_pkcs8_unwrap(quic_derseq* c, wired_span* inner) {
  wired_span alg;
...
static usz eckey_build_ed25519_privkey(const u8 seed[32], wired_obuf* out) {
  u8         inner[34];
  wired_obuf io = quic_obuf_of(inner, sizeof(inner));
  if (!quic_selfcert_der_tlv(
          QUIC_DER_OCTET_STRING, wired_span_of(seed, 32), &io))
    return 0;
  if (!quic_selfcert_der_tlv(
...
int wired_eckey_ed25519_pkcs8_encode(const u8 seed[32], wired_obuf* out) {
  ...
  wired_obuf po      = quic_obuf_of(pk, sizeof(pk));
...
static int eckey_curve25519_open(wired_span key_der, quic_derseq* c) {
  wired_span alg;
  u64        v;
  if (!eckey_open(key_der, c, &v)) return 0;
  if (v != 0) return 0;
