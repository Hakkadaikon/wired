#include "crypto/pki/cert/p256cert/p256cert.h"

#include "common/bytes/util/bytes.h"
#include "crypto/asymmetric/ecc/ecdsasig/sig_value.h"
#include "crypto/asymmetric/ecc/p256sign/sign.h"
#include "crypto/pki/cert/p256cert/enc.h"
#include "crypto/pki/cert/p256cert/tbs.h"
#include "crypto/pki/encoding/asn1/der.h"
#include "crypto/symmetric/hash/hash/sha256.h"

/* RFC 5280 4.1.1.3. signatureValue BIT STRING (0x00 unused-bits || sig DER).
 * sig is the ECDSA-Sig-Value DER of (r, s). Writes the whole TLV. 0 on fail. */
static usz pc_build_sigval(const u8 *sig, usz sig_len, u8 *out, usz cap) {
  u8  bits[80];
  usz off = 1;
  bits[0] = 0x00;
  if (!quic_put_bytes(bits, sizeof(bits), &off, sig, sig_len)) return 0;
  quic_p256cert_enc e = quic_p256cert_loaded(bits, off);
  return quic_p256cert_wrap(&e, QUIC_DER_BIT_STRING, out, cap);
}

/* SHA-256 the TBS, ECDSA-sign it, DER-encode (r, s) into sig. 0 on failure. */
static int pc_sign_tbs(
    const u8 priv[32], const u8 *tbs, usz tn, u8 *sig, usz cap, usz *sn) {
  u8 hash[32], r[32], s[32];
  quic_sha256(tbs, tn, hash);
  if (!quic_p256sign_sign(priv, hash, r, s)) return 0;
  return quic_ecdsasig_encode(r, s, sig, cap, sn);
}

int quic_p256cert_build(
    const u8 priv[32],
    const u8 pub_x[32],
    const u8 pub_y[32],
    u8      *out,
    usz      cap,
    usz     *len) {
  u8                tbs[512], alg[16], sig[80], sv[96], body[640];
  usz               tn, sn, an = quic_p256cert_sigalg(alg, sizeof(alg)), svn;
  quic_p256cert_enc e = {body, sizeof(body), 0, 1};
  if (!quic_p256cert_tbs(pub_x, pub_y, tbs, sizeof(tbs), &tn)) return 0;
  if (!pc_sign_tbs(priv, tbs, tn, sig, sizeof(sig), &sn)) return 0;
  svn = pc_build_sigval(sig, sn, sv, sizeof(sv));
  quic_p256cert_put_pre(&e, tbs, tn);
  quic_p256cert_put_pre(&e, alg, an);
  quic_p256cert_put_pre(&e, sv, svn);
  *len = quic_p256cert_wrap(&e, QUIC_DER_SEQUENCE, out, cap);
  return *len != 0;
}
