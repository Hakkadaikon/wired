...
int quic_p256cert_build(const quic_p256cert_key* k, wired_obuf* out) {
  ...
  wired_obuf        go = quic_obuf_of(sig, sizeof(sig));
  wired_obuf        vo = quic_obuf_of(sv, sizeof(sv));
  usz               an = quic_p256cert_sigalg(&ao), svn;
  quic_p256cert_enc e  = {body, sizeof(body), 0, 1};
  if (!quic_p256cert_tbs(k->x, k->y, k->san_ipv4, k->now_secs, &to)) return 0;
  if (!pc_sign_tbs(k->priv, wired_span_of(tbs, to.len), &go)) return 0;
  svn = pc_build_sigval(wired_span_of(sig, go.len), &vo);
  quic_p256cert_put_pre(&e, wired_span_of(tbs, to.len));
  quic_p256cert_put_pre(&e, wired_span_of(alg, an));
  quic_p256cert_put_pre(&e, wired_span_of(sv, svn));
