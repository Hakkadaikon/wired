...
static void pack_point(u8 bits[66], const u8 x[32], const u8 y[32]) {
  ...
  usz             off   = 0;
  bytes_put(wired_mspan_of(bits, 66), &off, wired_span_of(hdr, sizeof(hdr)));
  bytes_put(wired_mspan_of(bits, 66), &off, wired_span_of(x, 32));
  bytes_put(wired_mspan_of(bits, 66), &off, wired_span_of(y, 32));
}

int p256cert_spki(const u8 x[32], const u8 y[32], wired_obuf* out) {
