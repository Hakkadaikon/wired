...
static usz pc_build_alg(wired_obuf* out) {
  ...
  quic_p256cert_put(
      &e, QUIC_DER_OID, wired_span_of(oid_ec_pub, sizeof(oid_ec_pub)));
  quic_p256cert_put(
...
static void pack_point(u8 bits[66], const u8 x[32], const u8 y[32]) {
  ...
  usz             off   = 0;
  quic_put_bytes(
      wired_mspan_of(bits, 66), &off, wired_span_of(hdr, sizeof(hdr)));
  quic_put_bytes(wired_mspan_of(bits, 66), &off, wired_span_of(x, 32));
  quic_put_bytes(wired_mspan_of(bits, 66), &off, wired_span_of(y, 32));
}

int quic_p256cert_spki(const u8 x[32], const u8 y[32], wired_obuf* out) {
  u8                alg[32], bits[66], inner[128];
  wired_obuf        ao = quic_obuf_of(alg, sizeof(alg));
  quic_p256cert_enc e  = {inner, sizeof(inner), 0, 1};
  pack_point(bits, x, y);
  quic_p256cert_put_pre(&e, wired_span_of(alg, pc_build_alg(&ao)));
  quic_p256cert_put(&e, QUIC_DER_BIT_STRING, wired_span_of(bits, sizeof(bits)));
