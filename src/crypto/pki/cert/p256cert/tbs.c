...
usz quic_p256cert_sigalg(wired_obuf* out) {
  u8                inner[16];
...
static usz pc_build_atv(wired_obuf* out) {
  ...
  quic_p256cert_enc e = {inner, sizeof(inner), 0, 1};
  quic_p256cert_put(
      &e, QUIC_DER_OID, wired_span_of(pc_oid_cn, sizeof(pc_oid_cn)));
  quic_p256cert_put(
      &e, 0x0c,
...
static usz pc_build_validity(u64 now_secs, wired_obuf* out) {
  ...
  if (now_secs) {
    ...
    quic_p256cert_put(&e, 0x17, wired_span_of(nb, sizeof(nb)));
    quic_p256cert_put(&e, 0x17, wired_span_of(na, sizeof(na)));
  } else {
    quic_p256cert_put(
...
static void tbs_head(quic_p256cert_enc* e, wired_span name) {
  ...
  quic_p256cert_put_pre(e, wired_span_of(version, sizeof(version)));
  quic_p256cert_put_pre(e, wired_span_of(serial, sizeof(serial)));
  quic_p256cert_put_pre(e, wired_span_of(alg, quic_p256cert_sigalg(&ao)));
...
static usz pc_build_gennames(const u8* san_ipv4, wired_obuf* out) {
  ...
  if (san_ipv4)
    quic_p256cert_put(&e, PC_SAN_IPADDR_TAG, wired_span_of(san_ipv4, 4));
  return quic_p256cert_wrap(&e, QUIC_DER_SEQUENCE, out);
}
...
static usz pc_build_san_ext(const u8* san_ipv4, wired_obuf* out) {
  u8                gn[48], ext[64];
  wired_obuf        go = quic_obuf_of(gn, sizeof(gn));
  quic_p256cert_enc eg =
      quic_p256cert_loaded(gn, pc_build_gennames(san_ipv4, &go));
  quic_p256cert_enc e = {ext, sizeof(ext), 0, 1};
  quic_p256cert_put(
      &e, QUIC_DER_OID, wired_span_of(pc_oid_san, sizeof(pc_oid_san)));
  quic_p256cert_put(&e, QUIC_DER_OCTET_STRING, wired_span_of(eg.buf, eg.off));
  return quic_p256cert_wrap(&e, QUIC_DER_SEQUENCE, out);
}
...
static usz pc_build_extensions(const u8* san_ipv4, wired_obuf* out) {
  u8                ext[64], seq[80];
  wired_obuf        eo = quic_obuf_of(ext, sizeof(ext));
  wired_obuf        so = quic_obuf_of(seq, sizeof(seq));
...
int quic_p256cert_tbs(
  ...
  wired_obuf        xo = quic_obuf_of(exts, sizeof(exts));
  ...
  quic_p256cert_put_pre(
      &e, wired_span_of(val, pc_build_validity(now_secs, &vo)));
  quic_p256cert_put_pre(&e, wired_span_of(name, nn)); /* subject */
  quic_p256cert_put_pre(&e, wired_span_of(spki, so.len));
  quic_p256cert_put_pre(
      &e, wired_span_of(exts, pc_build_extensions(san_ipv4, &xo)));
