...
static usz build_sigalg(u8 *out, usz cap) {
  ...
  if (!quic_selfcert_der_tlv(
          QUIC_DER_OID, oid_ed25519_sig, sizeof(oid_ed25519_sig), oid,
          sizeof(oid), &n))
    return 0;
  if (!quic_selfcert_der_tlv(QUIC_DER_SEQUENCE, oid, n, out, cap, &n)) return 0;
  return n;
}
...
static int assemble(
    const slice *parts, usz cnt, u8 *out, usz cap, usz *out_len) {
  u8  body[768];
  usz off = 0;
  int ok  = 1;
  for (usz i = 0; i < cnt; i++)
  ...
  return ok &&
         quic_selfcert_der_tlv(QUIC_DER_SEQUENCE, body, off, out, cap, out_len);
}
...
int quic_selfcert_build(
    const u8 seed[32], u8 *cert_out, usz cap, usz *cert_len) {
