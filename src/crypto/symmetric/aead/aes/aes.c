#include "crypto/symmetric/aead/aes/aes.h"

/* FIPS 197 Figure 7 S-box. */
static const u8 SBOX[256] = {
    };

/* 10 entries cover both AES-128 (uses RCON[0..9], i/4-1 up to 9) and
 * AES-256 (uses RCON[0..6], i/8-1 up to 6). */
static const u8 RCON[10] = {0x01, 0x02, 0x04, 0x08, 0x10,
                            0x20, 0x40, 0x80, 0x1b, 0x36};

/* GF(2^8) xtime: multiply by 2 modulo the AES polynomial. */
#define XTIME(x) ((u8)(((x) << 1) ^ (((x) >> 7) * 0x1b)))

static void copy16(u8* dst, const u8* src) {
  for (usz i = 0; i < 16; i++) dst[i] = src[i];
}

static u32 sub_word(u32 w) {
  return ((u32)SBOX[(w >> 24) & 0xff] << 24) |
         ((u32)SBOX[(w >> 16) & 0xff] << 16) |
         ((u32)SBOX[(w >> 8) & 0xff] << 8) | SBOX[w & 0xff];
}

/* The per-word key-schedule core: rotate+sub+rcon on multiples of 4. */
static u32 schedule_word(u32 prev, usz i) {
  if (i % 4 != 0) return prev;
  return sub_word((prev << 8) | (prev >> 24)) ^ ((u32)RCON[i / 4 - 1] << 24);
}

/* FIPS 197 5.2 Figure 11: Nk=8 (AES-256) key-schedule core. Every 8th word
 * gets RotWord+SubWord+Rcon (as in AES-128's every-4th-word rule); Nk>6
 * additionally applies SubWord alone at the halfway word (i%Nk==4), which
 * AES-128/192 do not have. */
static u32 schedule_word256(u32 prev, usz i) {
  if (i % 8 == 0)
    return sub_word((prev << 8) | (prev >> 24)) ^ ((u32)RCON[i / 8 - 1] << 24);
  if (i % 8 == 4) return sub_word(prev);
  return prev;
}

void aes128_init(aes128* a, const u8 key[AES_BLOCK]) {
  for (usz i = 0; i < 4; i++)
    a->rk[i] = ((u32)key[4 * i] << 24) | ((u32)key[4 * i + 1] << 16) |
               ((u32)key[4 * i + 2] << 8) | key[4 * i + 3];
  for (usz i = 4; i < AES_RK_WORDS; i++)
    a->rk[i] = a->rk[i - 4] ^ schedule_word(a->rk[i - 1], i);
}

/* AddRoundKey: XOR 4 round-key words into the 16-byte state. */
static void add_round_key(u8 s[16], const u32* rk) {
  for (usz c = 0; c < 4; c++) {
    s[4 * c] ^= (u8)(rk[c] >> 24);
    s[4 * c + 1] ^= (u8)(rk[c] >> 16);
    s[4 * c + 2] ^= (u8)(rk[c] >> 8);
    s[4 * c + 3] ^= (u8)rk[c];
  }
}

static void sub_bytes(u8 s[16]) {
  for (usz i = 0; i < 16; i++) s[i] = SBOX[s[i]];
}

/* Fill one shifted column c of the output from state s. */
static void shift_col(const u8 s[16], u8 t[16], usz c) {
  for (usz r = 0; r < 4; r++) t[4 * c + r] = s[4 * ((c + r) % 4) + r];
}

/* ShiftRows on a column-major state (byte r,c at s[4*c+r]). */
static void shift_rows(u8 s[16]) {
  u8 t[16];
  for (usz c = 0; c < 4; c++) shift_col(s, t, c);
  copy16(s, t);
}

/* MixColumns on one column (4 bytes). */
static void mix_one(u8* col) {
  u8 a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
  col[0] = (u8)(XTIME(a0) ^ (XTIME(a1) ^ a1) ^ a2 ^ a3);
  col[1] = (u8)(a0 ^ XTIME(a1) ^ (XTIME(a2) ^ a2) ^ a3);
  col[2] = (u8)(a0 ^ a1 ^ XTIME(a2) ^ (XTIME(a3) ^ a3));
  col[3] = (u8)((XTIME(a0) ^ a0) ^ a1 ^ a2 ^ XTIME(a3));
}

static void mix_columns(u8 s[16]) {
  for (usz c = 0; c < 4; c++) mix_one(s + 4 * c);
}

/* One full round (rounds 1..9): sub, shift, mix, add. */
static void round_full(u8 s[16], const u32* rk) {
  sub_bytes(s);
  shift_rows(s);
  mix_columns(s);
  add_round_key(s, rk);
}

/* The final round (round 10) omits MixColumns. */
static void round_last(u8 s[16], const u32* rk) {
  sub_bytes(s);
  shift_rows(s);
  add_round_key(s, rk);
}

void aes128_encrypt(
    const aes128* a, const u8 in[AES_BLOCK], u8 out[AES_BLOCK]) {
  u8 s[16];
  copy16(s, in);
  add_round_key(s, a->rk);
  for (usz r = 1; r < AES_ROUNDS; r++) round_full(s, a->rk + 4 * r);
  round_last(s, a->rk + (usz)4 * AES_ROUNDS);
  copy16(out, s);
}

void aes256_init(aes256* a, const u8 key[AES256_KEY]) {
  for (usz i = 0; i < 8; i++)
    a->rk[i] = ((u32)key[4 * i] << 24) | ((u32)key[4 * i + 1] << 16) |
               ((u32)key[4 * i + 2] << 8) | key[4 * i + 3];
  for (usz i = 8; i < AES256_RK_WORDS; i++)
    a->rk[i] = a->rk[i - 8] ^ schedule_word256(a->rk[i - 1], i);
}

void aes256_encrypt(
    const aes256* a, const u8 in[AES_BLOCK], u8 out[AES_BLOCK]) {
  u8 s[16];
  copy16(s, in);
  add_round_key(s, a->rk);
  for (usz r = 1; r < AES256_ROUNDS; r++) round_full(s, a->rk + 4 * r);
  round_last(s, a->rk + (usz)4 * AES256_ROUNDS);
  copy16(out, s);
}
