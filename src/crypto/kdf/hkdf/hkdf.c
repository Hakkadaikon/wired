...
static void expand_block_384(
    const quic_hkdf_xctx_384* c, quic_mspan t, u8 counter) {
  ...
  buf[n++] = counter;
  quic_hmac_sha384(
      quic_span_of(c->prk, QUIC_HKDF_PRK_384), quic_span_of(buf, n), t.p);
}
...
/* Append src into info at *off, advancing *off. */
static void append_384(u8* info, usz* off, quic_span src) {
  for (usz i = 0; i < src.n; i++) info[*off + i] = src.p[i];
  *off += src.n;
}
...
static usz build_label_384(u8* info, const quic_hkdf_label* l, usz len) {
  ...
  info[n++]                 = (u8)len;
  info[n++]                 = (u8)(6 + l->label_len);
  append_384(info, &n, quic_span_of(prefix, 6));
  append_384(info, &n, quic_span_of((const u8*)l->label, l->label_len));
  info[n++] = (u8)l->ctx.n;
...
int quic_hkdf_expand_label_384(
  ...
  return quic_hkdf_expand_384(prk, quic_span_of(info, info_len), okm);
}
