...
static void expand_block(const quic_hkdf_xctx* c, wired_mspan t, u8 counter) {
  ...
  buf[n++] = counter;
  quic_hmac_sha256(
      wired_span_of(c->prk, QUIC_HKDF_PRK), wired_span_of(buf, n), t.p);
}
...
int quic_hkdf_expand_label(
  ...
  return quic_hkdf_expand(prk, wired_span_of(info, info_len), okm);
}

void quic_hkdf_extract_384(
    wired_span salt, wired_span ikm, u8 prk[QUIC_HKDF_PRK_384]) {
  quic_hmac_sha384(salt, ikm, prk);
}
...
static void expand_block_384(
    const quic_hkdf_xctx_384* c, wired_mspan t, u8 counter) {
...
/* Build the HkdfLabel struct (RFC 8446 7.1):
 *   uint16 length; opaque label<7..255> = "tls13 "+label; opaque context<>. */
static usz build_label_384(u8* info, const quic_hkdf_label* l, usz len) {
  static const u8 prefix[6] = {'t', 'l', 's', '1', '3', ' '};
  usz             n         = 0;
  info[n++]                 = (u8)(len >> 8);
  info[n++]                 = (u8)len;
...
int quic_hkdf_expand_label_384(
    const u8               prk[QUIC_HKDF_PRK_384],
  ...
  return quic_hkdf_expand_384(prk, wired_span_of(info, info_len), okm);
}
