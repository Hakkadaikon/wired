...
static void cur_keys(const quic_connrunner *r, quic_initial_keys *out)
{
    ...
    *out = quic_keyset_for_level(&r->io.loop.keys, QUIC_LEVEL_ONERTT, &k) ? *k : z;
}
...
static int recv_next_allowed(const quic_connrunner *r, int recv_bit, int cur_bit)
{
...
static int may_initiate(const quic_connrunner *r, u64 now, u64 threshold,
                        u64 pto)
{
    return quic_kudrive_should_initiate(r->ku_sent_in_phase, threshold,
                                        r->io.loop.handshake_confirmed)
        && !r->ku_unacked
        && reinit_floor_ok(r, now, pto);
}
...
static void do_initiate(quic_connrunner *r)
{
    ...
    quic_kuswitch_apply_phase(&r->ku_phase, r->ku.generation); /* ... before toggle */
    r->ku_unacked = 1;
