...
{
    while (t->count > 0 && over_capacity(t, need))
        evict_oldest(t);
}

static int can_insert(const quic_qpack_dyn *t, usz need,
                      usz name_len, usz value_len)
{
...
static void store_entry(quic_qpack_dyn *t, const u8 *name, usz name_len,
...
int quic_qpack_dyn_insert(quic_qpack_dyn *t, const u8 *name, usz name_len,
                          const u8 *value, usz value_len)
{
