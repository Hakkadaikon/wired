...
{
    u8 ph[12];
    ph[0] = (u8)(src >> 24); ph[1] = (u8)(src >> 16);
    ph[2] = (u8)(src >> 8);  ph[3] = (u8)src;
    ph[4] = (u8)(dst >> 24); ph[5] = (u8)(dst >> 16);
    ph[6] = (u8)(dst >> 8);  ph[7] = (u8)dst;
    ph[8] = 0; ph[9] = QUIC_IP_PROTO_UDP;
...
static void put_udp(u8 *out, u16 sport, u16 dport, u16 udp_len,
                    const u8 *payload, usz payload_len)
{
    put_be16(out, sport);
    put_be16(out + 2, dport);
    put_be16(out + 4, udp_len);
    out[6] = 0; out[7] = 0; /* checksum placeholder */
