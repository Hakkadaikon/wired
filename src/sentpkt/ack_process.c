...
static int pn_within(u64 pn, u64 lo, u64 hi)
{
    return pn >= lo && pn <= hi;
}
...
static int slot_in_range(const quic_sentpkt *t, usz i, u64 lo, u64 hi)
{
    ...
    return p->used && p->state == QUIC_SP_INFLIGHT &&
           pn_within(p->pn, lo, hi);
}
...
void quic_ack_process(quic_sentpkt *t, u64 ack_largest,
