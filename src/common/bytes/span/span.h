 ...
 *
 *   (const u8 *p, usz n)            -> wired_span   (by value: 16 bytes, two
 *   (u8 *p, usz n)                  -> wired_mspan   registers under SysV, the
 *                                                   same as the loose pair)
 *   (u8 *out, usz cap, usz *out_len)-> wired_obuf *  (one register instead of
 ...
 * wired_span_of(); pass an empty input as `wired_span_of(0, 0)` (also written
 ...
 *
 * Same shape as wired_span but writable: used for fixed-size outputs where
 * the callee fills exactly `n` bytes (e.g. a key or digest buffer).
 * Construct with wired_mspan_of().
 */
typedef struct {
 ...
 *
 * The span borrows p; the buffer must outlive every use of the returned
 * value. Example: `wired_mspan out = wired_mspan_of(key, sizeof key);`
 *
 * @param p first byte of the writable range
 * @param n number of writable bytes
 * @return the view as a value type
 */
static inline wired_mspan wired_mspan_of(u8* p, usz n) {
  wired_mspan s = {p, n};
  return s;
}
