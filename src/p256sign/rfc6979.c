...
static usz ps_build_seed(u8 msg[97], const u8 v[32], u8 sep,
                      const u8 priv[32], const u8 hred[32])
{
...
/* One HMAC step that mixes in (sep, priv, hred): K = HMAC(K, seed); V = HMAC(K, V). */
static void ps_mix(u8 k[32], u8 v[32], u8 sep, const u8 priv[32], const u8 hred[32])
{
    u8 msg[97];
