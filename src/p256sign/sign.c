...
/* s = k^-1 * (e + r*d) mod n. */
static void ps_compute_s(p256_fe s, const u8 kb[32], const p256_fe e,
                      const p256_fe r, const p256_fe d)
{
...
int quic_p256sign_sign(const u8 priv[32], const u8 hash[32],
                       u8 r[32], u8 s[32])
{
    u8 kb[32];
