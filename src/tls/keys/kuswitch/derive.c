...
static void kuswitch_derive_key_iv(
    const u8 next_secret[HKDF_PRK], initial_keys* next_keys, usz key_len) {
  ...
  hkdf_expand_label(
      next_secret, &li, wired_mspan_of(next_keys->iv, INITIAL_IV));
}
...
void kuswitch_next_keys(
  ...
  kuswitch_derive_key_iv(next_secret, next_keys, INITIAL_KEY);
}
...
void kuswitch_next_keys_suite(
    ...
    u8            next_secret[HKDF_PRK]) {
  ku_next_secret(current_secret, next_secret);
  kuswitch_derive_key_iv(next_secret, next_keys, aead_key_len(suite));
}
