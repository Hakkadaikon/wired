...
static void fill_pad(u8 out[64]) { for (usz i = 0; i < 64; i++) out[i] = 0x20; }
...
static int cert_spki_key(const u8 *cert, usz cert_len,
...
static int ecdsa_inputs(const u8 *cert, usz cert_len, const u8 *sig,
                        usz sig_len, u8 x[32], u8 y[32], u8 r[32], u8 s[32])
{
    const u8 *key;
    usz key_len;
...
static int verify_ecdsa(const u8 *cert, usz cert_len, const u8 *sig,
...
static int verify_rsa(const u8 *cert, usz cert_len, const u8 *sig,
                      usz sig_len, const u8 hash[32])
{
    const u8 *key, *n, *e;
...
static int verify_ed25519(const u8 *cert, usz cert_len, const u8 *sig,
                          usz sig_len, const u8 content[130])
{
    const u8 *key;
...
static int verify_hashed(u16 scheme, const u8 *cert, usz cert_len,
                         const u8 *sig, usz sig_len, const u8 content[130])
{
    u8 hash[32];
