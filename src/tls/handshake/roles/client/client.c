...
{
    (void)server_name; (void)sni_len; /* SNI carried by tlsdriver's ClientHello */
    if (!quic_rng_bytes(c->my_priv, QUIC_ECDHE_LEN)) return 0;
    quic_x25519_base(c->my_pub, c->my_priv);
    c->fd = quic_udp_socket();
    if (c->fd < 0) return 0;
    quic_udp_addr(&c->peer, port, server_ip[0], server_ip[1],
...
usz quic_client_build_initial(quic_client *c, u8 *out, usz cap)
{
    ...
    if (!quic_crypto_stream_emit(ch, ch_len, 0, QUIC_CLIENT_CRYPTO_FRAME,
                                 out, cap, &frame_len))
        return 0;
...
/* RFC 9001 4.1: ServerHello fixes the ECDHE secret; seed fullhs from it. */
