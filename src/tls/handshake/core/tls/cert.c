...
static int take_entry(cur* c, tls_cert_entry* first) {
  wired_span cert, ext;
 ...
 * closed on overflow). */
static int take_next(cur* lc, const tls_cert_chain_out* out) {
...
static int entries_loop(cur* lc, const tls_cert_chain_out* out) {
  while (lc->off < lc->n)
