...
static void sh_ext(unsigned t, const u8 *d, usz dlen, u8 pub[32],
                   u16 *version, int *have_ks)
{
    if (t == QUIC_EXT_KEY_SHARE) *have_ks = quic_tls_ext_key_share_parse(d, dlen, pub);
    else if (t == QUIC_EXT_SUPPORTED_VERSIONS) take_version(d, dlen, version);
}
...
{
    u8 type;
    return quic_hs_parse(buf, n, &type, body_len) == 4 && type == QUIC_HS_SERVER_HELLO;
}
