...
static void protection_keys(const u8 ts[QUIC_HKDF_PRK], quic_initial_keys *out)
{
                         ...
                         quic_initial_keys *out)
{
    ...
    quic_tls_derive_secret(early, "c e traffic", 11,
                           client_hello, client_hello_len, ts);
    protection_keys(ts, out);
}
