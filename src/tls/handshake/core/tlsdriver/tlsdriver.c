...
void quic_tlsdriver_init(quic_tlsdriver *d, const u8 my_priv[QUIC_ECDHE_LEN],
                         const u8 my_pub[QUIC_ECDHE_LEN], int is_server)
{
    usz i;
...
int quic_tlsdriver_client_hello(quic_tlsdriver *d, u8 *out, usz cap,
                                usz *out_len)
{
    u8 ch[512];
    static const u8 random[32] = {0};
    static const u8 tp[1] = {0};
    usz n = quic_tls_client_hello(ch, sizeof(ch), random, d->my_pub,
                                  0, 0, tp, sizeof(tp));
    if (n == 0) return 0;
    return quic_crypto_stream_emit(ch, n, 0,
                                   QUIC_TLSDRIVER_CRYPTO_MAX, out, cap, out_len);
}
...
static int peer_keyshare(const quic_tlsdriver *d, const u8 *msg, usz n,
                         u8 pub[32])
{
    u16 cipher, version;
...
static int derive_handshake(quic_tlsdriver *d, const u8 *msg, usz n)
{
    ...
    if (!quic_keysched_advance_handshake(&d->ks, d->shared, QUIC_ECDHE_LEN, msg, n))
        return 0;
...
static int reassemble(quic_tlsdriver *d, const u8 *frame, usz len,
                      u8 *msg, usz cap, usz *n)
{
    if (!feed_all(d, frame, len)) return 0;
    return quic_crypto_stream_read(&d->rx, msg, cap, n) && *n != 0;
}

int quic_tlsdriver_recv_crypto(quic_tlsdriver *d, const u8 *crypto_frame,
                               usz len)
{
    u8 msg[512];
    usz n = 0;
