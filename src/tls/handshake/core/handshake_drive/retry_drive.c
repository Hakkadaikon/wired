...
static void retry_emit(
    const quic_retry_packet *r, const quic_retry_process_out *out) {
  usz off = 0;
  quic_put_bytes(
      quic_mspan_of(out->token->p, out->token->cap), &off,
      quic_span_of(r->token, r->token_len));
  out->token->len = r->token_len;
  off             = 0;
  quic_put_bytes(
      quic_mspan_of(out->new_dcid, r->scid_len), &off,
      quic_span_of(r->scid, r->scid_len));
