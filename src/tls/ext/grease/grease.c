...
usz quic_grease_encode(u8 *buf, usz cap) {
  usz off = 0;
  if (!quic_varint_put(buf, cap, &off, QUIC_TP_GREASE_QUIC_BIT)) return 0;
  if (!quic_varint_put(buf, cap, &off, 0)) return 0; /* empty value */
  return off;
}
...
static int take_grease(const u8 *buf, usz n, usz *off) {
  ...
  if (!quic_varint_take(buf, n, off, &len)) return 0;
