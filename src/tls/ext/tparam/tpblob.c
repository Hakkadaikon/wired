...
static int blob_take_hdr(const u8 *buf, usz n, usz *off, u64 *id, u64 *len) {
  int ok =
      quic_varint_take(buf, n, off, id) & quic_varint_take(buf, n, off, len);
  return ok && *len <= n - *off;
}
...
static int pa_take_addrs(
    const u8 *v, usz len, usz *off, struct quic_preferred_address *pa) {
...
static int pa_take_cid_token(
    const u8 *v, usz len, usz *off, struct quic_preferred_address *pa) {
  int ok = quic_take_bytes(v, len, off, &pa->cid_len, 1) & (pa->cid_len <= 20);
...
static int pa_parse_value(
    const u8 *v, usz len, struct quic_preferred_address *pa) {
  usz off = 0;
  int ok =
      pa_take_addrs(v, len, &off, pa) & pa_take_cid_token(v, len, &off, pa);
  return ok && off == len;
}
