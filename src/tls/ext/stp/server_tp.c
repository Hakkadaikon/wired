...
static const struct { u64 id, val; } int_params[] = {
...
static int put_blob(u8 *out, usz cap, usz *off, u64 id,
...
{
    usz off = 0;
    int ok = put_blob(out, cap, &off, QUIC_TP_ORIGINAL_DESTINATION_CONNECTION_ID,
                      original_dcid, odcid_len)
           & put_int_params(out, cap, &off)
           & put_blob(out, cap, &off, QUIC_TP_INITIAL_SOURCE_CONNECTION_ID,
