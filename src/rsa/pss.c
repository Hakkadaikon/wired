...
static int pss_hprime_eq(const u8 *mhash, const u8 *salt, usz salt_len,
                         const u8 *h)
{
    u8 zeros[8] = {0};
    u8 hp[32];
...
static void pss_recover_db(const u8 *em, usz db_len, u8 *db, const u8 **h,
                           usz zero_bits)
{
...
static int pss_db_consistent(const u8 *em, usz db_len, usz zero_bits,
                             const u8 *mhash)
{
    u8 db[PSS_MAX_EM];
    const u8 *h;
