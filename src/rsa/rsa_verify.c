...
static const u8 sha256_prefix[19] = {
...
{
    for (usz i = 0; i < sizeof(sha256_prefix); i++) em[off + i] = sha256_prefix[i];
...
/* RFC 8017 8.2.2 steps 2-4: m = s^e mod n, then EM compare. s,n already < range. */
static int rsa_check(const quic_bn *bn_s, const quic_bn *bn_n, usz n_len,
                     const u8 *h, usz h_len)
{
    quic_bn bn_e, m;
    rsa_e_f4(&bn_e);
...
int quic_rsa_pkcs1_verify(const u8 *n, usz n_len, const u8 *sig, usz sig_len,
                          const u8 *msg_hash, usz hash_len)
{
